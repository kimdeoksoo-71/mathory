...
## Phase 65: 편집창 줄바꿈 끄기(VS Code식) · 블록 내 가로 스크롤 ✅ (본체 구현·검수 완료 2026-09-08 · **push 대기** · ⚠ 거터 1건 미해결)
...
- **검수 보완 — 좌측 행번호 거터(2회 왕복, 1차 전량 원복)**: 폭 2/3·구분선 강화는 되돌렸다
  ...
  ⚠ **"좌우 스크롤 시 행번호 거터가 밀린다"는 미해결로 보류**(2026-09-08 덕수: "크게 흉하진 않으니
  오늘은 이쯤"). **원인 미특정 · 처방 2건 실패** — ⓐ 넘침이 0인 요소(`.scaled-editor`)에 건
  `overscroll-behavior-x`는 브라우저가 스크롤 대상으로 고려조차 하지 않아 무의미했고 ⓑ `.content-frame`
  가로 패닝 제거는 경로 하나를 실제로 닫았으나 증상이 남았다(제3의 경로 존재).
  확정 사실: CM 거터 sticky는 **블록 안에서 정상**(400px 스크롤에 거터 이동 0, 3회 실측).
  ⚠ **재현 프로브로 3연속 오진했다** — 다음 착수는 실물에서 `scroll` 이벤트(capture)로 실제로
  스크롤되는 요소를 특정하는 것부터. 상세는 실행판 §9-4′ ②.
  행번호 서식은 조정 완료: 0.92em · Mathory 레드 55% · 활성 행 dark 85% · tabular-nums
