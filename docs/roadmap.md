...
- **S3 끔 모드 가로(B)**: 타자·삭제로 경계에 닿으면 커서 중앙(`centerCursorOnEdge`, 즉시·대칭)
- **S4 드로어 열림(C)**: Row 1은 저장까지·Row 2는 탭까지 밀고 클러스터(버전·댓글·Agent·스테퍼)는 덮인다 · nowrap · 제목/폴더 경로 잘라내기
- **S5 저장(D)**: 저장 후 편집기 리마운트 제거(로컬 id 유지 · dirty effect가 `problem`을 안 본다 — 잠복 버그 2건 해소) · 30분 silent 자동 저장
- **S6 블록 복사·붙여넣기(E)**: 메모리 싱글턴 · Row 2 버튼 2 · ⌘C/⌘V(접힘 모드 · 선택 텍스트 양보)
...
## 개선묶음 M8: 모바일 웹 디자인·기능 개선 ✅ (구현·덕수 검수 종결 2026-09-12 "모두 정상" · **배포 완료 2026-09-12** — 배포본 검수 정상)

계획서: `docs/phasedocs/개선묶음 M8 모바일 웹 디자인·기능 개선 v1 실행판.md`
(덕수 메모 → **v1 CLI 실측 = 실행판**. §9가 구현·검수 기록. 별도 검증 턴 없이 착수)

**서버 0 · 규칙 0 · 스키마 0 · raw_text 0 · 전처리 0 · 렌더 5사이트 0 · 데스크톱 픽셀 0.** Phase 64 폰 셸 실사용 판정 4건.
수정 10 · 신설 2(`lib/chromeAutoHide.ts` · `tests/chromeAutoHide.test.mjs`) · 커밋 S1~S4 · 로직 검증 387 → **398건**(`test:chrome` 11).

- **S1 워드마크**: 작은(19px) 사양을 `--wordmark-small`(#944728) 토큰 하나로 — PhoneShell 상단 바(옛 MiniShell 사양 600·무그림자였다)·MiniShell(Q3)을 Sidebar 사양(400·그림자)으로. 사양은 이제 두 벌(작은 19 · 큰 48)
- **S2 리더**: `[풀이 보기]`·`[문제 보기]` 알약·문제 보기 시트 삭제(이동 경로 = 탭 행 하나) · 탭별 읽던 위치 기억(Q2) · PhoneShell `forwardRef` 핸들 `getScrollTop/scrollTo`(무시 창 동반). 정의부 보존 div는 유지
- **S3 시트**: CommentPanel `chrome: 'drawer'|'sheet'`(기본 drawer = 데스크톱 무변경) — 시트 안에서 상자·테두리·그림자·`maxWidth 90vw`(393px 폰에서 패널을 354로 잘라 좌측 빈 띠를 만들던 원인)·1행 X 제거 · PhoneApp·PhoneReader 래퍼 여백 제거 · 공개 PublicComments는 PublicProblemView가 12px로 감쌈(픽셀 0)
- **S4 가로 보기**: 크롬(상단 바+탭 행 96px = 가로 뷰포트의 24%) 자동 숨김 — 순수 리듀서 4중 가드(러버밴드 무시 · 토글/프로그램적 스크롤 뒤 300ms 무시 창 · 히스테리시스 24 · 짧은 콘텐츠 금지) · 위로 24px 즉시 복귀(Q1) · 맨 위 항상 표시 · 세로 무접촉 · 루트 safe-area 좌우(Q4)
- ⚠ 수확: 무시 창이 없으면 접힘 직후 브라우저의 scrollTop 클램프 이벤트가 '위로 밀기'로 읽혀 **접자마자 펼친다** · 초기 `lastY 0` 함정(가로 전환 순간 현재 위치를 기준점으로) — 테스트 표본 3건이 같은 이유로 먼저 틀렸다
- 후속: 시트 내용의 가로 safe-area · 세로 보기 크롬 숨김(메모 범위 밖) · Phase 64 §10 잔여
...
`SidebarSectionHeader.tsx`) · ICONS 56 → **61종**(+9/−4 · toggle 2종은 검수로 철회 · user-circle 5차 · 헤더 bold 3종 6차) · 자산 3,024 → **1,512개**(regular 단일) ·
커밋 S1~S12 + 검수 후속 6(1차 6건 · 2차 카드 그림자 · 3차 색 팔레트 전수 정리 · 4차 체크박스·파비콘 · 5차 My user-circle · 6차 헤더 bold·13.5/700) · 로직 검증 371 → **373건**(`test:list` +2).
덕수 검수 종결(2026-09-09): 6차까지 반영 — 전항 종결.
...
- **카드**: 문제 카드 radius 12 + 은은한 그림자 `--card-shadow-problem` / 풀이 **0(직각)**·그림자 없음(D9~D11 → 검수 2차 확정) —
  테두리는 둘 다 0.5px `--border-content`(D10 hairline·검수 1차 1px은 "부담스럽다"로 원복, `--border-card-problem` 삭제)
...
- **덕수 검수 7차(2026-09-10)**: 문단폭 스테퍼 아이콘 `arrows-out-line-horizontal` → `text-h`(IconTextWidth 키 유지, 2곳)
- **덕수 검수 6차(2026-09-09)**: 최상위 카테고리 헤더 3종 아이콘 **bold** + 글자 12.5/600 → 13.5/700(regular 단일 규약의 명시 예외)
- **덕수 검수 5차(2026-09-09)**: 사이드바 'My' 헤더 아이콘 folder → `user-circle`(IconUserCircle)
...
- **검수 보완 — 좌측 행번호 거터(2회 왕복, 1차 전량 원복)**: 폭 2/3·구분선 강화는 되돌렸다
  (minWidth 1.8em은 2자리가 꽉 채우는 값이라 낮추면 블록마다 열 폭이 갈리고, 선을 진하게 하면
  '요약에 넣기' 블록의 얇은 바에서 끊겨 보인다). 숫자 정렬은 `tabular-nums`.
  ⚠ **"거터가 밀린다"의 정체는 스크롤이 아니라 끝의 튕김(러버밴드)이었다**(2026-09-09 덕수 영상). sticky 거터는
  스크롤 콘텐츠 레이어 안이라 컴포지터 탄성 이동에 딸려 간다 — `scrollLeft`가 변하지 않아 프로브 5회로는
  못 봤다. **S11**: 끔 모드 거터를 `fixed`로(`.cm-editor`에 transform → containing block · 패딩은 ResizeObserver ·
  툴팁은 body). 본문·커서는 그대로 튕긴다(의도). **S10**: 조상 패닝 트립와이어(`data-nohscroll`) — 별개로 실재하던
  경로의 봉인. 규약은 CLAUDE.md 두 절이 소유. 덕수 실기기 확인 "해결"
...
**서버 0 · 규칙 0 · 스키마 0 · 전처리 0.** 신규 3 · 커밋 12(구현 7 + 검수 5). 로직 검증 336 → **341건**.

- **B4** 요약보기 여닫이 상하 떨림 — `.outline-section` 상하 패딩·마진을 좌우처럼 **상시**로(D7′).
  조건부 패딩의 마진 붕괴 반전 + 스크롤 앵커 보정이 떨림의 전말이었다. 음수 마진 상쇄는 기각(web v3)
- **B3** '앞부분 펼치기' 헛표시 — `needsPrefaceToggle`(lib 소유, test:case 41→46)로 "요약 항목으로
  닿을 수 없는 블록"이 있을 때만. items 없는 전문은 계속 조용(D9=(b), 덕수: "요약은 요약일 뿐")
- **B2** 분수 가로바 연장(`--frac-ext`/`--frac-nest`, globals 한 곳 — 인쇄 같은 document) + 번분수
  세로 여백(`katexMacros()` 팬텀, **팩토리 — \gdef in-place 오염을 프로브로 실증**). 검증 카드·`\cfrac` 의도적 제외
- **B1**(`\[..\]→$$`)은 확인 결과 **이미 세 겹**(시트 정규화·[교정]·렌더)으로 구현돼 있어 작업 0
- **아이콘 5건**: 코칭 Tip 17·라벨 상시·클릭 범위 확대 / `IconComment` 신설(💬 이모지 5곳 대체,
  공개 뷰어 로컬 BubbleIcon 통일) / OCR을 수식 컨텍스트 스왑 그룹으로 / 팔레트 트리거(테라코타 틴트·
  고딕 시그마·`toolbarIcons.tsx` 신설로 순환 import 회피) / 스테퍼 정비('em' 제거·`IconTextWidth`·
  큰A작은A·`SizeStepper` 공용화로 바이트 동일 사본 4벌 해소)

**검수 6~13차(같은 날)**: 편집창 댓글 아이콘 17·숫자 500 · agent 토글 "AI"→"Agent" 4곳(편집창
통짜 13/500 확정) · 드로어 1행 제목 3종(댓글 아이콘·'Agent'·버전 기록 아이콘). 시각 최종값은 실행판 §4-2 표.

⚠ 사고 기록: dev 중 `npm run build` → 다음 핫 리로드에서 `/` 404(같은 `.next` 공유). CLAUDE.md 규칙 5에 명문화.
...
(v1 web → v4 CLI 실측 교차검토 → **v5 = 실행판**. v2·v3는 만들지 않았다 —
 §3이 v1 정정 4건(E1~E4)·보완 9건, §3-1이 v4 → v5 착수 점검 보강 4건(S10~S13))

**수정 5파일 · 신규 0 · 커밋 5(S1~S5). 서버 로직 0(상한 배선만) · Firestore 규칙 0 · 스키마 0 ·
마이그레이션 0 · 리포트 카드 0 · 클라이언트 0 · 렌더 5사이트 0 · 전처리 0 · 그림 경로(61f) 0 ·
일괄 검증(61d) 코드 0.** 로직 검증 365 → **371건**(`test:verify` 40 → 46). 프로덕션 빌드 통과.

시트 STEP3가 2026-09-06에 결함 유형을 **2 → 6종**으로 넓히고 1차를 recall 지향으로 뒤집었다
(`prompts/pmt.csv` `dda7bba` · `QualityVerification.gs` `95556fc`). 61b가 같은 STEP3를 이식한
것이라 구조가 같아, 풀이 **논리 패스**가 그 대응 지점이다. **원본이 시트인 첫 사례**라
`prompts.ts` 헤더에 계보를 남겼다 — 그 두 절의 이후 개정은 다시 이 파일이 원본이다.

- **논리 태그 4 → 7종**: `근거없는가정`·`충분성미확인` 신규 + `수식비일관`을 논리 패스에서도 발급
  (시트 (F)를 태그를 늘리지 않고 흡수). 1차에 "답이 맞더라도 논증이 정당화 못하면 결함" 전제와
  **전역 검토** 관점이 들어갔다 — 조건이 다 쓰였나 · 경우가 전체를 덮나 · 확정 전 충분성을 봤나.
- **2차에 유형별 valid 기준**(`[2']`). 판정 근거를 실제 수학적 검증으로 요구한다(재구성·반례·계산).
- **상한**: 논리 8 → 12 · 병합 12 → **20**(= 패스 상한의 합). 상한의 단일 출처를 `prompts.ts`로.

⚠ **E1 — 교차 병합은 구성상 무효였다**: v1은 "논리가 먼저 잘린다"를 교차 병합으로 풀려 했으나,
계산 패스 cap이 8인 한 이어 붙이기와 **결과 집합이 같고 순서만 달라진다**. 논리 몫을 늘리는 것은
전적으로 병합 상한이다. 테스트가 붙은 함수를 효과 없는 이유로 바꾸지 않았다.
⚠ **E3 — 프로브가 상한을 하드코딩하고 있었다**: 그대로 두면 회귀 측정이 프로덕션과 다른 상한에서
돈다. 상한 단일 출처(D9)의 직접적 동기이고, T6가 프롬프트 문자열과 `cap`의 일치를 고정한다.
⚠ **E4 — `추론/infer/invalid` 힌트는 유해했다**: 풀이 폴백이 이미 `논리오류`라 무익하고,
동시에 걸리는 문자열에서 뒤 힌트를 가로챈다. 시트가 그 힌트를 두는 것은 폴백이 `logic_gap`이기 때문.
⚠ **D5 — 시트 2차의 uncertain 조임 문구는 유일한 비이식**: `CONSERVATIVE_RULES`와 정면 충돌한다.
문장을 빼는 것만으로는 부족해 `[2']` 머리에 "가리지 못하면 uncertain" 한 줄을 넣었다(S1).
⚠ **잠복 결함 1건 동반 수정(S2)**: `alive`는 파싱 실패를 거른 배열이라 `passes[i]`로 대응시키면
한 패스가 죽었을 때 상한이 어긋난다(계산이 죽으면 논리에 cap 8). `{pass, json}` 짝짓기로 고쳤다.

**남은 것**: 프로브 회귀 측정(후보 0건 비율 · 2차 기각률 61b 베이스라인 56% · 2차 소요 시간
200초 문턱 · **유형 쏠림**) → 편집창 UI 1회(새 태그 칩 · 전역 결함 인용의 `quoteFound`) → 덕수 검수.
⚠ 검출 수의 ±1은 신호가 아니다 — n=10 표본으로 판본을 가리지 말 것(61b 교훈).
...
| 항목 | 처방 |
|---|---|
| **A** 폴더뷰 클릭 | 클릭 = 선택 · 더블클릭 = 진입 · Shift 범위·⌘ 토글(리스트) · 카드 단일 선택 · 터치 재탭 진입 · 선택 바는 2건 이상·체크박스 선택만 · Enter 진입 · 빈 곳 해제 |
| **B** 우측 패널 지속 | `lib/panelStore.ts` — 열림·세션·모델 칩·진행 표시·초안(모드별)·스크롤·폭·캐시가 문항 보기 ↔ 편집창 전환을 넘는다(호이스팅 없음). 문항 밖으로 나가면 닫힘 |
| **C** AI 비용 | Gemini 사고 토큰 합산(빠져 있었다) · `lib/aiPricing.ts` 단가 표 · 운영 1차 Gemini $0 해소 · 헤더 배지 하나(모델별 tooltip) · 교정·자동완성 비용 status |
| **D** 폰 가로 크롬 | 맨 위에서만 복귀(M8 Q1 번복) · 맨 위 판정을 무시 창 앞으로 |
| **E** ⇒ 행 위치 | 원인은 분리가 아니라 렌더 — `MATH_ONLY_LINE_RE`가 `⇒ $…$` 행을 몰라 문단이 합쳐졌다. 정규식 1줄 |
| **F** 빈 탭 | 문항 단위 원자 저장(writeBatch) · 로드·복원 시드 · 자리표시자 · 새 문항 보상 삭제 — 교차 탭 두 벌 잠복도 함께 |
| **G** ㄱ. 내어쓰기 | 원인 = 초성 자모 U+1100(`'ㄱ'.normalize('NFKC')`). `lib/invisibles.ts` 정규화(NFD 한글 보존) · 마커 비ASCII 공백 관용 · 편집기 가시화 |
...
계획서: `docs/phasedocs/Phase67 사이드바 hover peek v4 착수판.md`
(덕수 구상 → v1 web → v1 CLI 교차검토 → v2 CLI + 검증 에이전트 → v3 web → **v4 CLI 착수판**. §13이 구현 기록)

접힘 레일의 My·공유·최근 문항 아이콘에 포인터를 올리면 패널이 본문 위로 떠 있는 카드로 펼쳐지고(본문 리플로 0) 그 섹션만 열린다.
Chrome 세로 탭 hover 펼침과 같은 문법 — 우측 radius 10 + 드로어 그림자, 펼침 200ms · 접힘 160ms `cubic-bezier(0.33,0,0.2,1)`.
**AppShell 0 · 서버 0 · 규칙 0 · 스키마 0 · Firestore 0 · 아이콘 추가 0 · 폰 0.** 신규 3 · 수정 5 · 커밋 S1~S6 · 로직 검증 448 → **492건**(`test:peek` 44).

| 스텝 | 커밋 | 내용 |
|------|------|------|
| S1 | `958bb28` | 상태기계 리듀서 `lib/sidebarPeek.ts` + `peekView` · `test:peek` 44 |
| S2 | `a5862a5` | ShareTree 제어형(공유 상태 Sidebar 소유) · props · transition 토큰 2 — 화면 불변 |
| S3 | `c22340a` | aside 세 겹(자리·패널·콘텐츠 래퍼) — 화면·토글 애니메이션 불변 |
| S4 | `2b6360f` | `useSidebarPeek` · 레일 정리(공유 추가·점 삭제) · 단일 활성 섹션 · pin · 터치 |
| S5 | `d0c1b67` | 메뉴·피커·DnD 닫힘 보류 · 드롭 뒤 inside 재판정 |
| S6 | — | 문서(CLAUDE.md 규약·현재 Phase · roadmap · phasedocs 확정본) |

- **레일**: [열기·새 문제·검색·시트 가져오기·My·공유·최근 문항] — 공유 신설(IconShare 18), 최근 문항 점 4개 삭제, peek 3버튼은 title 대신 aria-label
- **peek**: 80ms 머물면 펼침 · 떠나면 180ms 유예 · 닫히는 중 재진입은 되감기 · 패널 안 다른 헤더에 150ms 머물면 섹션 전환(정지 포인터 연쇄 방지 4px 가드)
- **고정**: peek 중 사이드바 버튼 = "사이드바 고정"(pinning 단계로 전환 200ms 동안 z 80 유지)
- **보류**: 폴더 ⋯ 메뉴·아이콘 피커·이동 픽커·최근 ⋮ 메뉴·드래그 중에는 닫히지 않는다 · 메뉴 연 채 다른 섹션으로 안 바뀐다
- **터치**: 탭 즉시 열림 · 항목 선택 또는 바깥 탭으로 닫힘 · 키보드로는 열지 않는다
- **검증**: headless Chrome CDP 실제 입력 S4 36 · S5 14, 화면 불변 단계 스크린샷 바이트 대조, 변이 — W1(seq) FAIL 67ms · 메뉴 hold FAIL(빈 본문 경로) · 드롭 뒤 재판정은 Chromium에서 PASS(안전망)

**덕수 실물 검수 종결(2026-09-17, "모두 정상")**: §13-5 8항 전항 통과 — 곡선 체감 · 닫힘 꼬리 N6(수용 확정) · D17 인상 · 실데이터 계정 · 편집창 위 z · Safari · iPad · 공개 뷰어.
**후속 후보**: 드래그 중 레일 hover로 열기(spring-loaded) · 키보드 peek · My 트리 스크롤 위치 기억 · 레일 클릭 = 고정.

### Phase 67b 후속 — 실사용 개선 ✅ 구현 (2026-09-17) · **덕수 실물 검수 종결(2026-09-17, "대만족 · 잘 작동")**

덕수 실사용 요청 3건 → 타당성 검토 → 결정 11건 전항 권장안(확정본 §14).

| 스텝 | 커밋 | 내용 |
|------|------|------|
| P1 | `97e7c52` | 리듀서: 섹션 없는 레일 진입 · 레일 안 이동은 타이머 유지 · `test:peek` 47 |
| P2 | `c2fb0e7` | 섹션 헤더 hover 배경(peek·고정 펼침) · 레일 새 문제·검색·시트 툴팁 제거 — 배치 불변 |
| P3 | `cbbbb37` | 레일 전체가 peek 트리거 · peek 헤더 사이드바 버튼을 레일과 같은 자리로 |
| P4 | `421253d` | `SectionSlide` — peek 중 섹션 상하 슬라이드(펼침·접힘 동시 200ms) |
| P5 | — | 문서 |

- **트리거**: 레일 전체(열기 버튼·새 문제·검색·시트·빈 곳·푸터). 섹션 아이콘이 아닌 곳으로 열면 세 헤더 접힘. 새 문제 등은 같은 자리 클릭이 그대로 실행되고, 열기 버튼 자리 클릭은 "사이드바 고정"
- **hover 배경**: 포인터가 올라간 헤더에 폴더 행과 같은 알약
- **슬라이드**: 헤더 사이 이동 시 펼침·접힘을 동시에 200ms 슬라이드 — 고정 펼침 chevron과 peek 첫 열림은 현행 즉시
- **검증**: 기준 세 벌(하니스·홈·peek 좌표) 변경 전후 바이트 동일 · S4 44 · S5 14 · 슬라이드 9 · 로컬 빌드 통과 · 로직 검증 492 → **495건**

**덕수 실물 검수 종결(2026-09-17, "대만족 · 잘 작동")**: 트리거 체감 · 열기 버튼 자리 클릭 · hover 배경 · 슬라이드 체감 — 열림 지연 80ms 유지 확정.

---

## Phase 66b: 문답 검증 질문 카테고리(문제·풀이) · 문제 검증 질문 4종 ✅ 구현 (2026-09-15) · 전송 동작 확인 · **실물 검수·실험 진행 중**

계획서: `docs/phasedocs/Phase66b 문답 검증 질문 카테고리·문제 검증 질문 v3 착수판.md`
(질문 목록 v2 초안 web → v1 CLI → v2 web → **v3 CLI 착수판**. §11이 구현 기록)

질문 리스트를 **[문제]·[풀이] 탭**으로 나누고 문제 검증 질문 P1 성립 · P2 표기·표현 · P3 과조건·군더더기 · P4 개선 재작성을 더했다.
**서버 0 · 규칙 0 · 검증 프롬프트 0 · 61b/61d/61h 0 · 렌더 5사이트 0 · 폰 0.** 수정 5 · 커밋 S1~S5 ·
필드 `withTabs`(질문) · `askSeedKeys`(사용자 문서) · 로직 검증 446 → **448건**(`test:ask` 9).

- **씨앗 보충** — 기존 사용자에게는 열 때 P1~P4만 만든다. "이미 제안한 키"를 사용자 문서에 기록해 지운 질문이 되살아나지 않는다
- **범위는 질문 target별** — 문제 질문은 풀이까지 참고 자료로 표시하고 "지적 대상은 [문제]뿐" 안내를 항상 붙인다.
  P1은 풀이를 실제로 보내지 않는다(`withTabs false`). 타이핑 대화·풀이 질문은 66a와 바이트 동일(옛/새 조립 함수 실행 비교)
- **말풍선 `$` 문제 회피** — 씨앗 7개의 "수식은 …" 꼬리를 지웠다(시스템 프롬프트가 이미 강제한다 · T9가 `$` 0개 고정)
- **팝오버** — 탭 기억을 모델 기억과 한 키에 병합(보낼 때마다 지워지던 함정) · 문제 질문 전용 체크박스 · 대상 변경 시 탭 전환 · 안내 세 갈래

**덕수 실사용 확인(2026-09-15)**: 전송 정상. 직전 "Failed to fetch"는 dev 서버가 꺼져 있던 것(코드 무관).
**정리**: phaseSketch의 66a·66b 계획 작업물 9개 삭제 — 확정본은 phasedocs 두 파일.

**남은 것**: 덕수 준비물(기존 G1~G3 꼬리 삭제 · 문항 선정) · 실물 검수 10항 · 실험.
**이름 확정**: 폴더뷰 순차 실행 → **66c** · 검증 칩 정리 → **66d**.

---

## Phase 66a: 문답 검증 1단계 — agent 탭 질문 리스트 ✅ 구현 (2026-09-15) · 후속 2 · **실험·검수 진행 중**

계획서: `docs/phasedocs/Phase66a 문답 검증 1단계 agent 탭 질문 리스트 v6 착수판.md`
(66 v1 구상 → 브레인스토밍 → 66a v1 web → v2 CLI → v3 web → v4 CLI → v5 web → **v6 CLI = 착수판**.
 부록 B~D가 판본별 정정·보완, §0이 덕수 확정 Q1~Q6, **§13이 구현·후속·첫 실험 기록**)

agent 대화창 입력 상단의 **질문 리스트 버튼**에서 고른 검증 질문을 선택한 AI에게 보낸다.
질문 문안은 `users/{uid}/ask_questions`에 살며 앱 안에서 추가·수정·복제·삭제한다 —
**이번 판의 목적은 기능 완성이 아니라 실험이다.** 어떤 문안이 좋은 답을 내는지는 써 보기 전에는
모르므로, 문안을 고치는 왕복이 배포 없이 도는 구조를 먼저 만든다.

**서버 0 · 프롬프트(`lib/verify/prompts.ts`) 0 · 61d/61h 0 · 문항 스키마 0 · 전처리 0 ·
렌더 5사이트 0 · 폰 0.** 61b는 서버 0이고 **클라 저장 가드 1곳을 공유**한다(Q5).
신규 3 · 수정 7 · 커밋 S1~S5 + 후속 2 · ICONS 60 → **61종** · 로직 검증 439 → **446건**(`test:ask` 7) ·
규칙 테스트 블록 65 → **67**(번호 64·65).

**핵심 규약은 CLAUDE.md 「핵심 패턴」의 Phase 66a 절이 소유한다.**

- **D15′ 히스토리 미첨부** — 문답만 `discussionHistory: []`. 클라(`hasAnyFig`·`images.history`) ·
  서버(`validate`·`buildUserPrompt`) · 모델(시스템 규칙 10) 세 층이 빈 배열을 정상 처리한다.
  66b 배치가 쓸 갈래를 1단계에서 미리 만드는 셈이다
- **D20 트리거 낱말 회피** — 씨앗 G2 원문의 "검산"이 `CODE_EXEC_TRIGGER_RE`에 걸려
  *묻는* 질문이 *요청*으로 도착했다. "맞는지 확인하는 절차"로 교체(덕수 승인) · `triggerWarnings` 경고 · T7 회귀
- **D12″ 저장 가드** — `ensureSavedForAI`가 ref 4종만 읽는다. 61b에 있던 잠복 버그(자동 저장 중이면
  가드가 조용히 통과) 동반 수정
- **D14 실험 전용 `ai_models` 문서** — 800자·"단계 나열 금지"는 시스템 프롬프트에 있어 질문 본문으로는
  못 이긴다. 콘솔 문서 하나로 `appendPrompt`를 덮는다(코드 0)

- **후속(덕수 요청)**: 문답 전송의 지적 대상은 **문제 + 기본 풀이 탭**, 'AI 풀이' 등 추가 탭은 **참고 자료**
  표시로 함께 보낸다(`5aeae5f` — 풀이 탭 맨 앞 · 맨 앞 범위 안내 · 탭은 id로 가른다). 중간 판(`7a50cdd`, 추가 탭 제외)은 대체됐다
- **첫 실험의 발견 셋(§13-3)**: ① 콘솔에서 `maxTokens`를 string으로 저장하면 조용히 1024가 된다
  ② Opus 5는 사고가 기본으로 켜져 사고 토큰도 한도에서 깎이므로 한도가 모자라면 **본문이 0자**다
  ③ 대화 기록에서 `$...$`가 `…`로 보이고 **백틱으로 감싸도 `$\displaystyle ...$`로 보인다**(렌더러가 인라인 코드를
  보호하지 않는다) — 안전 문안은 "달러 기호($) 한 쌍"

**남은 것**: 실험 §10 재개 · 실물 검수 11항 · 후속 **66b 구현 완료**(위 절). 옛 예약 "66b = 폴더뷰 순차 실행 · 66c = 칩 정리"는
**66c·66d**로 밀렸다. 배치 시간 추정은 **문항 × 질문 × 도구 턴 최대 4회**다.
