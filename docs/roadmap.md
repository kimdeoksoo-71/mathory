...
## 정렬 기능 개선 ✅ (2026-09-04 · 배포 대기)

덕수 메모 기반 소개선. **수정 3파일 · 신규 0 · 서버 0 · 스키마 0.**

- **리스트 보기(ListView) 기본 정렬 = 제목 오름차순** — "수정하면 맨 위로 튀는" 수정일
  내림차순 자동 정렬을 없앴다. 헤더 클릭 수동 정렬은 그대로.
- 대신 **최근 저장 신호를 자리 이동 없이** 낸다: 24시간 이내 수정 문항은 수정일 칸에
  저장 아이콘(`IconSave` 구름+체크, `--mathory-red-dark` — 상태 표시기 3:1 규약) +
  상대 시각(60분 미만 "N분 전" / 24시간 미만 "N시간 전" / 1분 미만 "방금 전").
  칸 폭 72 → 86(헤더·행 동시).
- **카드 보기는 현행 유지**(수정일 내림차순 기본 + SortControls).
- **일괄 검증 다이얼로그는 제목 오름차순으로 자체 고정** — FolderView의 보기 모드·정렬
  상태에 따라 표시·실행 순서가 달라지지 않는다.
- 제목 비교에 `numeric: true` 추가("문제2" < "문제10") — 카드 보기의 이름 정렬도 일관.
