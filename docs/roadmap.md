...
## Phase 59: 구조 보기(structure at a glance) · '경우' 블록 🔨 구현 완료 / 검증 대기

착수 문서: `docs/phasedocs/Phase59 구조 보기·경우 블록.md` (v1 web → v2 CLI → v3 web → v4 CLI 착수본)

**디자인 목표와의 대응** — "다 읽기 전에는 구조가 안 보인다"는 긴 풀이의 문제를 두 축으로 푼다:
**P1~P2 구조 보기**(제목 + 핵심문장 + 경우 제목행만 남긴 뼈대) / **P3~P4 경우 블록**(케이스 분기를 rail·dot·자동 번호로 시각화).
Phase 58이 만든 두 자산(제목 블록 위계, `**` key 마커)이 여기서 회수된다 — 구조 보기의 스켈레톤이 정확히 그 둘로 구성된다.
...
| 구조 보기 | 열람 2뷰 전용, 기본 full, 비영속. 섹션·경우 개별 여닫이 + 스크롤 앵커링 |
| 레거시 | Phase 54 `**Case n.**` 표기는 렌더 무변화. 구조 보기는 **행 단위 스캔**으로 항목 승격 |
| 규모 | Firestore 규칙 0 · 서버 0 · 마이그레이션 0 (순수 클라이언트) |

**핵심 교훈 1 — 같은 기능이 이미 절반 구현돼 있었다**: v1은 '경우' 블록을 신설로 규정했으나, Phase 54가 이미 텍스트 규약(`**Case 1.**` / `- **Case 1a.**` + `.marker-case-sub` 불릿 숨김)을 파이프라인에 넣어 두었다. 더 중요한 것은 **그 라벨의 `**`가 Phase 58의 key 마커와 같은 문자열**이라는 점 — 케이스 분석을 쓴 기존 풀이는 이미 `has-key` 발동 상태였고, 순진하게 발췌하면 구조 보기가 "Case 1. / Case 2."로 도배된다. 신설 전에 **같은 개념의 선행 구현부터 grep**할 것.
