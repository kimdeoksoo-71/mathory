...
## Phase 65: 편집창 줄바꿈 끄기(VS Code식) · 블록 내 가로 스크롤 ✅ (구현·검수 완료 2026-09-08 · **push·배포 대기**)
...
- **검수 보완 — 좌측 행번호 거터(2회 왕복, 1차 전량 원복)**: 폭 2/3·구분선 강화는 되돌렸다
  ...
  ⚠ **"행번호 열이 움직인다"의 진범은 조상 `.content-frame`(overflowX:auto)의 가로 패닝**이었다 —
  CM sticky는 정상(블록 안 이동량 0). 프레임을 `hidden`으로 닫고 넘침은 **미리보기 열 래퍼**가 받는다
  (고정폭 계약 보존). 편집창 안 가로 스크롤은 그대로(687px 중 400px 이동, 거터 이동 0 = VS Code 동작).
  ⚠ `overscroll-behavior`로 막으려던 1차 처방은 **효과 없었다** — 넘침이 0인 요소는 스크롤 대상이 아니다.
  행번호 서식: 0.92em · Mathory 레드 55% · 활성 행 dark 85% · tabular-nums
