# Phase 64 구현 계획서 — 휴대폰 열람 전용 화면(모바일 웹) (v4 — 실행판)

> 대상: CLI Claude (구현 착수) · 원시 자료: 목시 캔버스 「Mathory 모바일 열람 화면」(web, 2026-09-07, 7장) · 덕수 전제 확정(2026-09-07) · 결정 Q1~Q11 전항 확정(권장안)
> 계보: v1(web) → v2(CLI 실측 교차검토 — E-1~E-9 · S-1~S-11) → v3(web 재검증 — F-1~F-5 · T-1~T-6 · §8-0) → **v4(CLI 착수판, 2026-09-09 — v3 전항 실측 재확인(오류 0) + HEAD 재앵커링 G-1~G-5) = 실행판**(§12가 구현 기록 · §12-1 검수 1차 · §12-2 검수 종결·Stage 5). 계획을 이기는 실측 확정은 §12 R1~R5.
...
### 12-2. 덕수 검수 종결 (2026-09-12) — Stage 5 수행

덕수 판정: **"검수 완료, 모두 정상"** — §12-1 미판정 잔여(안드로이드 ①~⑳ 폴백 · 카톡 인앱 Google 로그인(Q10) ·
iOS 키보드 가림(D9 ④) · 앱 댓글 CodeMirror IME · 딥링크 폰 동작 · safe-area 하단(T-6))까지 전항 정상으로 닫힘.
§12 보류 4건은 그대로 후속(홈 상단 검색 · 공유자 닉네임 · D15 (a) 코드 0 종결 · 시트 닫힘 180ms 내부 상태 — 마지막은 구현 사실이라 보류가 아니다).

Stage 5(2026-09-12, CLI): 이 문서를 `docs/phaseSketch/phase64-mobile-web-plan-v4-착수판.md`에서 **`docs/phasedocs/`로 이관**(실행판) ·
CLAUDE.md §11 9항 반영(핵심 파일 구조 · 폰 셸 규약 절 · 우측 드로어 단서 · 작업 규칙 9) · roadmap Phase 64 절 · `MiniShell.tsx:13` U8 주석 갱신.
로직 검증 실측 재확인: `test:device` 10 · `test:deeplink` 4 통과(합계 **387건**).

⚠ 후속 묶음 **M8(모바일 웹 디자인·기능 개선, `docs/phaseSketch/M8-mobileWeb-plan-v1.md`)** 이 이 Phase 위에 선다 —
D10의 `[풀이 보기]`·`[문제 보기]` 시트 · D16의 시트 안 `CommentPanel` 드로어 크롬 · §7-4 "폰은 MiniShell 사양" · D3 "가로 레이아웃 없음"이 M8의 개정 대상이다. 이 넷은 M8 실행판이 이긴다.
