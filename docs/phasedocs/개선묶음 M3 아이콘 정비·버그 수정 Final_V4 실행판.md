...
| 댓글·Agent (검수 6~11차, 2026-09-05) | 편집창 Row1: 댓글 아이콘 **17**(버전 기록과 동일)·버튼 숫자 **13/500**(스테퍼와 두께 통일) · agent 토글 텍스트 **"AI" → "Agent"**(편집창·열람뷰·폴더뷰·리스트 4곳) — 편집창만 **A 15/500 + gent 12/400** 2단 조판, 나머지는 기존 크기 · 댓글 드로어 1행 제목에 `IconComment 16`, agent 드로어 제목 **'Agent'**. ⚠ CommentPanel 작성자 옆 "AI" 신원 태그는 의도적 제외(토글이 아니라 "AI가 쓴 메시지" 표시) · ⚠ 열람뷰 버튼(아이콘 13·숫자 400)은 편집창과 **비대칭인 채 현행** — 덕수가 편집창만 지정했다. 맞추려면 별도 지시로 |
