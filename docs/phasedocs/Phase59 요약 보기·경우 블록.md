...
풀이의 그림은 핵심 아이디어를 나르는 경우가 많아 요약에서 통째로 빠지면 뜻이 통하지 않는다. 그렇다고 전부 남기면 "한눈에"가 무너진다 → **작성자가 고른 그림만** 남긴다.
...
- 편집 UI: 그림 블록 옵션 줄에 **"요약에 표시"** 토글(액자 · 컬러 유지 옆). 명칭에 '강조'를 쓰지 않은 이유는 이 프로젝트에서 강조가 이미 두 축(강조문 블록 · 핵심문장 `**`)을 뜻하기 때문이다.
