# Phase 59 — 풀이 요약 보기 · '경우(Case)' 블록 **✅ 완료**

> ⚠ **레이아웃 절(§4)과 §11-1~11-7의 좌표는 Phase 59a로 대체됐다.**
> Phase 59는 rail 때문에 본문에 2단 들여쓰기(본문 3em / 하위 6em / 그 안 6·9em)를 세웠는데,
> Phase 59a에서 rail·dot·chevron을 본문 **바깥 거터**로 내보내고 그 체계를 전면 철거했다.
> 지금 유효한 좌표는 `docs/phasedocs/Phase59a Case 레이아웃·강조 체계 정비.md` §1이다.
> 이 문서에서 계속 유효한 것: 경우 블록의 **데이터 모델**(제목행/본문·자동 번호·이어짓기),
> rail의 **기하 원리**(위쪽 브리지·불투명·종단), 요약 보기의 골격.
> 무효가 된 것: 들여쓰기 좌표 전부 · `.case-gap-body` · `**` 발췌(§11-9의 절반) · `.has-key` 톤 조건.
>
> (§11-10·§11-11 참조는 실재하지 않는 절을 가리키고 있었다 — 내용이 §11-5·11-7에 흡수되고
> 번호만 남은 것이라 §11-7로 정정했다. Phase 59a R6.)
...
## 0-0. 완료 상태 (2026-08-17) — 최종 사양

> 이 문서는 v4 착수본 위에 구현 중·실사용 후 개정(§11-1~11-11)이 누적된 형태다.
> **아래 표와 §11이 최종 사양이고, §1~§10은 그렇게 결정된 이유를 담은 근거다.**
> 착수본과 어긋나는 곳은 언제나 §11이 이긴다. 커밋 15개(`3ceec29` … `0b3649d`).

| 축 | 최종 사양 |
|---|---|
| 블록 타입 | `case`·`subcase` 2종 additive. 첫 줄 = 제목행, 둘째 줄부터 본문 |
| 번호 | `C1.` · `C1a.`(대시 없음). **raw_text에 넣지 않고 렌더 시 산출** — 삽입·삭제·이동에 강하다. 섹션(heading)에서 리셋 |
| 이어짓기 | 첫 줄이 빈 case/subcase = 직전 경우의 연속. 번호·dot 없음, rail만 이어짐 — 한 경우 안에 이미지를 끼우는 방법 |
| 제목행 여백 | 렌더 시 제목행 뒤 빈 줄을 보장한다 — 없으면 둘째 줄의 `$$…$$`·리스트가 제목행 문단에 흡수된다 |
| rail | 첫 dot ~ 마지막 dot **하나의 선**. 사이에 낀 블록은 `.case-gap`으로 관통, 마지막 dot에서 종단. 조각을 **위쪽으로** 이어 붙이고 색은 **불투명 헤어라인**(화면 1px / 인쇄 0.15mm) |
| dot | 상태 표시기 — 펼침 = 채움 / 접힘 = 배경색 속 + 테두리(`--case-ring` 0.07em, 하위 0.055em). 크기 0.52em / 하위 0.3em. 색 `--case-dot`(= `--mathory-red-dark`, 인쇄 `#000`) |
| 좌측 기준 | **rail·dot 1em / 경우 본문·제목행 3em / 한 단 더(수식·리스트·① 밭·발췌) 6em** (인쇄 0.7 · 2 · 4em). 개재 블록도 같은 기준(미디어 제외). ⚠ 들여쓰기는 `padding-left`로 — `margin-left`는 rail 박스를 통째로 민다 |
| 요약 보기 | 열람 2뷰 전용, 비영속. **기본값 앱 outline / 공개 full**. 뼈대가 없으면 훅이 full로 강제 해제(빈 화면 방지). 접으면 제목·`**` 발췌·경우 제목행만 |
| 요약에 넣기 | `Block.showInSummary` — 스켈레톤 원칙의 **유일한 예외**. 블록 종류 무관(그림·표를 담은 텍스트·글상자). 표는 자기 블록에 두고 켠다. **제목 블록은 제외**(항상 들어가므로 스위치가 무의미 — 2026-08-18) |
| 토글 UI | 앱의 on/off는 전부 공용 `components/ui/ToggleSwitch`(트랙 22×13). 블록 상단바 '요약에 넣기'(휴지통 왼쪽, 비활성 블록에서도 켜져 있으면 노출) · 열람뷰 '요약' · 댓글 패널 '보이기/쓰기 허용' |
| 위계 | 요약 보기의 제목 좌단은 **본문과 같은 0em**, chevron만 왼쪽 여백으로 뺀다(§11-7). 경우 줄 chevron은 dot 오른쪽 절대배치 |
| 레거시 | Phase 54 `**Case n.**` 표기는 렌더 무변화. 요약 보기는 **행 단위 스캔**으로 항목 승격 |
| 규모 | **Firestore 규칙 0 · 서버 0 · 마이그레이션 0** (순수 클라이언트) |

**신설 파일**: `lib/caseBlock.ts` · `lib/solutionOutline.ts` · `hooks/useOutlineState.ts` · `components/problem/TabBody.tsx` · `components/problem/OutlineSections.tsx` · `components/ui/OutlineToggle.tsx` · `components/ui/ToggleSwitch.tsx`(CommentPanel에서 승격) · `tests/caseBlock.test.mjs`

**검증**: `npm run test:case` 27개(번호·이어짓기·레거시 스캔·요약 항목) · `npm run test:export` 13개 · headless Chrome 실측(레일 연속성·좌측 좌표·dot 링 가시성·명암비).

**남은 눈 확인**: 인쇄 PDF에서 rail·dot 농도 / 하위경우 dot↔제목행 간격(5em, 조정 카드는 §6.4).
...
| D5 | 기본 상태 — **앱 열람뷰 `outline` / 공개 뷰어 `full`**(2026-08-17 개정. 최초 설계는 양쪽 full이었다). 뼈대가 없으면 훅이 full로 강제 해제한다 |
...
| | 가로 | 세로 |
|---|---|---|
| 제목 줄 | chevron을 흐름 밖으로(§11-7) → **제목 좌단 0em = 본문과 같은 선** | `align-items: baseline` |
...
### 11-6. 좌측 기준 정리 (덕수, 2026-08-17)

경우 구역 안에서 x좌표를 세 자리로 못박았다 — **rail 1em / 경우 본문 3em / 한 단 더 6em**(인쇄 0.7em / 2em / 4em).
...
**⚠ 들여쓰기는 `padding-left`로 줄 것 — `margin-left`는 금지다.** `.case-gap`·`.outline-keys`는 자기 박스에 rail을 그리는데, margin을 주면 박스가 통째로 밀려 **rail이 1em이 아니라 4em에 그려진다**(그 구간만 레일이 끊기고 엉뚱한 자리에 세로선이 하나 더 생긴다). 실제로 한 번 밟았다.
...
또한 토글은 **라벨 '요약' + 공용 스위치**로 바꿨다(11-10). 방향형 문구('~로')를 잠깐 썼으나, 스위치는 켜짐/꺼짐이 곧 상태라 라벨이 상태 이름 하나면 충분하다.

### 11-8. 앱 열람뷰의 기본값을 요약 보기로 (덕수, 2026-08-17)

문항을 열면 풀이가 **요약 보기로 먼저** 뜬다. 필요한 곳만 펼쳐 보는 것이 이 기능의 본래 동선이고, 기본값이 full이면 존재를 알고도 매번 눌러야 한다.

- 공개 뷰어는 **full 유지** — 방문자는 곧장 읽는 것이 자연스럽고, 이미 공개된 링크의 첫인상을 바꾸지 않는다.
- ⚠ **빈 화면 가드 필수**: 제목·핵심문장·경우가 하나도 없는 풀이를 요약 보기로 열면 아무것도 남지 않는다. `useOutlineState`가 `available`이 false면 모드를 full로 강제 해제한다(토글도 disabled).
- 실측: 요약거리 있는 탭 → `aria-pressed=true`·스켈레톤 표시·본문 숨김 / 없는 탭 → 토글 disabled·본문 전체 노출.

### 11-9. 요약 보기에 그림 남기기 (덕수, 2026-08-17)

풀이의 그림·표는 핵심 아이디어를 나르는 경우가 많아 요약에서 통째로 빠지면 뜻이 통하지 않는다. 그렇다고 전부 남기면 "한눈에"가 무너진다 → **작성자가 고른 블록만** 남긴다. 블록 종류는 가리지 않는다(그림·표를 담은 텍스트·글상자). 표는 자기 블록에 두는 것이 자연스러운 저작 형태라 별도 추출 로직을 두지 않았다.

- 데이터: `Block.showInSummary?: boolean` (additive, true일 때만 기록 → 규칙 0 · 마이그레이션 0). `imageWidth`·`imageTreatment`·`imageGray`와 같은 계열이라 `toPersistedBlock`의 화이트리스트에 한 줄 추가로 끝난다.
- 편집 UI: **제목 블록을 뺀 모든 블록의 상단바(휴지통 왼쪽)에 "요약에 넣기" 스위치**(라벨 왼쪽·스위치 오른쪽). 댓글 패널의 '보이기'와 **같은 공용 `components/ui/ToggleSwitch`** 를 쓴다 — 원래 CommentPanel 안의 지역 컴포넌트였던 것을 이때 공용으로 옮겼다(사본 금지).

> **개정 (2026-08-18, 기타 개선 1)** — **제목 블록에는 이 스위치를 두지 않는다.** `buildOutline`은 `heading`을 만나면 섹션을 열고 즉시 `continue`하므로(`lib/solutionOutline.ts:102-105`) `showInSummary` 검사에 도달하지 않는다. 제목은 요약의 뼈대라 언제나 들어가며, 스위치는 아무 일도 하지 않으면서 오해만 줬다. 비활성 제목 블록의 '요약전용 얇은 바'도 함께 뺐다. 기존 문항에 남아 있는 `showInSummary: true`는 원래도 무시되던 값이라 그대로 둔다.
  - **요약에 넣은 블록은 비활성일 때도 스위치만 남긴다** — 상단바 전체가 아니라 스위치 하나만, 배경·구분선 없이. 켜 둔 사실이 목록에서 바로 보여야 하고, 활성 헤더처럼 보이면 안 된다.
  - OFF 트랙 색을 `--bg-active`(#E8E2D9) → `--text-placeholder`(#C8C1B6)로 낮췄다. 활성 블록 배경(#E8DFCE)과 **명암비가 1.03:1**이라 트랙이 사실상 보이지 않았다(→ 1.35:1). 공용 컴포넌트라 댓글 패널의 '보이기'도 함께 또렷해진다. 처음엔 그림 옵션 패널에 두었으나 블록마다 위치가 달라져 일관성이 없었다(덕수) → 헤더 한 자리로 통일. 명칭에 '강조'를 쓰지 않은 이유는 이 프로젝트에서 강조가 이미 두 축(강조문 블록 · 핵심문장 `**`)을 뜻하기 때문이다.
- 렌더: `OutlineItem`에 `kind: 'block'`을 추가하고 **사이트별 `renderBlock`을 그대로 호출**한다 → 앱은 treatment·크기가, 공개 뷰어는 자기 스타일이 자동 적용된다(D16 렌더러 분리 유지).
- ⚠ **스켈레톤에서 renderBlock 결과를 div로 감싸지 말 것.** 감싸면 `.case-gap`이 형제가 아니게 되어 경우 사이 그림에서 **rail이 앞뒤로 끊긴다**(F1과 같은 함정을 그대로 다시 밟았다 — 스크린샷으로 발견).
- 그림 크기는 본문과 같다(작성자가 정한 `imageWidth` 그대로). 요약 전용 축소는 두지 않았다 — 필요하면 후속.
- 실측: 표시한 그림 2개만 요약에 남고 나머지는 빠짐 / 경우 사이 그림 구간에서 rail 연속 / 본문은 숨김 유지.
...
레거시 `Case` 텍스트 → 블록 자동 변환 도구 · 펼침 애니메이션 · 편집창 미리보기 요약 보기 · FolderView 적용 · 요약 보기 상태 URL 공유 · callout B안(`>> `)과 경우 제목행 문법 통합.

~~Phase 58 P6 긴 display 수식 접기~~ — **폐기(2026-08-18)**. 본 Phase의 요약 보기가 같은 목적을 달성해 중복이다.
