...
| 요약에 넣기 | `Block.showInSummary` — 스켈레톤 원칙의 **유일한 예외**. 블록 종류 무관(그림·표를 담은 텍스트·글상자). 표는 자기 블록에 두고 켠다 |
...
- 편집 UI: **모든 블록의 상단바(휴지통 왼쪽)에 "요약에 넣기" 스위치**(라벨 왼쪽·스위치 오른쪽). 댓글 패널의 '보이기'와 **같은 공용 `components/ui/ToggleSwitch`** 를 쓴다 — 원래 CommentPanel 안의 지역 컴포넌트였던 것을 이때 공용으로 옮겼다(사본 금지).
