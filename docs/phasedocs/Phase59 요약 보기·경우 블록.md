...
- 편집 UI: **모든 블록의 상단바(휴지통 왼쪽)에 "요약에 넣기" 스위치**(라벨 왼쪽·스위치 오른쪽). 댓글 패널의 '보이기'와 **같은 공용 `components/ui/ToggleSwitch`** 를 쓴다 — 원래 CommentPanel 안의 지역 컴포넌트였던 것을 이때 공용으로 옮겼다(사본 금지). 처음엔 그림 옵션 패널에 두었으나 블록마다 위치가 달라져 일관성이 없었다(덕수) → 헤더 한 자리로 통일. 명칭에 '강조'를 쓰지 않은 이유는 이 프로젝트에서 강조가 이미 두 축(강조문 블록 · 핵심문장 `**`)을 뜻하기 때문이다.
