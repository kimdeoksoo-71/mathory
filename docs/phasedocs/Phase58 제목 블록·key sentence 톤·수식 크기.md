...
>
> **검증 완료 (2026-08-16, 덕수 육안)**: 인쇄 톤 복원(T9) · 기존 문항 `**` 실태 — **볼드 용도 사용처 없음**(T12, D4' 자동 승격의 영향 범위 0) · ProblemView·FolderView 카드·공유뷰 3곳(T11·T17) · 마커 없는 문항 무변화(T2) · 원문자 내어쓰기와 선택지 baseline(T14) · 툴바 거부 흔들림과 아이콘 정렬(T10·T10'). 앞서 확정한 T1(제목 여백 실측)·T13(수식 크기)·T3~T8(톤)과 합쳐 **Phase 58 검증 종료**.
>
> **미실행**: T16(`aligned`·`gathered`·`array` 사용 비율) — Phase 58 검증 항목이 아니라 분리된 P6의 착수 판단 자료였다. **P6가 폐기(2026-08-18)되어 불필요해졌다**(§6).

> **이 문서는 자립형이다.** v1·v2·v3를 대체하며, 착수 시 **이 문서 하나만** 보면 된다. 이전 판들은 이력 보존용으로만 남긴다(§8에 판별 요약).
> **미결 없음** — 구 Q1~Q12, v3 Q1~Q2, v4 Q3까지 전부 확정됐다.

**배포 범위 = P1 · P2 · P3 · P4 · P5.** P6(긴 display 수식 접기)는 사양만 확정하고 **별도 Phase로 분리**한다(§6). — ⚠ 그 별도 Phase는 **착수 전 폐기됐다(2026-08-18)**: Phase 59 요약 보기가 같은 목적을 달성했다. §6 참조
...
| **D12'** | P6 분리 | **본 Phase 범위 밖.** T16 조사 후 별도 Phase → ⚠ **착수 전 폐기(2026-08-18, §6)** |
...
## 6. P6 사양 (별도 Phase 승계분) — **❌ 폐기(2026-08-18)**

> **폐기 사유**: 분리 후 착수 전에 Phase 59 **요약 보기**가 "긴 풀이를 한눈에 조망한다"는 같은 목적을 달성했다. 접기 장치를 하나 더 얹으면 사용자가 두 종류의 접힘(요약 보기 / 수식 접기)을 구분해야 해서 오히려 복잡해진다 — 덕수 판단.
> 아래 사양·실측은 **되살릴 일이 생겼을 때의 출발점으로만** 보존한다. T16(aligned 사용 비율 조사)도 이로써 불필요해졌다.
