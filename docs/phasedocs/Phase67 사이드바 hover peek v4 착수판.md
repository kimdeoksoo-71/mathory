...
## 14. Phase 67b 후속 — 실사용 개선 (2026-09-17) — 덕수 검수 종결("대만족")

덕수 실사용 요청(`docs/phaseSketch/Phase67-sidebar-hover-peek-plan-v4b.md`): ① 자동 펼침 영역을 레일 전체로 ② 섹션 헤더에도 hover 배경
③ 상하 펼침·접힘에도 좌우 peek과 같은 슬라이드(접힘 슬라이드 필요 여부는 의견 요청). 타당성 검토 → 결정 11건 전항 권장안 확정 → P1~P5.

### 14-1. 결정(전항 권장안)

| # | 질문 | 확정 |
|---|---|---|
| 1 | 레일 "열기" 버튼을 트리거에 넣을 때 클릭 빗나감 | peek 중 헤더의 사이드바 버튼을 레일과 같은 왼쪽 자리(중심 x 28)에. 고정한 순간 원래 배치 |
| 2 | 트리거 범위 | 레일 전체(버튼 사이 빈 곳·푸터 포함) |
| 3 | 섹션 아이콘이 아닌 곳으로 열었을 때 | 세 헤더 모두 접힘(section null) |
| 4 | 넓어진 트리거의 열림 지연 | 80ms 유지(실사용에서 원치 않는 열림이 잦으면 늘린다) |
| 5 | 새 문제·검색·시트 가져오기 네이티브 툴팁 | 레일에서 제거 · aria-label 유지 · 사이드바 버튼 툴팁 유지 |
| 6 | 헤더 hover 배경 범위 | peek·고정 펼침 모두 |
| 7 | hover 배경이 따르는 대상 | 포인터가 실제로 올라간 헤더 |
| 8 | 접힘 슬라이드 | 넣는다 — 펼침과 동시에 같은 200ms 곡선(접힘만 즉시면 위 섹션이 접힐 때 아래 헤더가 수백 px 튀고, 접힘만 짧으면 사이 헤더가 출렁인다) |
| 9 | 고정 펼침 chevron에도 슬라이드 | 이번엔 peek 전용 |
| 10 | peek 첫 열림에 상하 슬라이드 | 하지 않는다(좌우 펼침과 겹친다) |
| 11 | 헤더 hover 뒤 전환 150ms | 유지 |

### 14-2. 구현

| 스텝 | 커밋 | 내용 |
|---|---|---|
| P1 | `97e7c52` | 리듀서 — `railEnter { section \| null, dragKind, collapsed }` · idle→pending만 seq+1(레일 안 이동은 타이머 유지) · section null로 열림 · `test:peek` 47 |
| P2 | `c2fb0e7` | `SidebarSectionHeader` hover 배경(절대배치 조각) · 레일 새 문제·검색·시트 `title={false}` — 배치 불변 |
| P3 | `cbbbb37` | 패널 진입 = railEnter(null) · 아이콘 이탈 = railEnter(null) · 패널 이탈 = railLeave · peek 헤더 사이드바 버튼 왼쪽 |
| P4 | `421253d` | `components/layout/SectionSlide.tsx`(신규) — My·공유·최근을 grid 0fr↔1fr 슬라이드, peek 전용 |
| P5 | (이 커밋) | 문서 |

### 14-3. 조용히 틀어질 뻔한 것
- **헤더 hover 배경을 margin/padding으로 넓히면 마진 겹침으로 밀린다(P2)** — `margin -2/-8 + padding 2/8`로 점유 높이를 맞췄는데, My 헤더 다음 "미지정" 행의 `marginTop 4`와 세로 마진이 겹쳐 **폴더가 없는 계정에서만** 아래 행이 2px 밀렸다. 폴더가 있는 하니스에서는 첫 행에 마진이 없어 안 보였고, **비로그인 홈 스크린샷**이 잡았다. → 배치에 참여하지 않는 절대배치 배경 조각.
- **grid 칸 안에서는 첫 자식 marginTop이 바깥과 겹치지 않는다(P4 ③)** — 같은 "미지정" 행이 폴더 없는 계정에서 4px 밀릴 자리. → `marginTop: visibleFolders.length ? 4 : 0`(두 경우 모두 옛 배치 그대로). 이번에도 홈 스크린샷이 판정자.
- **grid 기본 열은 내용 최소 폭 아래로 안 줄어든다(P4 ④)** — 긴 폴더 이름(nowrap·말줄임)이 칸을 244 → 371px로 넓혀 말줄임·행 높이가 바뀌었다. 짧은 이름뿐인 홈에서는 안 보였고 **가짜 긴 폴더 이름 하니스**가 잡았다. → `gridTemplateColumns: minmax(0, 1fr)` + `minWidth 0`.
- **두 결함은 서로 다른 데이터에서만 보였다** — 폴더 없는 화면(홈)과 긴 이름 폴더 40개(하니스)를 둘 다 기준으로 둔 것이 두 번 다 살렸다.
- **레일 "열기" 버튼 클릭 빗나감(결정 1)** — 사람의 클릭(보통 200ms+)이 80ms peek보다 늦어 고정 펼침 배치 그대로면 워드마크에 떨어진다. 요청 문면에 없던 문제라 검토 단계에서 결정으로 올렸다.

### 14-4. 검증(3000 dev 하나로 · 임시 라우트 `/dev-p67`, 커밋 안 함 · 끝나고 dev 끄고 삭제)
- **기준을 바꾸기 전에** 세 벌: 하니스 고정·접힘·폭 320·토글 애니메이션 스크린샷 6장 + 기하 / 실제 `/` 5장 / peek 3상태+고정의 사이드바 요소 좌표. P2·P3·P4마다 대조해 **전부 바이트 동일**. peek 상태는 그림자가 본문 글자 위에 합성돼 픽셀이 매번 조금씩 달라(2회 비결정) **요소 좌표**로 대조했다.
- 실제 입력: S4 44/44(67b N1~N7 — 새 문제 위 133ms에 섹션 없이 열림 · 같은 자리 클릭이 새 문제 실행 · 빈 곳 · 열기 버튼 위 → peek 사이드바 버튼 중심 (28,28) → 같은 자리 클릭으로 고정 · 고정 펼침 hover 무반응 · 아이콘에서 빈 곳으로 · 섹션 없는 peek에서 헤더 hover) · S5 14/14 · 슬라이드 9/9(My→공유 공유 헤더 805→226을 13개 좌표로 약 185ms · 접힌 My 폴더는 슬라이드 끝에 언마운트 · 반대 방향 · 도중 되돌림 · peek 첫 열림 무슬라이드 · 고정 펼침 chevron 즉시) · hover 배경 켜짐/꺼짐.
- T4 판정: 레일 안 My→공유 점프 뒤 66ms에 열림(타이머 재시작이면 같은 측정법으로 약 133ms).
- 로컬 `npm run build` 통과 · `tsc -p .` 0건.
- **측정 함정**: 서버 렌더링된 인라인 스타일은 `overflow:auto`처럼 공백 없이 직렬화된다(선택자 `style*="overflow: auto"`가 빗나갔다).

### 14-5. 알고 두는 것
- 사이드바가 꽉 차면 맨 위 헤더 행이 flex로 56 → 52로 눌려 아래가 4px 올라간다 — **67b 이전부터의 배치**(고정 펼침도 같음). 슬라이드는 이 4px을 순간 이동 대신 점진으로 만든다.
- peek이 열릴 때 섹션1 항목의 `transition: all`(레일 패딩 → 펼침 패딩)이 헤더들을 12px 함께 움직인다 — 67 이전부터(N6과 같은 뿌리). 섹션 간격은 첫 프레임부터 일정.
- **덕수 실물 검수 종결(2026-09-17, "대만족 · 잘 작동")** — 레일 전체 트리거 체감 · 열기 버튼 자리 클릭 · 헤더 hover 배경 · 슬라이드 체감(접힘 동시 · 200ms) 모두 만족. 결정 4(열림 지연 80ms)는 유지로 확정.
