...
> **구현·검수 완료(2026-09-04) — §9가 구현 기록, §9-3이 검수 결과.** 61e·61e-2차와 함께 배포 대기.
...
계획대로 **신규 2 · 수정 8 · 커밋 5**(`997cb72`~`7e615b1`). 관문 1~6 전부 통과 —
로직 검증 10종 **336건**(`test:verify` 45 → **74**) · `tsc --noEmit` 전체 · `next build` ·
dev 스모크(무토큰 verify 401 / images 형식 오류 400 / 참조 51개 400 / 정상 형식 404 통과).

### 9-1. 계획에서 이탈한 것 (R-1~R-7)

| # | 이탈 | 이유 |
|---|---|---|
| **R-1** | 전송용 자리표시자 = `[그림]`이 아니라 **`⟦그림⟧`** (모델이 보는 최종형은 계획대로 `[그림 k]`) | 본문에 `[그림]`·`[그림N]`이 **자연 발생**한다(전처리의 `Fig.N → [그림N]` 변환 · "위 [그림] 참조" 류). 토론 경로는 조립된 문자열을 스캔하므로 충돌하면 번호가 밀린다. `⟦⟧`는 proofread `⟦M0⟧`의 기계 삽입 표식 관례. 렌더가 전부 치환하므로 어떤 경우에도 모델에 새지 않는다(테스트 R2가 고정) |
| **R-2** | 토론 `images`는 평평한 배열이 아니라 **필드별**(`{problem, tab, history[][], message}`) | 평평한 배열은 "등장 순서" 계약을 클라·서버가 손으로 맞춰야 한다 — D19가 막은 것과 같은 부류의 어긋날 자리. 필드별이면 서버가 자리표시자 수와 짝을 맞춰 검증할 수 있다 |
| **R-3** | `planSlots`에 **우선순위 축 분리** | 계획 §4-8-4의 *"등장 순서가 곧 D12 순서"*는 **틀렸다** — 등장은 문항→히스토리→메시지, D12는 문항→메시지→히스토리다. 그대로 구현하면 상한 초과 시 **사용자가 방금 붙인 그림이 먼저 잘린다**(D12의 존재 이유 정반대). k·첨부 순서는 등장 순서, 상한 생존만 우선순위가 정한다(테스트 S5) |
| **R-4** | `SlotStatus`는 판별 유니온이 아니라 단일 인터페이스 | 하니스가 tsconfig 없이 tsc를 돌려 strict가 꺼져 있고, 그 모드에서 `if (s.ok)` 판별 좁히기가 일어나지 않는다(실측 TS 5.9.3) |
| **R-5** | `buildImageNote(attachedKs: number[], …)` — 계획의 `(attached: number, …)`에서 확장 | 누락이 섞이면 첨부 k가 불연속이다(1, 3, …). 개수만으로는 모델이 "몇 번째 이미지 = [그림 몇]"을 알 수 없다 — 꼬리말이 k 목록을 첨부 순서대로 나열해야 잇는다 |
| **R-6** | `verifyProbe`에 images 경로를 태우지 않고 **한계를 머리말에 기록** | 프로브 말뭉치는 시트 텍스트라 image 블록이 생길 수 없다 — images는 항상 비고 바디는 61f 이전과 동일(D10). D8의 "첨부됨" 갈래는 실사용 검수 전담 |
| **R-7** | `normalizeBlocks`는 화이트리스트를 하지 않고 **운반만** 한다(계획 §4-6-2 변경) | 판정을 `planSlots` 한 곳에 두면 탈락 사유가 "허용되지 않는 그림 주소"로 정확히 남는다. normalizeBlocks에서 지우면 null 슬롯이 되어 "SVG/GeoGebra"로 오보된다 |

그 밖의 구현 세부: 자르기 후 **반토막 난 `⟦…` 잔재를 한 줄로 제거**(`⟦`는 이 표식 말고 안 쓰는
글자다) · 토론 서버는 자리표시자·슬롯 수가 어긋나면 앞에서부터 자리표시자 수만큼만 쓴다(방어) ·
버킷 env 부재 시 토론은 "서버 그림 설정 누락"으로 전량 누락 처리하고 계속(D6), 검증은 readEnv
필수라 이름을 짚는 500.

### 9-2. 실사용 검수 항목 (§5 수동표)

① 그림 문항 검증이 `skip`이 아니라 실제 판정 ② 토론에서 민·클·쳇이 그림 기반 답, 식은 "그림
확인 필요" ③ 메시지에 붙인 그림을 본다 ④ 그림 없는 문항 무회귀 ⑤ 그림뿐인 문항이 61d에서
`empty_question`으로 안 막힘.

### 9-3. 덕수 검수 (2026-09-04) — **①~⑤ 전항 정상**

**Q2 종결** — ②에서 클(Claude)이 그림 기반 답을 냈다 = `image` 블록 + `thinking:'adaptive'` +
토론 경로의 `code_execution` 도구가 실호출에서 공존한다. 열린 질문 중 남은 것은 Q5(히스토리
그림이 상한을 밀어내는 실사용 빈도)·Q7(장당 토큰 증가 실측)뿐이며 둘 다 운영 관측 항목이다.

---

*v3 착수판 — 결정 D1~D20 확정 · 구현 완료. 인용: mathory `a208e9e` · node_modules 실물 타입 · audition `8ebb218`.*
