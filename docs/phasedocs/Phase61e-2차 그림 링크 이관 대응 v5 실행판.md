...
| # | 항목 | 상태 |
|---|---|---|
| O1 | `hasImages` 죽은 필드 — 그림이 image 블록이 되면 정밀 검증이 그림을 못 본다 | **착수됨** → `docs/phaseSketch/Phase61f 정밀 검증 그림 첨부 구현 계획서 v1.md`(web v1, CLI 교차검토 대기). 실증상: 그림 문항을 검증하면 "이미지를 확인할 수 없어…"로 끝난다 |
...
**남은 것**(검수는 다음 회차로 미룸): §7-5의 눈으로 볼 항목(①②③④ — 미리보기=저장 후 렌더 · 구형 세트 무회귀 ·
두 형식 공존 행 · 중복 배지) · §7-6 운영(재가져오기·마법사 안내문) · §7-7 문서(roadmap·CLAUDE.md).
