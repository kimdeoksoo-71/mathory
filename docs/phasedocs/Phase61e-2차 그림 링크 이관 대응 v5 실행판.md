...
> **구현 완료(2026-09-03) — §10이 구현 기록.** 남은 것은 실데이터 검수(§7-5)와 운영(§7-6)뿐이다.
...
| # | 항목 | 상태 |
|---|---|---|
| O1 | `hasImages` 죽은 필드 — 그림이 image 블록이 되면 정밀 검증이 그림을 못 본다 | 61f 후보 · 우선순위 하향(GAS 패치 12·13이 검증 쪽은 해결) |
| O2 | ~~`[4점]` → `[$4$점]` (61e §5 O-A)~~ | **해결됨(2026-08-30, 61e-2차와 별개 커밋).** 보호 목록에 `/\[\d+점\]/` 추가 |
| O3 | Drive 파일명 정규형 실태 | 관문 5에서 실측. **N2′ 때문에 신·구 파일 모두 NFD일 수 있다.** NFD가 나오면 61e부터 조용히 실패하던 그림이 있었다는 뜻이라 기록할 것 |
...
**남은 것**: §7-5 실데이터 검수 · §7-6 운영(재가져오기·마법사 안내문) · §7-7 문서(roadmap·CLAUDE.md).
