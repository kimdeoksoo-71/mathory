...
## 11. 수락 기준
> [x] = 코드 완료 + tsc/next build/규칙 스모크/1단계 에디터 회귀로 검증됨. [~] = 코드 완료, **덕수 규칙 배포 후 육안 확인 대기**.
- [x] "문항 공개" 노드 하나에 스냅샷·실시간이 방식 배지로 한 리스트에 보임(publishedAt 정렬)
- [x] 오너가 방식 라디오로 공개, 각 링크 발급
- [~] 실시간: 비로그인 열람, 공개 탭 필터 적용, **편집 즉시 반영(`watchTabBlocks`)** (코드 완료, 배포 후 확인)
- [~] **비로그인 포함 누구나** 댓글 **읽기** 가능, **agent 메시지 비노출**(`commentSessionId` 포인터 + `isCommentStream` 필터) (규칙 스모크 통과, 배포 후 육안 확인)
- [x] 공개 뷰어에 **댓글 작성 UI 없음**(작성은 멤버가 에디터에서만)
- [x] `mapDoc` 수정 후 에디터 댓글/AI 배지·`countAgentSessions`·유령 카드 회귀 없음 (1단계 덕수 확인 완료)
- [x] 오너 모더레이션/AI-삭제 분기 (규칙 스모크 9·11 통과)
- [x] 공개 토글 시 `ensureCommentSession` 선제 호출로 `commentSessionId` 채워짐
- [x] `publishedAt`: 최초 공개 1회 스탬프, 일반 편집 시 불변, 재공개 시 갱신(`setProblemPublic`)
- [~] `/p/{id}` 메신저 공유 시 OG 링크 카드 노출 (코드 완료, **`public/og-default.png` 추가 + 배포 후 확인**)
- [~] 비공개 전환 후 재공유 시 OG 메타 stale 없음(`cache: 'no-store'`) (코드 완료, 배포 후 확인)
- [x] 비공개 전환 시 뷰 차단 + 안내
- [x] build 통과, 규칙 스모크 테스트 통과(11/11)

**v1 수용 확정**: 비로그인 작성자명 '익명'(users read 규칙) — v2 authorName 비정규화로 해소.
