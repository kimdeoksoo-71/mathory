...
## 9. 구현·검수 기록 (2026-09-08) — **본체 완료 · 거터 1건 미해결**

본체(줄바꿈 토글·가로 스크롤)는 계획대로 구현했고 **계획을 뒤집은 것은 없다**.
실물 검수에서 반영 2건 — D9′ 아이콘 단일화(§9-4) · 거터(§9-4′).
⚠ **거터 항목 중 "좌우 스크롤 시 거터가 밀린다" 하나는 미해결로 보류**됐다(덕수 판정:
"그리 크게 흉해 보이진 않으니 오늘은 이쯤"). 원인 미특정 · 처방 2건 실패 — §9-4′ ②가 전말이다.
...
| S9 | 미해결 기록 — 실패 처방 2건·다음 착수 절차 | 문서 3 |
...
거터가 밀린다"는 **미해결로 보류**됐다. 아래 표는 1차 시점의 기록이다.
...
### 9-4′. 검수 보완 — 거터 (2026-09-08, 3회 왕복 · 1차 전량 원복 · **1건 미해결**)
...
| ② | 좌우 스크롤 시 고정 — 근본 해결 | ⚠ **미해결(보류)** — 처방 2건 실패·원인 미특정(아래 ②) |
...
**② 좌우 스크롤 시 거터가 밀리는 현상 — ⚠ 미해결(2026-09-08 보류).**

> 덕수 실기기 판정(3차): *"좌우스크롤에 따라 거터가 좌우로 밀리는 현상은 여전한데,
> 그리 크게 흉해 보이진 않으니 오늘은 이쯤 하자."*

**세 번 처방했고 세 번 다 증상을 없애지 못했다.** 아래는 다음 사람이 같은 길을 다시 걷지 않도록
남기는 기록이다 — **확정된 사실 / 실패한 처방 / 다음에 할 일**로 나눈다.

**확정된 사실(실측)**

| 사실 | 근거 |
|---|---|
| CM 거터 sticky는 **블록 안에서 정상**이다 | 재현 프로브 3회 — `.cm-scroller`를 `scrollLeft 400`까지 밀어도 거터 이동 **0px**(가능폭 687px) |
| 조상이 가로로 스크롤되면 **거터도 함께 밀린다** | `.content-frame`을 200px 밀자 거터가 화면에서 정확히 **−200px** 이동 |
| `.content-frame`은 **상시 넘칠 수 있는 상태**였다 | 좌측 열 `minWidth:420` + 미리보기 열 고정폭 → 드로어를 열면 실측 넘침 **304px** |
| 그 경로를 닫은 뒤에도 **증상은 남았다** | 덕수 실기기 확인(3차) |

→ **아직 특정하지 못한 제3의 경로가 있다.** 위 세 줄은 참이지만 그것만으로는 증상이 설명되지 않는다.

**실패한 처방 2건 — 다시 시도하지 말 것**

1. `.scaled-editor`에 `overscrollBehaviorX: 'contain'`(S7). **효과 없음** — 그 요소는 가로 넘침이 0이라
   브라우저가 가로 스크롤 대상으로 **아예 고려하지 않아** overscroll-behavior가 적용될 기회가 없다.
   → 넘침이 없는 요소에 overscroll-behavior를 걸어 체이닝을 막으려는 발상 자체가 틀렸다.
2. `.content-frame` 가로 패닝 제거 + 미리보기 래퍼(S8). **증상 미해소.** 다만 이 변경 자체는
   *demonstrated* 경로 하나를 실제로 닫았고 부작용 없이 통과했으므로 **코드에 남겨 두었다**
   (아래 "남겨 둔 변경" 참조). 이것이 답이었다고 적지 말 것 — 답이 아니었다.

**⚠ 이번 왕복의 진짜 교훈 — 재현 프로브로 3연속 오진했다.**
매번 "실제 구조를 재현했다"고 믿고 측정했지만 세 번 다 실물과 어긋났다(1차: 프로브 블록이 전부
2자리라 열 폭 갈림을 놓침 / 2차: 블록 안만 보고 조상 패닝을 놓침 / 3차: 조상을 재현했는데도 증상이
남음 = 재현 자체가 여전히 불완전). **다음에는 재현하지 말고 실물에서 특정할 것.**

구체적 방법(다음 착수 시 첫 단계):
```js
// 편집 화면에서 콘솔에 붙이고 가로로 스와이프한다. 실제로 스크롤되는 요소가 그대로 찍힌다.
document.addEventListener('scroll', (e) => {
  const t = e.target === document ? document.scrollingElement : e.target;
  if (t && t.scrollLeft !== 0) console.log('가로 스크롤:', t.className || t.tagName, t.scrollLeft);
}, true);   // ⚠ capture 필수 — scroll 이벤트는 버블하지 않는다
```
`.cm-scroller`가 찍히면 CM sticky 쪽을(프로브와 실물이 갈리는 지점을) 파고, 그 밖의 요소가 찍히면
그 요소가 곧 범인이다. **요소를 특정하기 전에는 어떤 처방도 추측이다.**

**남겨 둔 변경(효과 미확인 · 되돌릴 후보)**
`.content-frame` `overflowX: 'hidden'` + 미리보기 열 가로 스크롤 래퍼. 편집 열이 패닝되는 경로
하나를 닫고, 미리보기의 고정폭 계약(본문 측정폭 = widthEm)도 보존한다(래퍼만 `flexShrink:1`,
안쪽 열은 `flexShrink:0`). 좁은 창에서 미리보기에 닿는 통로도 유지된다.
⚠ **증상을 고치지는 못했으므로, 진짜 원인을 찾은 뒤 이 변경이 여전히 필요한지 재평가할 것.**
⚠ 편집창 안 가로 스크롤은 **손대지 않았다** — 블록 `.cm-scroller`가 그대로 담당한다
(실측: 가능폭 687px 중 400px 이동 정상).
...
⚠ **미해결 1건** — "좌우 스크롤 시 행번호 거터가 밀린다"(§9-4′ ②). 원인 미특정으로 보류.
다음 착수 시 **재현 프로브가 아니라 실물에서 스크롤 요소를 특정하는 것**부터 할 것.
