...
| S7 | **검수 보완 2차 — S6 전량 원복 + 진범 수정**(§9-4′) | `MarkdownEditor.tsx` · `EditorView.tsx` + 문서 3 |
...
`.content-frame`(Row 3, `EditorView.tsx:3613`)은 `overflowX: 'auto'`이고, 좌측 열 `minWidth: 420` +
미리보기 열 `flexShrink: 0` 고정폭이라 **드로어를 열거나 창이 좁으면 상시 가로로 넘친다.**
블록에 가로 여지가 없는 자리에서 가로 스와이프하면 그 프레임이 밀리고 편집 열이 거터째 이동한다.

실측(실제 조상 구조를 재현한 프로브):

| 항목 | 값 |
|---|---|
| 프레임 가로 넘침 | `scrollWidth 1004` − `clientWidth 700` = **304px** |
| 프레임을 200px 밀었을 때 **거터 화면 이동** | **−200px** (블록째 밀린다) |
| 같은 시점 블록 안 거터 sticky | `scrollLeft 400`에서 이동량 **0** (정상) |

처방은 **편집 패널을 가로 막다른 길로** 만드는 두 줄이다(`.scaled-editor`):
`overflowX: 'hidden'`(자신이 밀리지 않게) + `overscrollBehaviorX: 'contain'`(여기서 시작된 가로
제스처가 프레임으로 전파되지 않게). **하나만 두면 다른 경로로 샌다.**
⚠ **`.content-frame`의 `overflowX: 'auto'`는 닫지 않았다** — 좁은 창에서 열에 닿는 유일한
통로다(Phase 62 T8″). 대가: 프레임이 넘치는 좁은 창에서 포인터가 편집 패널 위에 있는 동안에는
프레임을 가로로 밀 수 없다(미리보기 열·프레임 여백에서는 여전히 된다).
⚠ **실기기 확인은 남아 있다** — `overscroll-behavior`는 *사용자 제스처의 체이닝*만 막으므로
프로그램적 `scrollLeft`로는 검증되지 않는다. 합성 프로브로 판정하지 말 것(Phase 63 G1의 반대 방향).
