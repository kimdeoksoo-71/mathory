...
## 9. 구현·검수 기록 (2026-09-08) — **덕수 검수 종결**

계획대로 구현했고 **계획을 뒤집은 것은 없다**. 실측 검증 뒤 덕수 실물 검수를 받아 **전항 정상**으로
종결했다. 반영 2건 — D9′ 아이콘 단일화(§9-4) · 거터 3건(§9-4′). 남은 것은 push·배포뿐이다.
...
| S8 | **검수 보완 3차 — 프레임 패닝 제거(진범 확정) + 행번호 서식**(§9-4′) | `EditorView.tsx` · `MarkdownEditor.tsx` + 문서 3 |
...
### 9-3. 덕수 실물 검수 — **종결(2026-09-08, "모두 정상")**
...
### 9-4′. 검수 보완 — 거터 (2026-09-08, 2회 왕복 · **1차는 전량 원복**)
...
| ② | 좌우 스크롤 시 고정 — **근본 해결** | **진범은 CM sticky가 아니라 조상 `.content-frame`이었다**(아래) |
...
**② 진범 — CM sticky는 무죄였고 조상 프레임이 편집 열을 통째로 밀고 있었다.**
`.content-frame`(Row 3)은 `overflowX: 'auto'`였고, 좌측 열 `minWidth: 420` + 미리보기 열
`flexShrink: 0` 고정폭이라 **드로어를 열거나 창이 좁으면 상시 가로로 넘친다.** 그 상태에서 가로
스와이프를 하면 프레임이 밀리고 **편집 열이 통째로 패닝**되며 거터도 함께 이동한다.
(실측: 프레임 넘침 304px, 200px 밀자 거터가 화면에서 정확히 −200px 이동. 같은 시점 블록 안
거터는 `scrollLeft 400`에도 이동량 0.)

⚠ **처방을 두 번 틀렸다 — 실패 기록을 남긴다.**
1차: `.scaled-editor`에 `overflowX:'hidden'` + `overscrollBehaviorX:'contain'`.
**효과 없었다.** 그 요소는 가로 넘침이 0이라 브라우저가 **가로 스크롤 대상으로 아예 고려하지 않고**,
따라서 overscroll-behavior가 적용될 기회가 없다. 제스처는 곧장 프레임으로 간다.
→ **넘침이 없는 요소에 overscroll-behavior를 걸어 체이닝을 막으려 하지 말 것.**

**최종 처방(2차)**: 넘침을 받는 주체를 프레임에서 **미리보기 쪽으로 옮긴다.**
- `.content-frame` → `overflowX: 'hidden'` (편집 열은 어떤 경우에도 패닝되지 않는다)
- 미리보기 열을 `overflowX:'auto'` 래퍼로 감싼다 — 래퍼만 `flexShrink:1`이고 **안쪽 열은
  `flexShrink:0`이라 고정폭 계약(본문 측정폭 = widthEm)이 그대로 보존**된다. 좁은 창에서
  미리보기에 닿는 통로도 유지된다(열 자체를 shrink시키는 안은 그 계약을 깨서 기각).

검증(실제 조상 구조 재현):

| 항목 | 값 |
|---|---|
| **편집창 안 가로 스크롤** | 스크롤 가능폭 **687px** · 400px 이동 성공 → **그대로 살아 있다** |
| 그동안 거터 이동 | **0px** (= VS Code 동작) |
| 프레임 패닝 | `overflowX: hidden` · 넘침 0 · 거터 이동 **0px** |
| 미리보기 래퍼 | 넘침 304px를 흡수 · 스크롤 가능 |
| 블록 간 열 폭 | 1자리·2자리 블록 **둘 다 27.59px** (어긋남 0) |

⚠ **편집창의 가로 스크롤을 없앤 것이 아니다** — 없앤 것은 "편집 열이 통째로 옆으로 밀리는 패닝"이다.
줄바꿈을 끈 블록의 `.cm-scroller`는 그대로 스크롤되고 거터만 제자리에 남는다.
...
**덕수 검수 종결(2026-09-08, "모두 정상") — 반영 1건(D9′). 남은 것은 push·배포뿐이다.**
