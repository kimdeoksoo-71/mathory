...
| S5 | 검수 종결 기록 | 문서 3 |
| S6 | **검수 보완** — 거터 폭 2/3 · 구분선 강화(§9-4′) | `MarkdownEditor.tsx` + 문서 3 |
...
### 9-4′. 검수 보완 1건 — 거터 3건 (2026-09-08, 덕수)

> *"1. 폭을 지금의 3분의 2로 줄여 편집 영역의 공간을 늘이자. 2. 좌우 스크롤할 때 왼쪽의 행번호 열은
> 움직이지 않는게 좋겠다. 3. 열과 편집창 사이의 세로 구분선을 지금보다 조금만 더 진하게 해서 … 텍스트가
> 밀려들어갔음이 직관적으로 단번에 인식되게끔."*

| # | 요청 | 결과 |
|---|---|---|
| ① | 폭 2/3 | **49 → 33.4px (68%)**. 목표 32.7px 대비 +0.7 |
| ② | 좌우 스크롤 시 고정 | **이미 만족하고 있었다** — 조치 0. `scrollLeft 400`에서 거터·번호 모두 이동량 0(§9-2에서도 확인) |
| ③ | 구분선 강화 | `--border-subtle` → **`--block-hairline`**. 대비 **1.06 → 1.66:1**(활성 카드 1.50) |

**① 폭을 줄인 방법 — `minWidth`가 아니었다.** 옛 `minWidth: 1.8em`(27px)은 *제약*이 아니라 이미
2자리 번호가 **꽉 채우고 있던 값**이었다(실측: 15px에서 숫자 한 자 ≈ 9.2px → 2자리 18.4px + base padding
`0 3px 0 5px` = 8px → 26.4px). 그래서 `minWidth`만 낮춰도 폭이 줄지 않는다. 실제 레버는 셋이다:

| 레버 | 옛 | 새 | 효과 |
|---|---|---|---|
| `.cm-gutters` `fontSize` | 1em(15px) | **0.82em**(12.3px) | 번호·lint 열의 모든 em이 함께 줄어든다 |
| `.cm-lineNumbers .cm-gutterElement` `padding` | `0 3px 0 5px`(8px) | **`0 2px 0 3px`**(5px) | 번호 열 27 → **20.1px** |
| `.cm-gutter-lint` `width` | 1.4em(21px) | **1em**(12.3px) | lint 열 21 → **12.3px** |

⚠ **lint 열이 가장 큰 낭비였다** — 마커를 10px `●`로 오버라이드해 놓고(`.cm-lint-marker-*::after`)
base theme의 1.4em 폭은 그대로 둬서, 점 하나에 21px을 쓰고 있었다. `.cm-lint-marker`(0.8em)와
gutterElement padding(0.1em)도 함께 줄였고, 스크린샷으로 점이 잘리지 않음을 확인했다.
⚠ **3자리 자동 확장은 유지된다** — CM의 spacer가 최대 줄 번호로 폭을 잡으므로 `minWidth`와 무관하다
(실측: 105줄 문서에서 40.9px로 자동 확장, 잘림 없음).

**② 조치 0.** CM은 `unfixGutters` facet이 없으면 `.cm-gutters`를 `position: sticky`로 만들고(dist 11137),
줄바꿈을 끄면 스크롤 컨테이너가 `.cm-scroller`가 되므로 그 안에서 고정된다. 계획 단계(§9-2)에서 이미
측정했고 이번에도 `moved: false`였다. **요청이 곧 현행 동작이라 코드를 건드리지 않았다.**

**③ 현행 선은 보이지 않는 선이었다.** `--border-subtle`(= `--border-light` #E8E4DF)은 거터 배경과
대비가 **1.06:1** — 배경과 사실상 같은 색이다. 줄바꿈을 끄면 본문이 이 선 밑으로 들어가 사라지는데
경계가 안 보이면 "어디서 잘렸는지"를 알 수 없다. 블록 사이 구분선과 같은 `--block-hairline`(#C2B7A2)로
올렸다. 계산값(실배경 2종 기준):

| 후보 | 비활성 거터 #F0EAE0 | 활성 거터 #E8DFCE |
|---|---|---|
| `--border-subtle` #E8E4DF (옛) | 1.06 | 1.05 |
| `--border-content` #D2C8B8 | 1.38 | 1.25 |
| **`--block-hairline` #C2B7A2 (채택)** | **1.66** | **1.50** |
| `--border-content-active` #B89B78 | 2.19 | 1.99 |

⚠ **알고 둔 부수 효과**: 거터가 16px 좁아진 만큼 **본문 텍스트의 시작점이 왼쪽으로 이동**한다.
`.cm-content`의 `padding-left: 16px`(Phase 45a 좌측 기준선)은 **건드리지 않았다** — 바·하단툴바·미디어
블록·교정 박스가 맞추는 것은 그 패딩 값이고, 거터는 그 바깥에서 본문 전체를 미는 별개 축이다.
