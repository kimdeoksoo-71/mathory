...
## 9. 구현·검수 기록 (2026-09-08~09) — **전항 해결**

본체(줄바꿈 토글·가로 스크롤)는 계획대로 구현했고 **계획을 뒤집은 것은 없다**.
실물 검수에서 반영 2건 — D9′ 아이콘 단일화(§9-4) · 거터(§9-4′).
거터 항목 중 "좌우 스크롤 시 거터가 밀린다"는 2026-09-08에 미해결로 보류됐다가 **2026-09-09 해결**됐다 —
정체는 스크롤이 아니라 **끝의 튕김(러버밴드)** 이었다(§9-4′ ②가 전말).
...
| S2 | 배선 — 상태·localStorage·⌥Z·토글 버튼·두 경로 가로 노출 | `EditorView.tsx` · `UnifiedToolbar.tsx` · `FindReplacePanel.tsx` · 아이콘 |
| S3 | 문서 — CLAUDE.md 규약 2건 · roadmap · 실행판 이관 | `CLAUDE.md` · `docs/roadmap.md` · 이 문서 |
| — | 컨택트시트 재생성 | `docs/icons-contact-sheet.html` |
| S4 | **검수 반영** — 아이콘 ↵ 단일화 · 켬일 때 박스(D9′) | `UnifiedToolbar.tsx` · `gen-phosphor-paths.mjs` + 생성물·문서 |
| S5 | 검수 종결 기록 | 문서 3 |
| S6 | 검수 보완 1차 — 거터 폭 2/3 · 구분선 강화 | `MarkdownEditor.tsx` + 문서 3 |
| S7 | 검수 보완 2차 — S6 전량 원복 + 진범 오진(overscroll) | `MarkdownEditor.tsx` · `EditorView.tsx` + 문서 3 |
| S8 | 검수 보완 3차 — 프레임 패닝 제거(**증상 미해소**) + 행번호 서식(§9-4′) | `EditorView.tsx` · `MarkdownEditor.tsx` + 문서 3 |
| S9 | (당시) 미해결 기록 — 실패 처방 2건·다음 착수 절차 | 문서 3 |
| S10 | 가로 트립와이어 `data-nohscroll` — 조상 패닝 경로 봉인(별개 실재 경로) | `EditorView.tsx` |
| **S11** | **끔 모드 거터 sticky → fixed — 튕김의 진짜 처방** | `MarkdownEditor.tsx` |
| S12 | 해결 기록 — 문서 4·메모리 | 문서 |
...
### 9-3. 덕수 실물 검수 1차 — "모두 정상"(2026-09-08)

⚠ **이 판정은 그날 오후에 뒤집혔다** — 실사용에서 거터 3건이 나왔고(§9-4′), 그중 "좌우 스크롤 시
거터가 밀린다"는 하루 보류 뒤 **2026-09-09 해결**됐다(§9-4′ ②). 아래 표는 1차 시점의 기록이다.

| # | 항목 | 판정 |
|---|---|---|
| ① | **가로 스크롤바 5px 노출과 블록 높이 점프**(D6) | **정상 — 현행 유지.** 스크롤바를 숨기는 되돌림(코드 3줄)은 쓰지 않는다. 노출이 곧 발견성이고 5px 점프는 수용됐다 |
| ② | **아이콘**(D9 → D9′) | **반영 1건.** ↔가 Row 1 가로폭 아이콘과 겹쳐 보여 **↵ 단일 + 켬일 때 박스**로 확정 → §9-4 |
| ③ | **토글 직후 세로 보정 필요 여부**(D11) | **정상 — 추가 보정 불필요.** 가로 노출 1회로 충분하다. `computeBlockAwareScrollTop` 추가안은 **넣지 않는다** |

그 밖의 실물 전용 항목도 함께 통과: 한글 IME 조합 중 경계 넘기 · ⌥Z가 `Ω`를 넣지 않음 ·
드래그 선택의 가장자리 자동 가로 스크롤(§2-3 — CM이 공짜로 준다) · 활성 카드 radius 8 모서리의
스크롤바 클리핑.

⚠ **③이 닫혔다는 것은 "세로를 건드리지 않는 것이 정답이었다"는 뜻이다** — 켬↔끔에서 블록 높이가
일제히 바뀌는데도 보정이 필요 없었다. 후속에서 "토글할 때 시야가 흔들린다"는 인상이 들더라도
`scrollTop`을 읽고 쓰는 코드를 새로 넣기 전에 이 판정을 먼저 볼 것(ProblemView 디자인 개선의
"튐을 보정하지 말고 튈 일을 만들지 말 것"과 같은 자리다).
...
### 9-4′. 검수 보완 — 거터 (2026-09-08~09, 5회 왕복 · 1차 전량 원복 · **튕김 해결**)

**1차(S6)에서 넣은 "폭 2/3 + 구분선 강화"는 2차 검수에서 전량 되돌렸다.** 좁히는 것 자체가
정렬을 깨뜨렸고, 선을 진하게 하니 다른 자리가 상했다. 아래는 그 왕복의 기록이다 —
**되살리려는 사람은 ①②를 먼저 풀 것.**

| # | 요청(2차) | 결과 |
|---|---|---|
| ① | 폭 원복 | **33.4 → 49px 원복.** `minWidth 1.8em` · 패딩 · lint 열 폭 · 거터 fontSize 전부 되돌림 |
| ② | 좌우 스크롤 시 고정 — 근본 해결 | **해결(2026-09-09)** — 정체는 튕김, 처방은 fixed 거터(아래 ②) |
| ③ | 숫자 세로 정렬 | **`tabular-nums`** 채택. 가운데 정렬은 쓰지 않음(사유 아래) |
| ④ | 구분선 원복 | `--block-hairline` → **`--border-subtle`** 원복 |

**① 왜 좁히면 안 되는가 — `minWidth 1.8em`은 여유가 아니었다.**
1차에서 `minWidth`를 1.5em(18.45px)으로 내렸는데, 2자리 번호의 실제 폭은 20.09px이다
(15px에서 숫자 한 자 ≈ 9.2px → 18.4 + 패딩). 그 결과 **1자리 블록은 minWidth가, 2자리 블록은
글자 폭이 이겨** 블록마다 열 폭이 갈렸다(18.45 vs 20.09) — 덕수가 본 "블록마다 조금씩 달라
어긋나 보인다"가 이것이다. 옛 주석 "2자리까지 폭 통일, 3자리 이상부터 자연 확장"이 바로 그
정렬을 지키는 장치였고, `1.8em`(27px)은 **2자리가 꽉 채우는 값**이라 1자리 블록도 같은 폭이 된다.
⚠ 1차 계측이 이걸 놓친 이유: 프로브 블록 3개가 **전부 10줄 이상(2자리)** 이었다. 2차 프로브는
1자리 블록과 2자리 블록을 나란히 두고 **둘 다 27px**임을 확인했다.

**② "거터가 밀린다" — 해결(2026-09-09). 정체는 스크롤이 아니라 끝의 튕김이었다.**

> 덕수(2026-09-09): *"오른쪽 편집영역은 좌우스크롤에 의해 내용이 스크롤되기도 하고, 스크롤이 왼쪽 끝 오른쪽 끝까지
> 가면 튕김효과가 나오지. 그것까지 꼭 필요한 기능이고 현재 잘 작동돼. 문제는 … 그 튕김 효과가 행번호 거터에서도
> 같이 나는 것이 문제라는 거야."* — 영상 2편 첨부.

**원인.** `position: sticky` 요소는 스크롤러의 *스크롤 콘텐츠 레이어* 안에 있다. 끝에서의 튕김(러버밴드)은
컴포지터가 그 레이어를 통째로 탄성 이동시키는 효과라 sticky 거터도 함께 튕긴다. **이때 `scrollLeft`는 변하지
않는다** → scroll 이벤트도 없고, 프로그램적 `scrollLeft` 대입·CDP 합성 휠 어느 것으로도 재현되지 않는다.
영상 프레임 실측(0.1s 간격, 거터 영역 크롭): 5.8~6.3s · 6.7~7.2s · 7.7~8.2s 세 번 모두 왼쪽 끝 튕김에 "1"이
본문과 함께 오른쪽으로 밀렸다 복귀. VS Code 영상은 같은 조건에서 행번호가 제자리.

**왜 이틀간 프로브 5회가 전부 빗나갔나.** 재현 구조가 틀린 게 아니라 **증상 언어를 오독**했다 — "움직인다"를
"스크롤된다"로 읽고, 스크롤 값이 변하는 경로만 쳤다. 튕김·떨림처럼 **스크롤 값이 변하지 않는 시각 효과는
계측으로 볼 수 없다**. 다음부터 이런 증상은 재현 전에 영상을 받는다.
(단, S6~S10이 닫은 "조상 패닝"은 실제 휠 이벤트로 증명된 **별개의 실재 경로**였다 — ⑥ 참조. 처방이 헛된 건
아니었고 증상의 처방이 아니었을 뿐이다.)

**처방(S11, 끔 모드 한정)** — 거터를 스크롤 콘텐츠 레이어 밖으로 뺀다:
- `.cm-editor`(`&`)에 `transform: translate(0)` → `position: fixed` 자손의 containing block
- `.cm-gutters { position: fixed !important; top:0; bottom:0; left:0; height:auto }` —
  `!important`는 CM이 인라인으로 박는 `sticky`(dist 11152)를 이기기 위해 필수
- 흐름에서 빠진 자리: `.cm-scroller { padding-left: var(--gutter-w) }`, 값은 마운트 시 ResizeObserver가
  `.cm-gutters.offsetWidth`로 공급(2→3자리 확장 추적, `syncGutters`가 같은 노드를 떼었다 붙여 관찰 유지)
- CM 툴팁(자동완성·lint)은 `tooltips({ parent: document.body, position: 'fixed' })` + 테마 `.cm-tooltip z-index 10200`
- 커서·선택 레이어(`.cm-layer`)는 스크롤러 안에 그대로 → **본문과 함께 튕긴다**(의도)
- 켬 모드 무접촉(sticky·패딩 0·transform none — D2)

**구현 중 함정 둘(실측으로 잡음).**
① transform을 `.cm-scroller`에 두면 fixed 거터의 containing block이 스크롤러 자신이 되어 **스크롤 콘텐츠의
일부로 취급**된다 — `scrollLeft 400`에서 거터가 −400으로 본문과 같이 밀렸다. containing block은 스크롤러
**바깥**이어야 한다. ② `.cm-editor`에 transform이 생기면 거기 마운트되던 CM 툴팁이 fixed 좌표를 잃고 래퍼
`overflow:hidden`에 갇힌다 → body 마운트가 CM 공식 해법.

**헤드리스 검증**(튕김 자체는 실기기 전용 — 레이아웃 정합만):

| 항목 | 값 |
|---|---|
| 거터 `position` | `fixed` (끔) / `sticky` (켬 복귀) |
| `scrollLeft 400`에서 거터 L(스크롤러 기준) | **0** (본문 L −352) |
| 본문 padding-left | 48px / 3자리 블록 54px (`--gutter-w` 자동) |
| 둥근 활성 카드 클리핑 | 유지(스크린샷) |
| 자동완성 툴팁 | body 마운트 · 캐럿 바로 아래(top = 캐럿 bottom) · 뷰포트 안 · z 10200 |
| 켬 복귀 | sticky · 패딩 0 · transform none · room 0 |

**덕수 실기기 확인(2026-09-09)**: *"됐어. 해결!"* — 본문은 튕기고 거터는 제자리.

⚠ CM `scrollMargins` 제공자(dist 11282)는 `unfixGutters` 미설정이면 `dom.offsetWidth`를 그대로 내보내므로
캐럿 노출 마진은 fixed에서도 손댈 것이 없다. ⚠ `overscroll-behavior-x`는 `contain`이 튕김을 **살리고**
`none`은 죽인다 — 본문 튕김 유지가 요구라 `contain`.

**⑥ 별개로 실재하던 경로 — 조상 패닝(S8·S10).** `.content-frame`이 `overflow-x:auto`이던 때, 좌측 열
`minWidth:420` + 미리보기 고정폭이라 드로어를 열면 상시 넘쳤고(실측 304px), **활성 블록 헤더 바 위** 휠이
프레임을 154px 밀어 편집 열이 거터째 이동했다(실제 휠 이벤트 CDP 실측: 거터 X 1 → −153, 블록 X 0 → −154,
거터-블록 상대 1 불변). 짧은 줄 블록 위에서는 안 밀렸다 — `overflow-x:auto`인 `.cm-scroller`는 여지 0이어도
스크롤 컨테이너라 `contain`이 체이닝을 끊기 때문(어제 "넘침 0이면 무의미"는 `overflow-x:hidden`에만 맞는 말).
S8이 프레임을 `hidden`으로 닫고 넘침을 미리보기 래퍼로 옮겼으며(고정폭 계약 보존), S10이
`data-nohscroll` 트립와이어로 불변식을 코드에 고정했다. 이 경로는 튕김과 무관하지만 실재했으므로 유지한다.

**③ 가운데 정렬 대신 `tabular-nums`.** 원인은 정렬 방식이 아니라 본문 글꼴(`var(--font-ui)`)의
**비례 숫자**다 — 오른쪽 정렬이라 오른쪽 변은 맞는데 숫자 폭이 제각각이라 왼쪽 변이 들쭉날쭉했다.
고정폭 숫자를 켜면 두 변이 함께 맞는다. **가운데 정렬은 오히려 나쁘다**: 자릿수가 다르면 1의 자리가
서로 다른 세로선에 서서 블록을 오갈 때 번호가 좌우로 흔들린다(모든 코드 편집기가 오른쪽 정렬인 이유).

**③′ 행번호 서식 (3차, 덕수)**: 글자 크기 **한 단계 작게**(`0.92em` — 글꼴 조절 스텝 1px과 같은 감각,
실측 13.8px) · 색은 **Mathory 레드를 옅게**(`color-mix(--mathory-red 55%)`, 미지원 브라우저는 같은 값의
rgba) · 활성 행만 `--mathory-red-dark 85%`로 톤을 올린다(다른 번호가 레드인데 활성 행만 회색이면 튄다).
⚠ **`minWidth`를 1.8em → 2em으로 함께 올렸다** — 글꼴을 줄이면 2자리 실측폭(≈24.9px)이 옛
1.8em(24.8px)을 **넘어서** 블록마다 열 폭이 갈린다(①과 같은 함정). 글꼴을 더 줄이면 여기도 볼 것.
⚠ 하드코딩 rgba를 토큰과 따로 두지 않으려고 `color-mix`로 토큰을 직접 섞는다(Phase 62 `78a780f` 사고).

**④ 구분선을 되돌린 이유.** 1차에서 대비를 1.06 → 1.66:1로 올렸더니 **'요약에 넣기' 블록에서 선이
끊겨 보였다** — 거터 선은 CM 높이만큼만 그려지는데 그 블록은 위에 얇은 바(`showSummaryOnlyBar`)가
있어 선이 중간부터 시작한다. 선이 보이지 않을 때는 드러나지 않던 결함이 선을 살리자 드러난 것이다.
⚠ **다시 진하게 하려면 선을 끊기지 않게 만드는 일이 먼저다**(블록 래퍼가 세로선을 소유하게 하는 등).
색만 올리면 같은 문제가 재발한다.
...
중간 판본 v1은 **2026-09-08 phaseSketch 정리에서 삭제**됐다(git 추적 밖이라 복구 불가).
v1의 내용 중 살아 있어야 하는 것은 이 판이 전부 흡수했다 — **§2-2가 v1의 안전 논거를 실측으로 교체**했고
**부록 C가 v1 정정 10건·보완 8건**을 항목별로 남긴다. v1을 다시 찾을 이유는 없다.
**덕수 검수(2026-09-08): 본체 정상 · 반영 2건(D9′ 아이콘 · 거터 서식).**
**거터 튕김 해결(2026-09-09, S11)** — §9-4′ ②. 교훈: 스크롤 값이 변하지 않는 시각 효과(튕김·떨림)는
계측으로 못 본다. 증상은 재현 전에 **영상**으로 받을 것.
