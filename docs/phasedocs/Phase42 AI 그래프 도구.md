...
**권장: GeoGebra 재사용.** Phase 34 인프라(`loadGGB`, `GgbViewer`의 fullscreen/zoom)를 그대로 활용하면 요구사항 1·2·3을 신규 코드 최소로 만족한다. AI는 `.ggb` 파일 대신 **GGB 명령 배열**을 emit하고, `GgbViewer` 변형이 `evalCommand`로 주입한다.

> ⚠️ 최종 확정 필요 항목 — 11절 결정표 참조.
...
| 컴포넌트 구조 | GgbGraphView 신규 / GgbViewer 일반화(url\|commands) | 신규 후 공통화 검토 (위험 분리) |
