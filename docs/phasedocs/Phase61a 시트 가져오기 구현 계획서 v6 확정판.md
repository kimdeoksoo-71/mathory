...
### 10-6. 스텝 4 검수에서 잡힌 것 — 지운 문항을 다시 못 가져옴 (D15)

덕수 검수(2026-08-22): 폴더를 잘못 골라 가져온 10건을 **삭제한 뒤 다시 가져오려 하니
"이미 가져옴"으로 전부 막혔다.**

원인: **Mathory의 '삭제'는 영구 삭제가 아니라 휴지통 이동이다**
(`AppShell` `handleProblemAction` case `'trash'` → `moveToTrash` → `folder_id = TRASH_FOLDER_ID`).
문서가 Firestore에 그대로 남으므로 `listProblems`가 계속 돌려주고 중복 검사에 걸린다.
`deleteProblem`(영구 삭제)은 휴지통 안에서만 부르는 경로다.

**D15 — 두 가지를 고친다.**

1. **휴지통 문항은 중복이 아니다.** 중복 집합을 `existing`(살아 있는 문항)과 `trashed`로 나누고,
   휴지통에만 있는 것은 `dupe: 'trashed'`로 표시해 **기본 선택에 포함**한다.
   "휴지통에 있음" 배지로 전에 가져온 적이 있다는 사실은 알린다.
2. **중복이어도 체크를 막지 않는다(D3 개정).** 기존 D3는 "체크 불가(skip 고정)"였으나,
   조용히 막으면 이번 같은 막다른 골목이 생긴다. 이제 **기본만 해제하고 배지·툴팁으로 알린 뒤
   판단은 사용자에게 남긴다.** 체크를 막는 것은 초안 자체가 없는 오류 행뿐이다.

> ℹ️ 이 결정으로 같은 문항을 의도적으로 한 벌 더 만들 수 있다(다른 폴더에 두는 등).
>   기본이 해제이고 배지가 붙으므로 실수로 그렇게 되기는 어렵다.

### 10-7. Stack 전량 변환 결과 (초안 품질)
