# 개선묶음 M8 — 모바일 웹 디자인·기능 개선 구현 계획서 v1 (CLI 실측 = 실행판)

> 계보: 덕수 메모 `docs/phaseSketch/M8-mobileWeb-design-improving.md`(2026-09-12) → **v1 CLI**(이 문서 — 메모 4항을 Phase 64 코드와 대조하고 결정·함정·검수를 세웠다). 판정 Q1~Q4 전부 권장안(2026-09-12) → 착수 → **§9가 구현·검수 기록**(별도 검증 턴 없이 v1에서 바로 구현 — v1이 이미 CLI 실측판이고 1~3항은 삭제·한 줄 교체, 4항의 불확실성은 문서가 아니라 실기기에 있었다).
...
> ⚠ **Q1 번복(M9, 2026-09-19)** — 덕수 실사용 요청으로 **(b) 맨 위 도달 시만(`reveal-at-top`)**이 기본이다. 리듀서 기본값이 바뀌었고 맨 위 판정이 무시 창보다 앞으로 옮겨졌다(`docs/phasedocs/개선묶음 M9 … v5 실행판.md` D19′·D20). 이 문서의 (a) 서술은 M8 시점 기록이다.
...
## 9. 구현·검수 기록 (2026-09-12, CLI)

커밋 4(S1~S4) · `npx tsc --noEmit` 0오류 · 로직 검증 387 → **398건**(`test:chrome` 11 신설).
**덕수 검수 종결(2026-09-12): "검증 완료. 모두 정상"** — §6 T1~T11 전항(기기 모드 + 실기기 가로 포함).

| 커밋 | 내용 |
|---|---|
| S1 `147a95e` | `--wordmark-small`(#944728) 토큰(G1) · PhoneShell(D1)·MiniShell(Q3)을 Sidebar 사양(19/400/그림자)으로 — 옛 MiniShell 사양(600·`--mathory-red`·무그림자) 소비처 0. `Wordmark.tsx` 헤더 "사양은 두 벌(작은 19 · 큰 48)" |
| S2 `49d99bb` | PhoneReader `[풀이 보기]`·`[문제 보기]`·문제 보기 시트 삭제(D2·D3) · 탭별 scrollTop 기억(D4) · PhoneShell `forwardRef` 핸들 `getScrollTop/scrollTo`(D4′) |
| S3 `049be16` | CommentPanel `chrome: 'drawer' \| 'sheet'`(D6) · PhoneApp 래퍼 `paddingLeft 8` 폐기 + `chrome="sheet"`(D7) · PhoneReader 댓글 슬롯 패딩 제거 → `PublicProblemView`가 `PublicComments`를 12px로 감쌈 · BottomSheet 무변경(D8) |
| S4 `bafe715` | `lib/chromeAutoHide.ts`(순수 리듀서) + `tests/chromeAutoHide.test.mjs` 11 + `test:chrome` · PhoneShell `chromeAutoHide`(가로 `matchMedia` · 크롬 래퍼 `margin-top` 음수 0.2s · `ResizeObserver` 실측) · 루트 safe-area 좌우(D9) · PhoneReader만 켬 |

**계획 대비 개정(구현 중 확정):**
- **테스트 11건**(계획 8): `markProgrammaticScroll` 검증·상수 검증 추가. 테스트 표본 3건이 처음에 틀렸다 — 초기 상태 `lastY 0`에서 첫 샘플 120이 "120px 하향"으로 읽혀 즉시 hide. 리듀서가 아니라 표본 쪽 실수였고 `from(y)` 헬퍼로 고쳤다. **같은 함정이 런타임에도 있다** → PhoneShell은 기능이 켜지는 순간(가로 전환) `markProgrammaticScroll(reset, 현재 scrollTop)`으로 기준점을 잡는다. 세로에서 800px 읽다가 가로로 돌리면 첫 scroll 이벤트가 800 하향으로 읽혀 즉시 접히던 것의 차단
- `scrollTo`의 무시 창은 별도 ref가 아니라 **리듀서 상태(`ignoreUntil`) 하나**를 쓴다 — 두 창을 따로 두면 한쪽만 세워지는 경로가 생긴다
- 크롬 래퍼는 `transition`을 **활성일 때만** 건다(세로·리스트 화면은 `none`) — 세로 픽셀 0을 바이트 단위로 지키기 위함
- D9 safe-area는 루트 `paddingLeft/Right` + `boxSizing:border-box`. BottomSheet(absolute inset:0)는 패딩 상자 기준이라 시트는 여전히 전폭 — 함정 9 그대로, 시트 내용의 좌우 인셋은 후속

**변경 파일**: 수정 8(`globals.css` · `Sidebar` · `MiniShell` · `Wordmark` · `PhoneShell` · `PhoneReader` · `PhoneApp` · `CommentPanel` · `PublicProblemView` · `package.json`) · 신설 2(`lib/chromeAutoHide.ts` · `tests/chromeAutoHide.test.mjs`). 서버 0 · 규칙 0 · 스키마 0 · 데스크톱 픽셀 0(CommentPanel drawer 갈래는 diff상 코드 이동뿐).

**Stage 5(같은 날)**: 이 문서 phasedocs 이관 · CLAUDE.md 폰 셸 절 개정(§8-2) · roadmap M8 절.
