...
## 8. ~~좌우 스크롤 시 편집창 행번호 거터가 밀린다~~ → **해결 (2026-09-09, Phase 65 S11)**

정체는 스크롤이 아니라 **끝에 닿았을 때의 튕김(러버밴드)** 이었다 — 덕수가 영상으로 짚었다. sticky 거터는
스크롤러의 스크롤 콘텐츠 레이어 안에 있어 컴포지터의 탄성 이동에 딸려 간다. `scrollLeft`가 변하지 않으므로
scroll 이벤트도 없고 프로그램적 스크롤·합성 휠로는 재현되지 않는다(프로브 5회 전부 빗나감).
처방: 끔 모드 거터를 `position: fixed`로(`.cm-editor`에 `transform` → containing block · 자리는
`padding-left: var(--gutter-w)` + ResizeObserver · CM 툴팁은 `parent: document.body`). 본문·커서는 그대로 튕긴다.
상세·함정은 CLAUDE.md "줄바꿈 끔 모드의 행번호 거터는 fixed다" 절과 실행판 §9-4′ ②.
아래는 해결 전 기록(재현 프로브의 한계를 보여 주는 사례로 남긴다).


**증상**: 줄바꿈을 끈 편집창에서 좌우로 스크롤하면 왼쪽 행번호 거터가 함께 밀린다.
VS Code에서는 본문만 움직이고 거터는 제자리다.
**덕수 판정**: *"그리 크게 흉해 보이진 않으니 오늘은 이쯤 하자."* → 보류.

**확정된 사실(실측)**
- CM 거터 sticky는 **블록 안에서 정상**이다 — `.cm-scroller`를 `scrollLeft 400`까지 밀어도
  거터 이동 **0px**(가능폭 687px). 재현 프로브 3회 모두 동일.
- 조상이 가로로 스크롤되면 거터도 함께 밀린다 — `.content-frame`을 200px 밀자 거터가 **−200px**.
- `.content-frame`은 좌측 열 `minWidth:420` + 미리보기 열 고정폭 때문에 드로어를 열면
  실측 **304px** 넘쳤다. **그 경로를 닫았는데도 증상이 남았다 → 미특정 경로가 하나 더 있다.**

**실패한 처방 2건 — 재시도 금지**
1. `.scaled-editor`에 `overscroll-behavior-x: contain`. 그 요소는 가로 넘침이 0이라 브라우저가
   **가로 스크롤 대상으로 아예 고려하지 않아** 발동 기회가 없다.
2. `.content-frame` 가로 패닝 제거 + 미리보기 래퍼. 경로 하나는 실제로 닫혔으나 증상 미해소.
   (부작용 없이 통과해 코드에 남겨 뒀다 — 진짜 원인을 찾은 뒤 필요 여부를 재평가할 것.)

**착수 시 첫 단계 — 재현하지 말고 실물에서 특정할 것.**
이 건은 재현 프로브로 **3연속 오진**했다(인증 게이트 때문에 실물을 못 본 것이 근본 원인).
편집 화면 콘솔에 아래를 붙이고 가로로 스와이프하면 실제로 스크롤되는 요소가 그대로 찍힌다:
...
`.cm-scroller`가 찍히면 프로브와 실물이 갈리는 지점을 파고, 그 밖의 요소가 찍히면 그것이 범인이다.
**요소를 특정하기 전의 처방은 전부 추측이다.** 상세는
`docs/phasedocs/Phase65 편집창 줄바꿈 끄기·블록 내 가로 스크롤 v2 실행판.md` §9-4′ ②.
