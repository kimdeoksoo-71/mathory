...
> **구현 완료(2026-09-04) — §9가 구현 기록.** 남은 것은 실사용 검수(§5 수동표)뿐이다.
...
### 9-2. 남은 것 — 실사용 검수 (§5 수동표)

① 그림 문항 검증이 `skip`이 아니라 실제 판정 ② 토론에서 민·클·쳇이 그림 기반 답, 식은 "그림
확인 필요" ③ 메시지에 붙인 그림을 본다 ④ `usage.inputTokens` 장당 증가분(Q7) ⑤ 그림 없는
문항 무회귀 ⑥ 그림뿐인 문항이 61d에서 `empty_question`으로 안 막힘 · **Q2**(Claude image +
thinking + code_execution 동시)는 ②에서 자연 확인.
