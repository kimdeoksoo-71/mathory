# 개선묶음 M3 — 아이콘 정비 · 버그 수정 · 기능 개선 구현 계획서 v3 (web 검증)

> 계보: 덕수 메모(`M3.md`) → v1(CLI 실측) → v2(CLI 확정판, 결정 D1~D8) → **v3(web 검증, 이 판)** → 착수판
...
| §3 B3 | 원인 정확 · 조건 인용 **불완전** · 판정 ①~⑤ 타당 | 실제 조건은 `items.length > 0 \|\| (open && blocks.length > 0)`. 처방 적용 시 **의미 선택 1건**(D9) |
...
규모: 서버 0 · 규칙 0 · 스키마 0 · 전처리 0은 유지. 신규 파일은 v2의 1개에서 결정에 따라 **1~3개**(D12·D14).
...
`OutlineSections.tsx:152`의 실제 조건은 `sec.items.length > 0 || (open && sec.blocks.length > 0)`이다. 둘째 항은 **이미 열린 전문 섹션을 닫을 수 있게** 두는 것이므로 처방 후에도 남겨야 한다(`hasHiddenBlocks || (open && …)`).
...
⚠ **의미 선택(D9)**: 지금은 `items`가 비면 여닫이가 **아예 없다** — 전문에 요약 항목 없는 일반 텍스트만 있으면 요약보기에서 그 내용은 보이지도, 펼칠 수도 없다. D6을 "`hasHiddenBlocks`일 때 그린다"로 그대로 적용하면 이런 섹션에 **여닫이가 새로 생긴다**(닿을 수 없던 내용이 닿게 됨 = 개선이지만 화면 변화 범위가 넓다). 좁게 가려면 `items.length > 0 && hasHiddenBlocks`. §9 D9.

테스트: `tests/caseBlock.test.mjs`(42건)에는 전문 여닫이 표시 여부를 보는 케이스가 **없다**(가장 가까운 것은 `:316` "첫 경우보다 위의 블록은 어느 구역에도 들지 않는다"). `hasHiddenBlocks`를 `lib/solutionOutline.ts`에 두면 여기서 검증 가능 — JSX 조건만 고치면 어떤 테스트도 덮지 않는다. v2의 4건에 **"items 없음 + 일반 텍스트만"** 1건(D9 결과에 따라 기대값 결정)을 더해 5건.
...
| A2 | `IconComment` 신설, 💬 4곳, 배지 2곳 size 12~13 | ✔ 4곳 줄번호 정확. ✘ 크기는 **13/13/11/11**(EditorView·ProblemView 버튼 13, FolderView·ListView 배지 **11**) → 배지 아이콘은 11~12. ⚠ **5번째 사이트**: `components/share/PublicViewerShell.tsx:162-168`의 로컬 `BubbleIcon`(둥근 말풍선, stroke 2, 16px) — 공개 뷰어 댓글 버튼(`:64`). D11. 참고: 💬 옆에는 항상 "AI" 쌍둥이 배지(`fontWeight 600, letterSpacing 0.3`)가 있다 — SVG로 바꾸면 이 쌍의 시각 균형을 함께 본다. Icons.tsx 규격은 1.8이 지배적이나 보편은 아님(2/2.5/1.6 혼재) |
...
| 매크로 주입구 | rehype 옵션 2곳 | ✔ `EditorPreview.tsx:316` · `PrintableContent.tsx:137` 줄 정확. 둘 다 `strict:false, trust:true`(+인쇄 `fleqn`) — 매크로 추가 시 유지. ⚠ 지금은 옵션 리터럴이 렌더마다 새 객체라 콘텐츠의 `\gdef`가 문서 밖으로 새지 않는다. **모듈 상수 하나를 공유하면 `\gdef`가 상수를 변형해 전역 오염**된다(KaTeX는 `macros` 객체를 in-place 수정). D13 |
...
D1·D2·D3·D4·D5·D6·D8 유지. **D7은 D7′로 대체 제안**(§2). D6은 D9로 의미를 확정한다.
...
2. **B3 판정이 놓치는 구성** — 이어짓기(제목 없는 case)는 curCase가 있으면 segment로 흡수(닿음), 없으면 항목도 segment도 아니어서 숨음(판정 ②·③이 맞게 처리). 레거시 라벨 혼합은 ④. 경우 뒤 일반 텍스트는 segment(닿음). 놓친 것 1건: **items 없는 전문**(§3 D9). 그리고 `heading` 섹션 제외는 맞다.
...
1. **B4**(D7′, globals.css 3규칙, 독립 커밋 — rect 대조 11/15/24px + 닫힌 목록 리듬 확인) → 2. **B3**(`hasHiddenBlocks` in solutionOutline + JSX 조건 `hasHiddenBlocks || (open && blocks.length>0)` + test 5건) → 3. **A2**(IconComment + 4곳 + D11이면 PublicViewerShell) → 4. **A1** → 5. **A3+A4**(D12 위치 확정 후) → 6. **A5**(2파일 4곳, D14) → 7. **B2**(`lib/katexMacros.ts`(D13 형태) + globals.css + rehype 2곳 — 화면·인쇄 대조, 인라인 번분수 줄간 11/15/24px, `\dbinom`·`\cfrac` 각 1건).
...
| # | 사항 | 선택지 | 권장 |
|---|---|---|---|
| **D7′** | B4 처방 | (a) v2 음수 마진 상쇄 (b) **상하 패딩·형제 간격을 상시로**(§2) | **(b)** — 떨림이 정의상 0, `:953-955`의 좌우 선례와 같은 논리. 닫힌 목록 간격 변화는 검수에서 값 조정 |
| **D9** | B3 여닫이 표시 의미 | (a) **`hasHiddenBlocks`만**(items 없는 전문에도 여닫이 생김 — 닿을 수 없던 내용이 닿게 됨) (b) `items.length>0 && hasHiddenBlocks`(현재 조용한 전문은 계속 조용) | **(a)** — 요약보기에서 내용이 영구히 사라지는 구성이 없어진다. 화면 변화 폭이 걱정되면 (b)로 시작해 나중에 넓힐 수 있음 |
| **D11** | 공개 뷰어 `BubbleIcon` | (a) **`IconComment`로 통일** (b) 그대로 | **(a)** — 같은 뜻의 아이콘이 둘이면 M3 취지(아이콘 정비)와 어긋남. 16px·stroke 2 → 규격 1.8로 |
| **D12** | 시그마 SVG·`SVG_PROPS` 위치 | (a) **`components/editor/toolbarIcons.tsx` 신설**(SVG_PROPS·CORNER_BRACKETS·SigmaIcon 이동, UnifiedToolbar·MathSymbolPalette가 import) (b) MathSymbolPalette에 SVG_PROPS 사본 | **(a)** — 순환 import 회피 + 사본 금지. 신규 파일 +1 |
| **D13** | `lib/katexMacros.ts` 형태 | (a) **팩토리 `katexMacros()`가 매번 새 객체 반환** (b) 상수 + 사용처 `{...KATEX_MACROS}` (c) 상수 그대로 | **(a)** — `\gdef` 전역 오염 차단을 모듈이 책임. (b)는 사용처가 잊으면 재발 |
| **D14** | A5 스테퍼 4곳 | (a) **`SizeStepper` 공용 컴포넌트 추출**(신규 +1, ProblemView·EditorView 교체) (b) 두 파일 수동 편집 | **(a)** — 꺾쇠 SVG가 바이트 동일한 사본 2벌인데 이번에 아이콘·'A a'를 또 두 번 붙인다. 스타일 차이(13/13.5 등)는 prop으로. 규모를 지키려면 (b)도 가능 |
| D15 | 배지 아이콘 크기(FolderView·ListView) | 11 / 12 | **12**(11px 텍스트와 캡 높이 맞춤, 실측 후 조정) |

*v3 — b2f6015 대조 · KaTeX 0.16.28 실렌더 · 오류 정정 4건(B1 참조, A2 크기, A4 색 위치, A5 편집창 폭) · 보강 5건(B4 마진 붕괴·특이도, B3 조건 둘째 항·items 없는 전문, B2 CSS 4중·\gdef, 5번째 💬 사이트) · 결정 7건. 다음: 덕수 결정 → 착수판.*
