# Phase 54 — Case 하위 케이스(sub-case) 들여쓰기 렌더링 **v2**

작성일: 2026-07-05 · 개정: 2026-07-05(v2) · 기준 커밋: `0a4d131` (Phase 53 E단계)

> **v2 개정 요약** — v1 검토에서 발견된 실제 렌더링 결함 2건과 스니펫 버튼 요청을 반영.
> ① lazy-continuation 탈출 결함 → 정규화 함수 `normalizeCaseBoundaries` 신설, ② "최대 2단계 중첩" 주장 철회(1단계 확정), ③ MathSnippetMenu에 Case/하위 케이스 템플릿 버튼 추가.
> 상세 변경 내역은 문서 맨 끝 **§12. v2 변경 설명** 참조.
...
- 하위 케이스: `- **Case 1a.**` → **한 단계 들여쓰기, 1단계만** (v2에서 확정 — v1의 "최대 2단계 중첩" 철회, §12 참조)
...
| D2 | 들여쓰기 구현 방식 | **top-level 리스트 + 불릿 숨김**. 부모 `**Case 1.**` 는 문단(리스트 아님)이고, 하위 케이스들은 **최상위 `ul`** 이 된다. 들여쓰기는 중첩이 아니라 **기본 `ul` 스타일**(padding-left 1.4em + margin-left 1em ≈ 2.4em)이 제공 → 신규 들여쓰기 값 도입 금지. (v1의 "nested list" 서술은 부정확했음 — §12 참조) |
...
4. **⚠️ lazy continuation (탈출) — v2에서 반영**: 하위 케이스 리스트 **뒤**에 빈 줄 없이 일반 문단(특히 `**Case 2.**` 같은 다음 케이스)이 오면, 그 줄이 직전 리스트 항목의 문단으로 **흡수**된다.
   예:
   ```
   **Case 1.** $x>0$ 가정.
   - **Case 1a.** …
 ...
 *  - 수식/펜스가 placeholder로 보호된 뒤 호출되어야 안전 (§4-2 배치 참조)
 *
...
수식/펜스 내부 오변환 방지를 위해 `protectMath`/`restoreMath` 로 감쌈 (이미 `lib/locale.ts:153-165`에서 export됨 → import 문에 `protectMath, restoreMath, normalizeCaseBoundaries, convertSubcaseMarkers` 추가):
...
인라인 `preprocessLocale` (line 84) 내부, **2단계(마커 행 빈 줄 삽입, line ~99-112) 이후 / 3단계(gana 변환) 이전**에 추가 (이미 수식이 `⟦MATH_n⟧`로 보호된 상태이고 `protectFences`(line 253) 이후이므로 코드펜스도 보호됨):
...
- v2 주의: 인라인 판의 정규화 블록과 lib의 `normalizeCaseBoundaries` 는 **로직·정규식이 동일**해야 함 (T7 범위에 포함)
...
- 특이도: `li:has(> .marker-case-sub)::before` = (0,2,2) > 기존 `ul li::before` (0,0,3) → `content: none` 이 이김 (검증됨)
...
- 특이도: `.print-body li:has(> .marker-case-sub)::before` = (0,2,2) > 기존 `.print-body ul li::before` (0,1,3) → 이김 (검증됨)
...
- 하위 케이스 템플릿은 `- ` 로 시작하므로, 커서가 줄 중간이면 리스트가 정상 분리되도록 사용자가 새 줄에서 삽입하는 것을 권장 (버튼 라벨/툴팁으로 안내). **자동 개행 삽입은 커서 위치 예측이 어려워 v2 범위 밖** — 필요 시 후속에서 `\n` prefix 검토
...
- [ ] **T10 (v2): 하위 케이스 리스트 바로 뒤에 빈 줄 없이 `**Case 2.**` 를 이어 써도, Case 2가 앞 항목에 흡수되지 않고 독립 문단으로 렌더** (lazy continuation 방지 검증)
- [ ] **T11 (v2): MathSnippetMenu 상단 "구조 템플릿"에서 Case/하위 케이스 클릭 시 커서 위치에 해당 마크다운이 삽입되고, 삽입 후 미리보기가 T1과 동일**
...
### 정정 없이 유지된 v1 판단 (검토 확인)

- 파이프라인 배치, 수식/펜스 보호, CSS `:has()` 특이도(0,2,2 > 기존), tight/loose 두 셀렉터, 로케일 무관 실행(D4), 원문 불변(D5) — 모두 실코드/실파이프라인 재확인 후 그대로 유지.
- v1 의 file:line 참조는 모두 정확했음 (커밋 `0a4d131` 기준 재확인).
