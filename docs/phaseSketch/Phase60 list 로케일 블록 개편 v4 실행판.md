...
### 3.5 레거시 공존 (D5) — **철회됨(2026-08-18)**

> 아래는 착수 시점의 판단이다. 실사용 결과 두 표기의 공존이 혼란스럽다는 판정으로 **레거시 변환을 전부 삭제**했다(D5 참조). `convertAlphaList`·`convertRomanList`·`GANA`/`GIYEOK` 테이블·`ALPHA_LINE_RE`·`ROMAN_LINE_RE`가 사라졌다.
...
| ~~**D5**~~ | 레거시 (a)/(i) | ~~유지~~ → **철회(2026-08-18): 변환 삭제** | 착수 시엔 "공존 무해"로 봤으나 실사용에서 두 표기가 같이 동작하니 **무엇을 쓰면 무엇이 나오는지가 흐려져** 오히려 혼란스러웠다(덕수). 변환·매핑 테이블을 걷어내고 옛 문항은 그때그때 리터럴로 고친다. **`MARKER_LINE_RE`의 `(a)`/`(i)`는 남긴다** — 행 분리는 변환이 아니라 렌더 보조이고, 빼면 옛 문항이 뭉친다 |
