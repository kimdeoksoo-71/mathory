...
| # | 요구 | 판정 | 비용 |
|---|------|------|------|
| **P1** | **display 수식 상하 여백 재조정(신규 기준) + 리스트 상하 여백 동일 적용** | **타당·가능** — CSS만. 덕수 확정(아래): 실질 추가 간격 **위아래 각 +0.5em 동일**. 현행 수식 여백(위 +0 / 아래 +0.8em 비대칭)을 이 기준으로 교정하고 리스트·①원문자 리스트(D1)·강조문(P3)에 동일 적용. 여백 스코프 주의(D5) | 소 (CSS) |
...
### ✅ 덕수 확정 (2026-08-14) — 상하 여백 통일 기준

- **display 수식의 상하 "실질 추가 간격"을 위아래 각 0.5em으로 동일하게** 재조정한다. (현행은 마진 상쇄 후 위 +0 / 아래 +0.8em 비대칭 — §2.1의 산정 근거 참조.)
- **같은 기준을 '목록' 블록(markdown 리스트 + ①원문자 리스트)과 '강조문' 블록에 그대로 적용**한다. → 문서의 세 가지 "돋보임 요소"(수식·목록·강조문)가 하나의 리듬을 공유.
...
2. §2.3 마진 상쇄(collapsing) 실측 — 신규 기준(마진 1.5em/인쇄 11pt)이 "문단↔수식·문단↔리스트·수식↔리스트" 세 조합 모두에서 동일 간격으로 렌더되는지 (display 수식 요소가 p 안에 중첩되는 구조면 상쇄가 깨질 수 있음 — 그 경우 조합별 보정).
...
## 2. P1 — 상하 여백 통일 기준 (display 수식 재조정 + 리스트 동일 적용)

### 2.1 스펙 — "실질 추가 간격 위아래 각 +0.5em" (덕수 확정)

마진 값이 아니라 **체감 간격**으로 정의한다: 일반 문단 사이 간격 대비 **위아래 각 +0.5em씩 동일하게** 추가.

**마진 값 산정 (상쇄 역산):**
- 화면: 일반 문단 간격 = p의 UA 기본 마진 1em(상쇄 후). 목표 간격 = 1em + 0.5em = **1.5em** → 요소 마진을 `1.5em`으로 주면 상쇄 `max(1.5em, 1em)` = 1.5em ✓ (위아래 대칭).
- 인쇄: 일반 문단 간격 = `.print-body p`의 margin-bottom 6pt(PrintStyles.css:50, 본문 10pt 기준 0.6em). +0.5em = +5pt → 목표 **11pt** → 요소 마진 `11pt` (위: max(11pt, 이전 p 6pt) = 11pt ✓ / 아래: 다음 p margin-top 0 → 11pt ✓).

**적용 대상 (전부 동일 값):** display 수식(**현행 1em/1.8em 비대칭을 교정**) · markdown ul/ol · ①원문자 리스트 런(D1) · 강조문 블록(P3).

### 2.2 구현 (CSS만)

```css
/* ── 화면 ── */
/* EditorPreview.tsx:441-444 inline <style> 교체 — display 수식 신규 기준 */
.preview-content .katex-display { margin-top: 1.5em !important; margin-bottom: 1.5em !important; }

/* globals.css — 리스트. D5: 콘텐츠 스코프 한정 (전역 ul/ol 규칙은 건드리지 않음) */
...
```css
/* ── 인쇄 (PrintStyles.css) ── */
.print-body .katex-display { margin: 11pt 0; }   /* 현행 8pt/14.4pt(65행) 교체 */
.print-body ul, .print-body ol { margin: 11pt 0; }
.print-body li ul, .print-body li ol { margin: 4pt 0; }
```

- 좌측 마진/패딩(불릿 내어쓰기·수식 들여쓰기 체계)은 **현행 유지** — 이번 Phase는 상하 여백만.
...
- **기존 문서 전체의 수식 간격이 바뀐다**(위 +0.5em 증가, 아래 -0.3em 감소) — 의도된 전면 변화임을 명시.

### 2.3 마진 상쇄 판단 (검증 요청 2)

§2.1의 산정은 마진 상쇄를 전제한다(요소가 문단의 형제로 렌더될 때). display 수식 요소가 p 안에 중첩 렌더되는 경우 상쇄가 달라질 수 있으므로, **실렌더에서 "문단↔수식·문단↔리스트·수식↔리스트" 세 조합의 간격이 전부 1.5em(인쇄 11pt)으로 측정되는지** 확인. 어긋나는 조합이 있으면 그 조합만 보정 셀렉터 추가.
...
.preview-content :not(p:has(.marker-circled)) + p:has(.marker-circled) { margin-top: 1.5em; }
.preview-content p:has(.marker-circled):first-child { margin-top: 1.5em; }
/* 런의 끝: 뒤 형제가 ①문단이 아닌 경우 — :has(+ …)로 판정 */
.preview-content p:has(.marker-circled):not(:has(+ p .marker-circled)) { margin-bottom: 1.5em; }
```
...
/* 화면 (globals.css) */
.callout-block {
  margin: 1.5em 0;           /* P1 통일 기준 — display 수식·리스트와 동일 (실질 추가 +0.5em) */
...
```
# P1 (CSS만)
components/editor/EditorPreview.tsx — inline <style>(441-444) katex-display 1.5em/1.5em 교체
app/globals.css                     — .preview-content ul/ol 상하 여백 + D1(①런 정규화)
components/print/PrintStyles.css    — .katex-display 11pt(65행) + ul/ol 인쇄 parity + D1
...
**Stage 1 · P1 상하 여백 통일 (CSS)**
- display 수식 신규 기준(1.5em/1.5em, 인쇄 11pt) 교체 + ul/ol + D1(①런). 검증: 문단↔수식·문단↔리스트·수식↔리스트 세 조합 간격이 전부 동일(§2.3 실측), 문단→①①①→문단, 중첩 리스트, 선택지 셀 내부 무영향, 앱 UI(사이드바 등) 리스트 무영향(D5 스코프 확인), 기존 문항 몇 개로 수식 간격 변화(위 +0.5em/아래 -0.3em) 육안 확인.
...
**A-4. display 수식 간격·들여쓰기 (현행 → 이번에 교정).** 화면: `.preview-content .katex-display { margin-top:1em; margin-bottom:1.8em }` (EditorPreview.tsx:441-444 inline style), `.katex-display { padding-left: 3em }` (globals.css:185-187). 인쇄: `margin: 8pt 0 14.4pt` (PrintStyles.css:65), fleqn `padding-left: 2em` (PrintStyles.css:24). **상쇄 분석**: 인접 p의 UA 마진 1em과 상쇄되어 실질 추가 간격은 위 +0 / 아래 +0.8em 비대칭 — 덕수 확정으로 위아래 각 +0.5em(마진 1.5em/1.5em, 인쇄 11pt)으로 교체(§2.1). 참고: 세로 큰 수식 행의 0.4em 패딩(globals.css:328-332)은 별도 장치로 유지.
