# Phase 66a — 문답 검증 1단계: agent 탭 질문 리스트 구현 계획서 v3 (web 재검증 · 착수판)

작성일: 2026-09-14 · 작성: **web 재검증** · 기준 커밋: **mathory `origin/main 9061cf9`**(fetch로 최신 확인 — v2 기준과 동일)
계보: 덕수 스케치 → Phase 66 v1(전체 구상) → 브레인스토밍 → 66a v1(web) → 66a v2(CLI 실측) → **66a v3 = web 재검증 착수판**
(부록 C가 v2 판정 — 수용 8 · **역정정 2** · 신규 보완 10)
...
Firestore 규칙 **1블록 추가** · 신규 3파일 · 기존 5파일 수정 · **ICONS 60 → 61종**.

⚠ **v3에서 뒤집힌 것 하나 — 실험 성패를 가르는 자리다.**
v2의 **D15(질문 1개 = 세션 1개)를 폐기**하고 **D15′(문답 전송은 히스토리를 싣지 않는다)**로 바꾼다.
v2가 지적한 오염(`HISTORY_LIMIT 5`가 앞 답을 뒤 질문에 실어 보낸다)은 실재하지만, 처방이 세션 분리면
문항 5개 × 질문 3개 = **세션을 손으로 15번 만들어야** 하고 세 답이 서로 다른 세션에 흩어져 비교가 불편해진다.
호출부에서 `discussionHistory: []`를 넘기면 오염이 원천 차단되고, 세 답을 **한 세션에 나란히** 쌓아 읽을 수 있으며,
66b 배치가 쓸 갈래(§11-1 "배치는 히스토리 없이")를 1단계에서 미리 만드는 셈이 된다. 코드는 한 줄 분기다(V1).
...
| 폰 대응 | — | **필요 없다**(V5) | `PhoneApp:392`가 agent를 `canComment={false}`로 마운트해 컴포저(`CommentPanel:1099`)가 아예 없다 → `headerLeft`도 질문 버튼도 렌더되지 않는다. **폰 전용 코드 0** |

**남는 것.** 질문 컬렉션 + 씨앗 3개 + 리스트 팝오버(선택·수정·복제·삭제) + 전송.

---

## 2. 확정 사실 (origin/main `9061cf9` — v3가 전수 재검증, 행 번호는 이 커밋 기준)
...
| 바디 조립 | `:749-764` | `discussionHistory: history` · `images.history: historySlots` |
...
⚠ `runVerify`(`:807-836`)는 실행과 함께 `setSelectedModelIds([])`(`:822`)를 한다. **문답 전송은 칩 선택을 건드리지 않는다** —
override로 자기 모델을 따로 들고 가므로 칩은 타이핑 대화용으로 그대로 남는 것이 맞다(G7).

### 2-2. 사용자별 편집 가능 컬렉션의 전례 — `math_snippets`

- 규칙 **`firestore.rules:30-32`**(v2의 "29-31" 정정은 오류 — C1. 29는 빈 줄이고 `match`가 30행):
  ```
  30:      match /math_snippets/{snippetId} {
  31:        allow read, write: if request.auth != null && request.auth.uid == userId;
  32:      }
  ```
  `toolbar_config`는 `:35-37`. 새 블록은 그 아래(`:38` 앞).
...
- **팝오버 기준 상자 = 컴포저 래퍼 `:1100-1104`**(`padding:'10px 16px 12px'` · `position:'relative'`) ✓ 실측.
  ...
  `left:0; right:0`** 으로 래퍼 폭을 그대로 쓸 것(E10 — `VerifyChips` 팝오버는 `:1490-1491`이 `width:250; left:50%; translateX(-50%)`).
...
### 2-4. 아이콘은 한 키를 늘려야 한다 (v1 정정 — E1 유효)

`PH.listChecks`는 이미 **`ProofreadIcon`(교정)**이 쓴다(`scripts/gen-phosphor-paths.mjs:78` 주석 `// ProofreadIcon` ·
...
```
**존재 확인(저장소 안에서 되는 방법 — C9)**: `lib/phosphor-ko.json`(카탈로그 1,414종)에 `list-dashes`가 있다.
`node_modules` 경로 확인은 설치된 환경에서만 되므로 계획서의 근거로 쓰지 않는다.
...
| 제약 | 근거 | 대응 |
|---|---|---|
| **답변 규칙은 길이만이 아니다** — 800자(고난도 1200자, `:98`) **+ "단계 나열(1./2./3.… 번호로 풀이 재작성)" 금지(`:101`) + "장황한 사고 과정 서술은 응답이 무효 처리되는 사유"(`:88`)** | `BASE_SYSTEM_PROMPT` `route.ts:60-115` | 질문 본문 한 줄로는 **시스템 프롬프트를 못 이긴다**. 레버는 `appendPrompt` → **D14** |
| `ai_models.maxTokens` 기본 **1024** | `lib/ai-models.ts:24` · `route.ts:571`이 `config.maxTokens`를 그대로 넘긴다 | D14에서 8192. ⚠ **21,333 미만을 유지할 것**(V4) |
| `getEnabledModels`는 **모듈 캐시** | `lib/ai-models.ts:13-14, 52-58` | 콘솔에서 문서를 고쳐도 **새로고침 전까지 반영 0**(E6) |
| `appendPrompt`는 시스템 프롬프트 뒤쪽 | `route.ts:241-243`(`[추가 지침] …`) | ⚠ **맨 뒤는 아니다**(C3) — `STRUCTURED_OUTPUT_INSTRUCTION`(`:244-246`)이 뒤에 올 수 있다. 단 그건 `structuredOutput`(DeepSeek) 전용이라 **Claude 경로에서는 실질적으로 맨 뒤** |
| `/api/discuss` 무인증 · 오류 500 일괄 | `route.ts` 전역 · catch `:600`, 반환 `:603-606` | 1단계는 단건 수동 실행이라 중단 정책이 필요 없다 |
| 컨텍스트 15,000자 자름 | `CommentPanel.tsx:553, 568-573`(`console.warn`만) | dev 콘솔로 확인(G6) |
| **히스토리 최근 5개가 딸려간다** | `HISTORY_LIMIT = 5`(`:33`) · `:600` · `:739` | **D15′ — 문답 전송은 히스토리를 싣지 않는다**(V1) |
| 리포트 라벨이 '정밀 검증' | `CommentPanel.tsx:405` | **61b UI만 '교차 검증'으로 개명**, 새 기능은 **'문답 검증'**(D1′) |
| AI 댓글 create는 오너만 | `firestore.rules:262-268`(C4) — `authorType=='ai'` ∧ `authorUid.matches('^ai:.*')` ∧ **`resolved == false`** ∧ `isOwnerCmt()` | `addComment`가 `resolved:false`를 항상 쓴다(`lib/comments.ts:101`) → 추가 작업 0 |
| 비용은 **메시지마다** 이미 보인다 | `:1931-1940`(`$…` 5자리 + 토큰 title) · 세션 합계 `:362, 970-982` | §9-5를 메시지 단위로 읽으면 된다 |
...
| 재시도가 공짜로 따라온다 | `handleRetryAI` `:838` + `retryContext` `:669` | 문답도 일반 모델 경로라 실패 시 [재시도] 버블이 그대로 동작(G2) |
...
| **D4** | 컬렉션이 비어 있으면 **씨앗 3개 자동 생성** — 트리거는 **팝오버 첫 열기**(패널 마운트 아님) + 모듈 in-flight 싱글턴 | 패널은 EditorView·ProblemView(·폰)에서 마운트된다. 마운트 훅이면 문항을 열 때마다 읽기가 돈다(G1) |
...
| **D8** | 모델은 **D14 실험용 문서 기본 선택**, 변경 가능 | 문안을 비교하려면 모델이 고정 변수여야 한다. 기본값 해석 = `localStorage` → 없으면 `provider==='anthropic'` 첫 모델 |
...
| **D12** | 전송 시 편집창이면 **저장 먼저** | discuss 컨텍스트는 Firestore 저장본을 읽는다. 61b 칩의 dirty 가드(**`EditorView.tsx:3113-3116`** — `handleSave(true)` 후 `lastSaveOkRef` 확인, C5)를 같은 방식으로 물려받는다. **열람뷰는 해당 없음** |
| **D13** | ICONS에 `listDashes` 1키 추가(60 → **61종**) | 2-4 |
| **D14** | **실험 전용 `ai_models` 문서 1개**(Claude 복제 · `maxTokens 8192` · 단가 2필드 복제 · `appendPrompt`로 길이·형식 완화) — 콘솔 작업, **코드 0** | 2-5. 800자·"단계 나열 금지"·"장황하면 무효"는 **시스템 프롬프트**에 있다. 기존 Claude 문서를 고치면 타이핑 대화·다른 경로까지 바뀐다 → 별 문서로 격리. ⚠ 단가를 빼면 비용 관측이 죽는다(V3) · ⚠ 칩이 하나 늘어난다(V2) |
| **D15′** | **문답 전송은 히스토리를 싣지 않는다** — `discussionHistory: []` · `images.history: []` (v2의 "질문 1개 = 세션 1개" 폐기) | V1. 오염은 실재하지만 처방이 세션 분리면 세션을 손으로 15번 만들어야 하고 세 답이 흩어진다. 한 줄 분기로 오염을 없애고, 세 답을 한 세션에 쌓아 나란히 읽는다. 66b 배치가 쓸 갈래를 미리 만드는 셈 |
| **D16** | [보내기] 활성 = **해석된 모델 ≥ 1** ∧ `activeSessionId` ∧ 진행 중 아님 | `invokedModels`는 `aiModels.find`로 해석되므로(`:722-724`) 사라진/비활성 modelId가 저장돼 있으면 **사람 메시지만 저장되고 AI 0회**(E4) |
...
⚠ **G3는 시스템 프롬프트와 정면으로 부딪힌다**(E5). `BASE_SYSTEM_PROMPT`가 "**단계 나열(1./2./3.… 번호로 풀이 재작성) 금지**"
(`route.ts:101`)와 "장황한 사고 과정 서술은 **응답이 무효 처리되는 사유**"(`:88`)를 못 박는데, G3가 요구하는 것이
정확히 **풀이 재작성**이다. D14의 `appendPrompt` 없이 G3를 돌리면 측정 대상이 "문안의 질"이 아니라 "시스템 프롬프트의 저항"이 된다.
...
**팝오버 2단 — 전송.** 질문 라벨 · 모델 칩(기본 D14 문서) · **"이 전송은 이전 대화를 참조하지 않습니다"** 한 줄(D15′) ·
[취소] [보내기](활성 조건 D16).
누르면 → (편집창이고 dirty면 저장 먼저, D12) → 팝오버 닫힘 → `setReplyingTo(null)`(D17) →
`handleSendMessage(전송문, { modelIds, noHistory: true })`.
사람 메시지가 대화에 뜨고 `PendingAIBubble`이 모델 수만큼(실패 시 [재시도]도 그대로 — G2).
입력창의 미완성 텍스트는 건드리지 않는다.

**편집 모달.** `dialogStyles` 규격(`dialogOverlay`·`dialogBody`·`dialogHead`(57)·`dialogContent`·`dialogFoot`·`dialogInput`·`dialogBtn()`).
...
⚠ `position:fixed` 안전 확인(V9): 같은 패널 안 `SelectionInsertPopup.tsx:291`이 이미 fixed로 동작한다(전례).
...
| `lib/askQuestions.ts` (firestore 접촉 · ⚠ `lib/ask/`에 두지 말 것) | `listAskQuestions(uid)`(⚠ `order` 없는 문서는 쿼리에서 빠진다 — dev 경고) · `createAskQuestion` · `updateAskQuestion`(rev+1) · `duplicateAskQuestion` · `deleteAskQuestion` · `ensureSeeded(uid)`(모듈 in-flight 싱글턴 — G1) — `lib/snippets.ts:16-64` 형태 그대로 |
...
| `firestore.rules` + `tests/firestore.rules.test.mjs` | §4 블록 + **케이스 64·65**(현재 번호는 63까지 — E7) |
| `package.json` | `test:ask` — 기존 문법 그대로 복제, ⚠ **`--rootDir .` 필수**(산출물이 `.test-build/lib/ask/`로 떨어져야 한다, E8) |
...
| S3 | `lib/askQuestions.ts` + `AskListPopover` + CommentPanel 배선(D15′ 포함) + D1′ 개명 | 씨앗 3개 자동 생성 · 목록·수정·복제·삭제 · 전송 · **요청 바디에 `discussionHistory: []`** · 답글 모드에서도 AI 호출됨 |
...
**실물 검수**
1. agent 탭에서 G1·G2·G3를 같은 고난도 문항에 각 1회 — **같은 세션에 연달아** 보내고, 네트워크 탭에서
   **`discussionHistory: []`**·`images.history: []` 확인(D15′). → §9 기록
...
5. 회귀: 기존 타이핑 전송(히스토리 **정상 첨부**) · 재시도 · 검증 칩(라벨 '교차 검증') · 그림 첨부(61f) 무변경
6. 폰(기기 모드) agent 시트 — 질문 버튼이 **없어야** 한다(V5, 컴포저 부재)
...
1. **실험 전용 `ai_models` 문서 1개 생성**(콘솔, D14) — 기존 Claude 문서를 복제하고
   - `maxTokens`: **8192** (⚠ **21,333 미만 유지** — 넘기면 SDK 비스트리밍 가드가 요청 전에 던진다, V4)
   - `nickname`: 기존과 다른 한 음절(`getReservedNicknames`가 사용자 닉네임 예약에 쓰므로 구별되게 — 예 '문')
   - `inputCostPerMillion` / `outputCostPerMillion`: **원본과 같은 값으로 복제**(빼면 비용이 0으로 찍혀 §9-5가 죽는다, V3)
   - `appendPrompt`: 길이·형식 완화 한두 줄
   ...
   ⚠ `enabled:true`인 문서는 **모든 문항의 agent 칩 바에 칩으로 뜬다**(V2) — 전역 `ai_models`라 그렇다.
     실험 종료 후 `enabled:false`로 내린다.
2. 실험 대상 문항 3~5개 선정 — **군더더기가 있을 법한 고난도·긴 풀이**.
...
문안 3개를 같은 문항에 돌리고(한 세션에 연달아 — D15′가 오염을 막는다) 다음 여섯 가지를 본다.
...
5. **비용** — AI 메시지 하단의 `$…`(`:1940`)를 문항당 합산 · 세션 합계(`:982`)도 함께 → 배치 예상 비용
...
- **검증 칩 삭제** — `VerifyChips`·`VERIFY_CHAR_CAP`·`runVerify`·`PendingAIBubble`의 `kind:'verify'` 합성 갈래 ·
  ...
  (`EditorView:3106, 4137, 4139` · `ProblemView:338, 1282, 1284`). 폰은 `onRunVerify`를 안 넘겨 영향 0.
...
- **제미나이 제안(2026-09-14 덕수 공유)의 소화**: 4개 렌즈 중 ①미사용 부산물은 G2에 흡수, ②우회 경로·③자명한 단계 과다·
  ④잉여 케이스는 전부 **압축 축**이라 G3의 세부. ⚠ 체크리스트 4개를 **한 프롬프트에** 넣는 형태는 위 원칙에 어긋나 채택하지 않았다.
...
| 버전 | 작성 | 산출 |
|---|---|---|
| 66 v1 | web (2026-09-14) | 전체 구상 — 질문 9종 · 폴더뷰 배치 · D1~D19 |
| 66a v1 | web (2026-09-14) | 1단계로 축소 — 칩 유지·lib 추출 유예·질문 3개·Firestore 편집 구조 · D1~D12 |
| 66a v2 | CLI 실측 (2026-09-14) | 정정 10 · 보완 8 · D1′·D13~D18 · S1~S5 · T6 · 규칙 64·65 |
| **66a v3** | **web 재검증 (2026-09-14)** | **v2 전 인용 재검증 — 수용 8 · 역정정 2(C1·C9) · 행 번호 정정 5(C2·C4·C5·C6·C7·C8) · 신규 보완 10(V1~V10) · D15 → D15′ 대체 · 착수 가능** |

## 부록 B. v2 판정 — 수용한 지적 (재검증 통과)

| v2 | 실측 결과 |
|---|---|
| **E1** `listChecks`는 ProofreadIcon(교정)이라 재사용 불가 | ✓ `gen-phosphor-paths.mjs:78`에 `// ProofreadIcon` 주석 실재 · `UnifiedToolbar.tsx:83`이 소비. **v1의 "ICONS 무변경"은 오류였다** |
| **E2** 답글 분기가 AI 호출을 삼킨다 | ✓ `if (replyingTo)` → `addComment(parentCommentId)` → `return`. 행은 **`:689`**(C7) |
| **E3** override는 대체여야 한다 | ✓ `:721`이 `selectedModelIds`를 읽는다. 합집합이면 켜 둔 칩 전부가 같은 질문을 받는다 |
| **E4** 해석 실패 시 사람 메시지만 저장 | ✓ `:722-724` `aiModels.find` → `:735` 조기 반환 |
| **E5** 제약은 길이만이 아니다 | ✓ `:88` 무효 처리 · `:101` 단계 나열 금지(행 정정 C2). **G3가 요구하는 것이 바로 풀이 재작성** |
| **E6** `getEnabledModels`는 모듈 캐시 | ✓ `ai-models.ts:13-14, 52-58` |
| **E7** 규칙 테스트는 63번까지 | ✓ 마지막 번호 `test('63. …')` — 64·65가 맞다(번호 없는 블록 2개는 별개) |
| **E8** `test:ask`에 `--rootDir .` 필요 | ✓ 기존 스크립트 전부가 그 문법(`test:tidy`·`test:list` 등) |
| **E10** `VerifyChips` 팝오버는 `width:250`·`left:50%` | ✓ `:1490-1491`. 목록·편집에는 좁다 |
| **G1·G2·G4·G5·G7·G8** | ✓ 전부 사실 — 씨앗 트리거(마운트 지점 다수) · 재시도 공짜 · `currentMessage` 별도 필드 · 히스토리 오염 · 칩 선택 비대칭 · order 규칙 |

## 부록 C. v3 판정 — 역정정(C)과 신규 보완(V)

### 역정정 — v2가 틀렸거나 행이 어긋난 것

| # | v2 서술 | 실측 | 결과 |
|---|---|---|---|
| **C1** | "`math_snippets` 규칙 행을 30-32 → **29-31**로 정정" | `sed -n '25,40p' firestore.rules` 실측: `match`가 **30**행, `allow` 31, 닫기 32. **v1이 옳았고 v2의 정정이 오류** | **30-32**로 환원 |
| **C2** | 단계 나열 금지 = `route.ts:102` | `:101`이 "단계 나열(1./2./3.… 번호로 풀이 재작성)", `:102`는 "일반론 서론" | **:101** |
| **C3** | "`appendPrompt`는 시스템 프롬프트 **맨 뒤**" | `:241-243` 뒤에 `STRUCTURED_OUTPUT_INSTRUCTION`(`:244-246`)이 온다 | DeepSeek 전용이라 **Claude 경로에서만 실질적 맨 뒤** — 문구 정확화 |
| **C4** | AI 댓글 오너 규칙 = `:260-266` | 실제 `:262-268` · 조건에 **`resolved == false`**가 함께 있다 | 행 정정 + `addComment`가 항상 `resolved:false`를 쓴다는 사실 추가(`comments.ts:101`) |
| **C5** | dirty 가드 = `EditorView:3111-3115` | `handleRunVerify` `:3106`, 주석 `:3109-3112`, **`if (dirty)` :3113 / `handleSave(true)` :3114 / `lastSaveOkRef` :3115 / 닫기 :3116** | **:3113-3116** |
| **C6** | `phIcon` 팩토리 = `Icons.tsx:37-42` | `function phIcon` **:38**, 닫기 :43 | **:38-43** |
| **C7** | 답글 분기 `:686` · 댓글 `:701-711` · 세션 가드 `:714-717` · `setSelectedModelIds([])` `:821` | 실제 **:689** · **:703-712** · **:715-718** · **:822**(:821은 주석) | 전부 정정 |
| **C8** | discuss 500 반환 `:598-606` | catch **:600**, `NextResponse.json(... 500)` **:603-606**(`:598`은 `costUsd` 필드) | **:600-607** |
| **C9** | `list-dashes` 존재를 `node_modules/@phosphor-icons/core/...`로 확인 | 저장소에 `node_modules`가 없는 환경에서는 재현 불가 | **`lib/phosphor-ko.json`(1,414종)에 `list-dashes` 존재**로 근거 교체 — 저장소 안에서 검증 가능 |

### 신규 보완 — v2가 다루지 않은 것

| # | 내용 |
|---|---|
| **V1** | **D15(질문 1개 = 세션 1개) 폐기 → D15′(히스토리 미첨부).** 오염(G5)은 사실이나 처방이 과하다 — 문항 5 × 질문 3 = 세션 15개를 손으로 만들어야 하고 세 답이 흩어져 비교가 불편하다. `:739` 한 줄을 `opts?.noHistory ? { history: [], slots: [] } : buildHistory()`로 바꾸면 오염이 사라지고(그러면 `images.history`도 자동으로 `[]`, `hasAnyFig`의 `historySlots.some` 계산도 자연히 false) 세 답을 한 세션에 쌓아 읽을 수 있다. §11-1이 이미 "배치는 히스토리 없이"라 적었으므로 **66b가 쓸 갈래를 1단계에서 만드는 셈** |
| **V2** | **D14 실험용 문서는 전역 `ai_models`라 모든 문항의 칩 바에 칩이 하나 더 생긴다** — `AIChipBar`가 `getEnabledModels()` 전체를 그린다(`:1160-1161`). 1인 사용이라 수용하되 nickname을 구별되게 짓고 실험 후 `enabled:false` |
| **V3** | **D14 문서에 단가 2필드(`inputCostPerMillion`·`outputCostPerMillion`)를 복제해야 한다** — 빠지면 `calcCost`가 0을 내고 §9-5(비용 관측)가 통째로 죽는다 |
| **V4** | **`maxTokens 8192`는 안전하되 21,333이 상한선이다** — Anthropic SDK 비스트리밍 가드(2026-09-11 사고, learnings)가 그 위에서 요청 전에 던진다. `ClaudeProvider`는 `timeout`을 명시하지만(`ai-provider.ts:387, 396`) 계획서에 선을 적어 둔다 |
| **V5** | **폰 영향 0** — `PhoneApp.tsx:392`가 `panelSlot('agent', false)`로 마운트해 `canComment=false` → 컴포저(`CommentPanel:1099`)가 렌더되지 않고 `headerLeft`도 없다. 폰 전용 코드·검수 항목이 필요 없다(단 "버튼이 없어야 한다"는 확인만 §7-6) |
| **V6** | **`orderBy('order')`는 `order` 필드가 없는 문서를 결과에서 제외한다** — 콘솔에서 손으로 만든 문서가 목록에서 조용히 사라진다. 규약으로 못 박고 `listAskQuestions`에 dev 경고 |
| **V7** | **질문 전문이 사람 메시지로 대화에 그대로 남는다** — G2는 ~600자라 답 앞에 벽이 선다. 실험 로그로는 이득이라 1단계는 수용, 거슬리면 66c에서 접는 렌더 |
| **V8** | **`CommentEditor.maxLength 1000`은 문답 전송에 안 걸린다** — 그 상한은 textarea 전용(`CommentEditor.tsx:51, 236`)이고 문답은 `handleSendMessage`를 직접 부른다. 유일한 상한은 `validateQuestion`의 8000자 |
| **V9** | **편집 모달의 `position:fixed`는 안전** — 같은 패널 안 `SelectionInsertPopup.tsx:291`이 이미 fixed로 동작하는 전례가 있다(패널 조상에 transform 없음) |
| **V10** | v2가 `dialogStyles`의 버튼 헬퍼를 `dialogBtn`으로 적은 것은 **맞다** — `export function dialogBtn(`(`dialogStyles.ts:79`). 다만 `export const` 목록만 훑으면 안 보이므로 §5에 시그니처를 명시해 둔다 |

---

*v3 — 착수 가능. 덕수 준비물은 §8(콘솔 작업 1건 + 문항 선정 + 규칙 배포). 서버·프롬프트·61b/61d/61h 코드 변경 0.*
