# Phase 62 구현 계획서 — 폴더뷰 구조 변경 · 좌우 사이드바 UI 통일 (v3 최종 · 착수본)

> 계보: v1(web, 2026-08-26) → v2(CLI 실측 교차검토, 2026-08-27) → **v3(web, v2 전수 재검증 · 착수본, 2026-08-27)**
> **v3의 성격**: v2의 정정 E1~E10 · 결정 D1′~D20′ · 명암비 표를 레포(`431c6f6` 클론)에 대고 **전건 재검증**했다. 결과: **E 10건 전부 승인 · D 20건 중 18건 승인 · 2건 정정(F1·F3) · 보완 4건(F2·F4·F5·F6)**. 이 문서는 v2를 대체하는 착수본이며, v2와 어긋나는 곳은 §0의 F 번호가 근거다.
> 진실 원천: mathory **origin/main `431c6f6`**. 모든 라인 인용은 CLI(v2)·web(v3)이 각각 그 커밋을 직접 읽어 확인했다.
...
| v2 항목 | 판정 | 비고 |
|---|---|---|
| E1 (aside `overflow:hidden` → 핸들 잘림) | **승인** | Sidebar.tsx:815 실측. `:265` absolute 인디케이터도 실측 — `position:relative` 회피 근거 성립 |
| E1′ (`overflow-y:auto` → x도 auto) | **승인** | CSS Overflow 3 규약(한 축이 `visible`이 아니면 다른 축의 `visible`은 `auto`로 계산) |
| E2·E3 (패널 위 세로선 · 228px 死공간) | **승인** | CommentPanel zIndex 50(:802) < 핸들 100 · 컨텐츠 행(:672)에 paddingRight 없음 · 래퍼 `paddingRight panelWidth+8`(:677) + 우측 단 220 = 228 재계산 일치 |
| E4 (`#DDD3C3` 기각) | **승인** | 독립 재계산 1.29 / 1.12 일치. globals.css:80 주석("`#DCD3C2`(1.24:1) 너무 흐려 기각(덕수)") 실측 |
| E5 (보정값 12·24 ≠ 경계선 8) | **승인** | strip `right: panelWidth+3`, 폭 10 → 중심 `panelWidth+8`. onMove `-12`/`-24`로 4px·16px 스냅 재계산 일치 |
| E6·E7·E8 | **승인** | ContextMenu `fixed·1000`(:71-74) · content-frame `overflowX:auto`(:3484) · 미리보기 `calc(38.5em+32px)`+`marginLeft 24`(:3601-3603) 실측 |
| E9 (stale 주석 4건) | **승인** | :1027-1031 "zIndex 60" · :3765 "200ms dwell" · FolderView:493 "14px 라운드" 실측 |
| E10 (공유 스코프 재사용) | **승인** | AppShell:711·:758 두 마운트 실측 |
| §3-D 명암비 표 | **승인** | 4행 전 수치 독립 재계산 일치 |
| D1′~D6″ · D8′~D20′ | **승인** | 아래 F3·F4·F5·F6은 보완이지 번복이 아니다 |
| **D7″ (제목행 배경 `--bg-hover`)** | **⚠ 정정 — F1** | "클레이보다 밝음"은 **거짓** — `#F0EBE3`은 클레이 `#F4EFE7`보다 어둡다 |
| **D9″ (`background !important`)** | **⚠ 정정 — F3** | `!important`가 필요 없는 구조가 D4′ 안에 이미 있다 |

### 0-2. F1 ★ — 제목행 톤의 전제가 틀렸다 (v2 유일의 사실 오류)

D7″는 제목행 배경 `--bg-hover(#F0EBE3)`를 "아이보리보다 톤, **클레이보다 밝음**"이라 썼다. 상대 휘도 실측:
아이보리 `#FEFDFB` 0.983 > 클레이 `#F4EFE7` 0.867 > **`--bg-hover #F0EBE3` 0.835** > `--block-bg #F0EAE0` 0.828 > `--block-bg-active #E8DFCE` 0.744.
즉 `--bg-hover`는 **행(클레이)보다 어둡다**. 구상의 "살짝만 톤"은 **아이보리 바탕 기준**이고, 아이보리와 클레이 사이(1.13:1)에는 쓸 만한 토큰이 없다.
따라서 제목행은 행보다 **한 단 어두운 카드**가 되는 것이 불가피하며, 그렇다면 토큰은 의미에 맞는 것을 써야 한다:

> **D7‴: 제목행 배경 = `--block-bg`(#F0EAE0).** 아이보리 대비 1.18 · 행(클레이) 대비 1.05 · hover(#E8DFCE)보다 밝다 → 위계 **아이보리 < 행 < 제목행 < hover**가 단조롭게 선다. `--bg-hover`는 hover 상태 전용 토큰이라 정지 요소에 쓰면 의미가 꼬인다(행 hover는 `--block-bg-active`를 쓰므로 더 혼란). 육안으로 무거우면 **클레이 그대로**(행과 동일, 고정만 담당)가 차선 — Stage 3 판정.

### 0-3. F2 — sticky 제목행 위 8px 틈으로 행이 지나간다 (보완)

ListView 루트 `padding: '8px 0 32px'`(D8′)의 **위 8px는 제목행 바깥**이다. 제목행이 `top:0`에 붙어도 그 위 8px 띠는 배경이 없어 **스크롤된 행이 그 띠를 통과하며 보인다**. 처방: 루트 상단 패딩을 0으로 두고 제목행을 **sticky 래퍼**로 감싼다 — `position:sticky; top:0; zIndex:2; background: var(--bg-functional); padding: 8px 0 4px` 안에 제목행 카드. 래퍼가 아이보리를 칠해 위 띠를 덮고, 아래 4px는 D7″의 `marginBottom 4` 역할을 대신한다(카드 자체의 marginBottom은 0). 라운드 모서리 밖은 래퍼 배경(아이보리)이라 v2 T2의 "모서리로 살짝 비침" 허용 조항도 소멸한다.

### 0-4. F3 — `!important`를 없앨 수 있다 (D9″ 정정, D4′ 확장)

D4′가 `--card-bg` 변수를 도입한 이상, **카드·행의 인라인 `background`도 `'var(--card-bg)'`로 두면** 시트의 `:hover { --card-bg: … }` 한 줄이 배경과 페이드를 **함께** 바꾼다. 인라인 값 자체가 변수 참조이므로 시트가 인라인을 이길 필요가 없고 `!important`가 소멸한다(A-5의 전제 자체를 제거). `transition: background .15s`는 계산값 변화를 따르므로 그대로 작동. 행도 같은 변수를 쓴다(`.folder-row { --card-bg: var(--bg-content) }`).

```css
/* Phase 62 — 폴더뷰 카드·행. 인라인 background가 var(--card-bg)를 참조하므로 !important 불필요.
   --card-bg 하나가 배경·페이드를 함께 움직인다 — 78a780f 사고의 구조적 차단. */
.problem-card, .folder-row { --card-bg: var(--bg-content); }
.problem-card:hover { --card-bg: var(--block-bg-active); box-shadow: 0 4px 14px rgba(0,0,0,0.08); }
.folder-row:hover   { --card-bg: var(--block-bg-active); box-shadow: 0 2px 8px  rgba(0,0,0,0.06); }
```

### 0-5. F4 — 드래그 중 밀어내기 transition도 꺼야 한다 (P5의 우측판)

P5는 사이드바 transition만 짚었지만, 우측 패널도 같은 구조다: Row1~3 `transition: 'padding-right 0.2s'`(EditorView:3139·:3284·:3480), ProblemView 제목바·래퍼 `0.18s ease`(:550·:678), 글자크기·토글 `right 0.18s ease`(:734·:786). **현행도** 드래그 중 본문이 패널을 0.2s 뒤따른다(값이 바뀔 때마다 transition이 재시작). 훅이 `dragging`을 내주므로 **드래그 중엔 `'none'`**으로 — D19′와 같은 처방을 우측 5곳에 적용. 단계 1 "동작 불변" 기준의 두 번째 의도된 예외로 명기(첫째는 스냅 제거).

### 0-6. F5 — 우측 단 토글 버튼은 패널 열림 중 무효 (D16′ 보완)

D16′로 패널이 열리면 우측 단이 사라지지만 토글 버튼(:777-791)은 남아 **보이지 않는 열을 켜고 끈다**. 패널 열림 중에는 토글도 **미렌더**(`!panelMode`)가 일관적이다. `right` 산식(:782)은 D16′대로.

### 0-7. F6 — `max` 함수는 렌더 중 호출 금지 (SSR)

`max: () => Math.min(480, innerWidth*0.33)`는 **드래그·`resize` 이벤트 안에서만** 호출한다. 초기 렌더에서 평가하면 Next.js SSR(`'use client'`도 서버 렌더된다)에서 `window` 참조로 죽는다. 훅 계약(§6)에 명기.

---

## 1. v1에서 뒤집힌 것 (실측 정정 — v2, v3 전건 승인)
...
## 5. 결정 (D1′~D20′ — v2 덕수 승인 7건 + v3 F 반영)
...
| **D2′** | **카드 = 클레이 카드.** 인라인 `background: var(--card-bg)`(`:557`, 기본값 = `--bg-content`를 globals `.problem-card`가 공급 — F3) · `border: 0.5px solid var(--border-content)`(`:558`) · radius 12 유지. **신규 토큰을 만들지 않는다** — `#DDD3C3`은 기각 전례와 동급(§3-D). "클레이 영역의 경계선 색이 이제 클레이 카드의 경계선"으로 서사가 그대로 이어진다 | E4 · P3 |
| **D3′** | **hover = `--block-bg-active`(#E8DFCE) + 현행 그림자.** 화면 최악(가장 어두운) 배경이 지금과 **동일하게** 유지되므로 Phase 58·59·59a 명암비 판정이 전부 그대로 유효하다 | P2 |
| **D4′** | **페이드·배경 하드코딩을 변수로 소거.** `.problem-card`·`.folder-row`에 `--card-bg`를 정의하고 `:hover`에서 그 변수만 갈아끼운다. **카드·행의 인라인 `background`도 `'var(--card-bg)'`**(F3 — 그래서 `!important`가 없다). 페이드는 `linear-gradient(180deg, transparent 0%, var(--card-bg) 100%)` → **hover용 페이드 규칙 자체가 사라지고** 하드코딩 rgba 2곳이 소멸한다. `transparent`는 현대 브라우저가 premultiplied alpha로 보간하므로 회색이 끼지 않는다(주석의 우려는 그 이전 세대 것) — **T1에서 육안 확인** | A-6 · `78a780f` 재발 차단 |
| **D5′** | **가로 구분선 2개 모두 제거**: FolderView:454(하위폴더 행) · ListView:102(제목행). 구상이 지목한 선이 어느 쪽이든 결과가 같다 | E6 · 구상 |
| **D6″** | **리스트 행 = 가로로 긴 클레이 카드.** 인라인 `background: var(--card-bg)`(기본 클레이, F3) · `border: 0.5px solid var(--border-content)` · `borderRadius: 8` · `padding: '9px 14px'`(세로 두께 현행 유지, 라운드 때문에 좌우만 +4) · `marginBottom: 4`(아이보리 바닥 노출 · 카드 gap 20보다 훨씬 좁게) · `borderBottom` 삭제 · `transition: 'background .15s, box-shadow .15s'` 추가. hover는 **인라인 핸들러를 걷어내고 CSS로**: `.folder-row:hover { --card-bg: var(--block-bg-active); box-shadow: 0 2px 8px rgba(0,0,0,0.06); }` (인라인 `background: 'var(--card-bg)'`, F3). 그림자는 카드(4px 14px)보다 작게 — 간격 4px에 맞춘다. ⚠ **`problem-card` 클래스를 붙이지 말 것**(A-15 예외가 딸려온다) → 별도 `folder-row` | 구상 · A-11 · A-15 |
| **D7‴** | **제목행 = 행과 같은 기하의 고정 카드, sticky 래퍼 안에.** 래퍼 `position:sticky; top:0; zIndex:2; background: var(--bg-functional); padding: '8px 0 4px'`(F2 — 위 8px 띠를 아이보리로 덮는다) · 카드 `background: var(--block-bg)`(#F0EAE0 — **F1**: 행보다 한 단, hover보다 밝음. `--bg-hover`는 클레이보다 어두운데다 hover 전용 토큰이라 폐기) · `borderRadius: 8` · `padding: '6px 14px'`(행과 좌우 정렬) · `borderBottom` 삭제 · 카드 marginBottom 0(래퍼 하단 4px가 대신). **배경이 있어야 스크롤된 행이 비치지 않는다**(A-12). sticky 성립은 A-13이 보장(래퍼가 sticky 대상이 되어도 조상 overflow 조건은 동일) | 구상 · A-12·A-13 · F1·F2 |
| **D8′** | **ListView 좌우 인셋 16 → 0, 상단 0**(`padding: '0 0 32px'` — 상단 8px는 F2 래퍼로 이동). 행 폭 = 1136px = 제목바·하위폴더 행과 같은 컨테이너 폭이므로 문항 제목이 폴더 제목과 세로로 정렬된다 | A-10 · 구상 |
| **D9″** | **hover 규칙은 globals.css 한 곳에 모은다**(카드·행 둘 다). `.problem-card` 규칙이 이미 globals.css:744-746에 있어 자연스럽고, "공용 컴포넌트가 붙이는 클래스는 스타일도 공용"(Phase 61b 교훈)에도 맞는다. FolderView:540-544의 인라인 `<style>`은 삭제. **`!important` 0건** — 카드·행의 인라인 `background`가 `var(--card-bg)`를 참조하므로 시트가 인라인을 이길 필요가 없다(F3, v2의 "계속 필요"를 정정). 규칙 본문은 §0-4 | A-5 · F3 · CLAUDE.md |
...
| **D16′** | **ProblemView: 패널이 열리면 우측 단과 그 핸들을 렌더하지 않는다**(`rightOpen && !panelMode`). 이유 셋: ① 핸들만 패널 위에 살아남는 새 버그를 원천 차단(E2) ② 228~368px 死공간 제거(E3) ③ 어차피 보이지 않던 열이다. `rightOpen` 상태는 유지되므로 패널을 닫으면 그대로 돌아온다. **토글 버튼(:777-791)도 같은 조건으로 미렌더**(F5 — 보이지 않는 열을 켜고 끄는 버튼을 남기지 않는다). 좌표 연쇄도 함께 정리: `:732`·`:782`의 `right`를 `16 + (panelMode ? panelWidth + 8 : (rightOpen ? rightWidth : 0))`으로 | E2·E3 |
...
export function useDrawerResize(opts: {
  defaultWidth: number;
  min: number;
  max: number | (() => number);   // 창 비례 상한은 함수로 — 드래그·resize 이벤트 안에서만 호출(F6, SSR)
...
부수 규약: `el.setPointerCapture(pointerId)` + el 리스너(B-1 방식) · 드래그 중 `document.body`의 `cursor:'col-resize'`/`userSelect:'none'` · pointerup과 **언마운트 양쪽에서** 복원 · `window.resize`에서 재클램프 · **소비처는 `dragging`으로 밀어내기 transition을 끈다**(F4).
...
components/problem/FolderView.tsx
  ...
  :557        background: 'var(--block-bg-active)' → 'var(--card-bg)' (D2′·F3 — 기본값은 globals의 .problem-card가 공급)
...
components/problem/ListView.tsx
  ...
  :99-109     제목행을 sticky 래퍼(background --bg-functional, padding '8px 0 4px', zIndex 2)로 감싸고
              카드에 background var(--block-bg)/borderRadius 8/padding '6px 14px', borderBottom 삭제 (D7‴)
  :122-132    행: className="folder-row", background 'var(--card-bg)'/border/borderRadius/marginBottom/transition 추가,
              borderBottom 삭제, padding '9px 10px' → '9px 14px',
              onMouseEnter/onMouseLeave 2줄 삭제 (D6″)

app/globals.css
  :744 근처   .problem-card / .folder-row 규칙을 Phase 59a Q5 예외 옆에 모은다 (D4′·D6″·D9″ — 규칙 본문은 §0-4, !important 없음)
```

(CSS 규칙은 §0-4 — v2의 `!important` 판은 F3으로 대체)
...
components/editor/EditorView.tsx
  ...
  :3139·:3284·:3480  transition: dragging ? 'none' : 'padding-right 0.2s' (F4)
...
components/problem/ProblemView.tsx
  ...
  :550·:678·:734·:786  transition을 dragging(둘 중 하나) ? 'none' : 현행 (F4)
  :732·:782   right: 16 + (panelMode ? panelWidth + 8 : (rightOpen ? rightWidth : 0))  (D16′)
  :777-791    토글 버튼 렌더 조건에 !panelMode 추가 (F5)
...
| 단계 | 내용 | 완료 기준 |
|---|---|---|
| **1** | **B축 훅·핸들 추출 → 댓글·agent 2곳 치환 먼저.** 폭 수치·마운트 위치 그대로 | T5 통과. ⚠ 완료 기준은 "픽셀 단위 동일"이 **아니다** — **드래그 시작 스냅(EditorView 4px · ProblemView 16px)이 사라지는 것**과 **드래그 중 본문이 0.2s 뒤따르지 않는 것**(F4)이 의도된 변화 2건이다(D11′). 그 외(최소 360·최대 90vw·활성선·커서·pointer capture)는 동일 |
| **2** | 버전 드로어(D14′·D15′) · ProblemView 우측 단(D16′·D17′) · 좌측 사이드바(D18′~D20′) | T6~T9 |
| **3** | **A축**: D1′ → D2′ → D3′·D4′ 카드 → D5′~D9″ 리스트 | T1~T4. 테두리가 흐리면 `--block-hairline #C2B7A2`로 한 단 올림(§3-D). **제목행 톤이 무거우면 `--block-bg` → 클레이로**(F1 차선) |
...
- 카드·행 배경에 `!important`를 다시 넣지 말 것 — 인라인이 `var(--card-bg)`를 참조하는 구조가 이유다(F3). 인라인을 리터럴 색으로 되돌리는 순간 hover가 죽는다.
...
- **T2** 리스트 보기: 행이 카드로 보인다 · 행 사이로 아이보리 바닥이 드러난다 · hover 그림자가 그 간격 안에서 보인다 · 제목행이 스크롤에 고정되고 **행이 비쳐 보이지 않는다 — 제목행 위 8px 띠·라운드 모서리 바깥까지**(F2 래퍼) · 제목행 톤이 행보다 한 단 진하고 hover보다 연하다(F1).
...
**B축**
- **T6** 댓글·agent 리사이즈: 최소 360 · 최대 90vw · 활성선 1.5px · 커서 col-resize — EditorView·ProblemView 둘 다. **드래그 시작 시 패널이 튀지 않고, 드래그 중 본문이 패널을 지연 없이 따라온다**(F4 — 의도된 개선 2건) · 드래그 중 텍스트 선택 안 됨 · 놓으면 body 커서 복원.
...
- **T8** ProblemView 우측 단: 150~360 조절 · 글자크기 컨트롤·토글 버튼이 폭을 따라간다 · **댓글 패널을 열면 우측 단이 사라지고 본문이 넓어진다**(死공간 0) · 패널을 닫으면 이전 폭 그대로 복귀 · 패널 위에 떠 있는 세로선이 **없다**(E2) · 패널 열림 중 우측 단 토글 버튼도 없다(F5).
...
- **T10** 새로고침 후 모든 폭이 기본값(420 · 460 · 220 · 260)으로 복귀 · `next build` + 첫 SSR 렌더에서 `window` 참조 오류 0(F6) · 드래그 중 다른 화면으로 이동해도 커서가 `col-resize`로 굳지 않는다 · 창을 좁혔다 넓혀도 패널이 화면 밖으로 나가지 않는다.
...
| **v3** | **web** | **v2 전건 재검증(§0): E1~E10 승인 · 명암비 재계산 일치 · 정정 2건(F1 제목행 톤 · F3 `!important` 소거) · 보완 4건(F2 sticky 띠 · F4 드래그 transition · F5 토글 · F6 SSR) → D7‴ · 착수본** |

*v3 — 착수본. 미결 항목 0(덕수 판정은 Stage 3 육안 2건: 테두리 단계 · 제목행 톤). 인용은 전부 origin/main `431c6f6` 기준. 사실 분쟁 시 "각자 무엇을 읽었는가"부터 대조할 것.*
