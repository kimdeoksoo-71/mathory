# Phase 61h — 정밀 검증 군더더기(garbage) 검출 구현 계획서 v3 CLI 실측 교차검토판

작성일: 2026-09-10 · 작성: CLI (Claude Code)
기준 커밋: **mathory `be03c40`** (main — Phase 61g 구현 완료·검수 대기, Phase 64 검수 진행 중). 작업 트리의 코드 파일 무변경 확인.
계보: v1(web, 2026-09-09) → v2(web, 덕수 결정 반영·착수판) → **v3 = CLI 실측 교차검토** — v2 §9 착수 점검 ⓪~⑩ 전항 실측 + 정정 E1~E9 · 보완 G1~G10 · 결정 N1~N5.
