...
- 코드는 짧고 한 목적만 — **검증할 최종 값을 반드시 \`print()\`로 명시 출력**. 단순히 식을 평가만 하고 끝내지 말 것 (그러면 stdout이 비어 결과를 회수 못 함).
- 한 응답에서는 **하나의 코드 호출**로 끝내세요. 여러 번 분할 호출하면 마지막 호출만 stdout이 잡히고 이전 결과는 누락될 수 있습니다.
- Bash 환경(Claude의 경우)에서는 \`python3 -c "...print(...)"\` 또는 \`cat > /tmp/x.py << EOF ... EOF; python3 /tmp/x.py\` 같이 **반드시 python3 실행까지 같은 명령에 포함**해서 stdout이 나오게 해야 합니다.
...
- ⚠️ **반드시 결과 값을 자연어로 명시 — 라벨만 쓰지 말 것**:
  - 좋은 예: "검산 결과 -1로 일치", "값이 4임을 확인", "풀이는 옳음(최솟값 -1 검증됨)", "오류 발견: 분모가 0이 되는 케이스 누락"
  - 나쁜 예 (절대 금지): "검산 결과:", "검산 결과: ", "결과:", "확인됨." (값·내용 누락된 라벨)
  - 응답 본문이 라벨(콜론 등)로 끝나고 그 뒤에 실제 값이 없으면 응답은 **무효 처리**됩니다.
- ⚠️ **순서 — 검산 도구를 먼저 호출한 뒤 결론을 쓰세요**:
  1. 도구로 SymPy/Bash 코드 실행 → 결과 확인
  2. **그 결과를 보고 나서** 본문 결론을 작성 (실제 수치/판정 포함)
  - 결론을 미리 적고 도구를 호출하면 결과를 반영하지 못해 라벨만 남게 됩니다.
- ⚠️ **본문(결론)에는 절대로 코드 블록(\`\`\`python …\`\`\`)이나 실행 출력 텍스트(stdout 등)를 적지 마세요.** 실행한 코드와 stdout 자체는 시스템이 자동으로 별도 부록(<details>)에 첨부합니다. 본문엔 코드 없이 자연어 결론만 — 단 결론에는 위 ⚠️와 같이 **실제 결과 값을 명확히 포함**해야 합니다.
