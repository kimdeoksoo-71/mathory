        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexShrink: 0 }}>
          {isEditor && (
            <button type="button" onClick={() => setStep('preset')} title="프리셋으로"
              style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 15, color: 'var(--text-muted, #888)' }}>←</button>
          )}
          ...
          {isEditor && (
            <button type="button" onClick={handleDone}
              style={{ padding: '6px 16px', borderRadius: 6, border: 'none', background: 'var(--accent-primary, #c96442)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              완료
            </button>
          )}
          <button type="button" onClick={onClose} aria-label="닫기"
            style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 20, color: 'var(--text-muted, #888)', lineHeight: 1, marginLeft: 4 }}>×</button>
        </div>

        {!isLoggedIn ? (
          <div style={{ padding: '28px 16px', textAlign: 'center', color: 'var(--text-secondary, #666)', fontSize: 14, background: 'var(--bg-primary, #faf9f7)', borderRadius: 8, border: '1px dashed #e0ddd7' }}>
            기호 커스터마이징은 <b>로그인</b> 후 이용할 수 있습니다.<br />
            <span style={{ fontSize: 12, color: 'var(--text-muted, #999)' }}>로그인하면 프리셋·그룹 설정이 계정에 저장됩니다.</span>
          </div>
        ) : isEditor && draft ? (
          /* ── 2단계: 세부 편집 ── */
          <div style={{ flex: 1, display: 'flex', gap: 12, minHeight: 0, marginTop: 8 }}>
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary, #666)', marginBottom: 6 }}>내 그룹</div>
              <div style={{ flex: 1, minHeight: 0 }}>
                <UserGroupEditor config={draft} selectedGroupId={selectedGroupId} onSelectGroup={setSelectedGroupId} onChange={setDraft} />
              </div>
            </div>
            <div style={{ width: 1, background: 'var(--bg-hover, #eee)', flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary, #666)', marginBottom: 6 }}>기호 카탈로그</div>
              <div style={{ flex: 1, minHeight: 0 }}>
          ...
          <div style={{ overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {PRESETS.map((p) => {
                ...
                return (
                  <div key={p.id} style={{
                    border: current ? '1.5px solid var(--accent-primary, #c96442)' : '1px solid var(--border-light, #e5e2dc)', borderRadius: 10,
                    padding: '12px 14px', background: current ? 'var(--accent-soft, #f5e6df)' : '#fff',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <span style={{ fontWeight: 600, fontSize: 15 }}>{p.label}</span>
                      {current && <span style={{ fontSize: 11, fontWeight: 600, color: '#fff', background: 'var(--accent-primary, #c96442)', borderRadius: 10, padding: '1px 8px' }}>현재</span>}
                      <span style={{ fontSize: 12, color: 'var(--text-faint, #aaa)' }}>{count}개 기호</span>
                      <div style={{ flex: 1 }} />
                      <button type="button" onClick={() => { void Promise.resolve(onApplyPreset(p.id)).then(onClose); }}
                        style={{ padding: '5px 12px', borderRadius: 6, border: 'none', background: 'var(--accent-primary, #c96442)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                        {current ? '다시 적용' : '이 프리셋으로 설정'}
                      </button>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted, #888)', marginBottom: 8 }}>{p.desc}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 16, color: 'var(--text-primary, #333)', flexWrap: 'wrap' }}
                      dangerouslySetInnerHTML={{ __html: previewHtml(p.id) }} />
                  </div>
                );
              })}

              <div style={{ marginTop: 6, paddingTop: 12, borderTop: '1px solid var(--border-light, #eee)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <button type="button" onClick={enterEditor}
                  style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid var(--accent-primary, #c96442)', background: '#fff', color: 'var(--accent-primary, #c96442)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                  직접 편집 →
                </button>
                <span style={{ fontSize: 11, color: 'var(--text-faint, #aaa)' }}>그룹·기호를 직접 구성합니다.</span>
              </div>
            </div>
          </div>
        )}
