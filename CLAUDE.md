...
- **카드(D9~D11 → 후속 강화)**: 문제 카드 radius **12** + **1px** `--border-card-problem`(= `--block-hairline`, 1.73:1) · 풀이 **0(직각)** + 0.5px `--border-content` —
...
- **덕수 검수 1차(2026-09-09, "매우 만족") 반영 6건**: ① 코칭 블록 로고·Tip·왼쪽 바 → `--mathory-red-dark`(토큰 1개) ② 리스트 칼럼 정렬 —
  제목 왼쪽 · 나머지 가운데, 헤더 동일 ③ 문제 카드 테두리 1px(2배) · 풀이 카드 radius 0 ④ 제목행 구분선이 스테퍼 아래서 끊김 —
