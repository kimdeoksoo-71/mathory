...
- **교정 = API 내용 검토, 정돈 = 분할 + 결정적 정형화 (M7 D19)** — `autoFixDeterministicIssues`의 소비처는 `lib/blockTidy.ts` 하나(편집창 broom 버튼 · 시트 가져오기 '블록 정돈' 토글). 교정 버튼에 결정적 규칙을 되돌리지 말 것. R2 머리 정리(`15.`·`정답 ③`)는 question이 아닌 탭 전부. `⇒` 규칙(Step 3)은 인라인 **앞머리**만 — 중간·display·`\iff` 무접촉
