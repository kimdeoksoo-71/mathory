...
lib/panelStore.ts         — 우측 패널 상태 스토어(열림·세션·칩·pending·초안·스크롤·캐시·폭) (M9 B)
scripts/ocrProbe.mjs      — Mathpix 옵션 연구 프로브(probe:ocr, 결과 .probe/) (M9 H)
components/layout/SectionSlide.tsx — 사이드바 섹션 상하 슬라이드(grid 0fr↔1fr · peek 전용) (Phase 67b)
...
- **탭 블록 저장은 `commitProblemSave` — writeBatch 한 번이다 (M9 F)**: 메타 update + 삭제된 탭 정리 + 탭별 delete→set. 실패하면 아무것도 안 바뀐다. 500 ops·페이로드 초과면 탭 단위로 내리고 **탭 커밋 직후마다** `origBlockIds`를 반환 id로 갱신. `origBlockIds`의 진실은 **커밋 반환 id**(재조회 아님). ⚠ delete/add 루프로 되돌리지 말 것 — 옛 저장은 중간 실패 시 **빈 탭**을, 탭 k 실패 시 다음 저장에서 **앞 탭 블록 두 벌**을 만들었다. ⚠ 문항 doc **생성**과 하위 블록을 한 배치에 넣지 말 것 — 규칙 `parentOwner()`의 `get()`은 배치 이전 상태를 본다(test:rules 67; 기존 문항 update + 블록 배치는 통과 = 66). 빈 탭은 로드·`applyVersionContent`에서 1블록 시드(읽기 실패 탭 제외, id `new-${tabId}-${nanoid()}`) — **저장 직전 시드 금지**(로컬 0 · 서버 1로 어긋난다)
- **비가시·초성 자모 정규화는 `lib/invisibles.ts` 하나다 (M9 G)**: 원인 실측 — ㄱ,ㄴ 상자 첫 행 `ㄱ`이 호환 자모 U+3131이 아니라 **초성 자모 U+1100**(모양 동일; `'ㄱ'.normalize('NFKC')`가 곧 U+1100). 순서: ① 결합 자모 연쇄를 **먼저** NFC 조합(NFD 한글 — Drive 파일명 절반 — 보존) ② 폭 0·한글 채움 제거(**ZWJ 보존** — 이모지) ③ 행머리 **비ASCII** 공백만 ④ **단독** 초성만 호환 자모로 ⑤ 자모 뒤 전각 마침표. ⚠ **전역 NFC 금지**(U+2126·호환 한자 부수 변경) · ⚠ 초성 "전 위치" 매핑 금지(NFD 음절이 쪼개져 복원 불가). 적용: 로드(`toLocal` — 저장은 로컬을 교체하지 않으므로 여기가 있어야 같은 세션이 낫는다) · `toPersistedBlock` · 정돈 선두 · OCR · 붙여넣기(`EditorView.clipboardInputFilter` — IME 무간섭) · 시트(Modal의 tidy 앞 — **stemHash 뒤**, `lib/sheetImport.ts`는 import 0이라 무접촉). 텍스트 계열만. 마커 3정규식(`GANA`·`GIYEOK`·`CIRCLED_NUM_LINE`)은 **비ASCII 공백 소비 · ASCII 들여쓰기 캡처 보존**(그룹 2개 — 소비처 `(_, ind, ch)`; 사본 `tests/chatExtract.test.mjs`·`proofread.ts` 마스킹 동기). 편집기는 단독 초성·폭 0을 점으로 보인다(범위 전체를 넣으면 NFD 초성이 전부 점)
- **블록 정돈 규칙 순서는 R0 → R1 → R2 → R3 → R4다 (M9 I)**: R0 `\section`·`\subsection`·`\subsubsection`(*) 벗기기(**분할 앞** — 그래야 `\section*{STEP1}`이 한 번에 제목 블록) · R1 분할(… `STEP n`·**`GUIDE`** 행은 `## …` 제목) · R2 머리 정리(**문제 탭: 첫 블록 첫 행 문제번호만** — `\d{1,2}.` + 공백 1개 이상 또는 단독 행 / 그 밖 탭: 번호·정답 행) · R3 결정적 세트(Step 00 section 벗기기 · 0-2b 규칙 ⑤ · 0c 번분수) · R4 trim. `STEP n`·`GUIDE`는 `collectHeadWordRanges` **하나로 두 수식화 함수에서** 보호(본문 참조 포함). 번분수는 **분수가 중첩된 묶음**의 `\frac`을 바깥·안 모두 `\dfrac`으로 — 지수·첨자 안·`\tfrac`·`\cfrac`·중괄호 없는 인자 무접촉
- **인용 인덱스는 `\tag{n}`·`(n)`이 정본이다 (M9 H)**: 규칙 ⑤(proofread Step 0-2b, `convertJamoRefs` **뒤**·같은 게이트) — 행 끝 리더(`\cdots`·`⋯`·`…`·`···`, **수식 안팎**)+`㉠`/`\text{㉠}`/`(n)` → `\tag{n}`(㉠=1 고정 매핑). ⚠ **인라인 수식 안에 `\tag`를 넣지 말 것**(KaTeX 오류 — 수식 밖 행 끝으로) · display는 닫는 `$$` 직전(라벨 둘 이상이면 무변환 — Multiple \tag) · 수식 안 리더는 토큰 2개 이상일 때만(정당한 `\cdots` 보호) · 번호 충돌은 탭 전체 기존 `\tag`로 판정해 건너뛴다. **OCR**: Mathpix는 `⋯⋯ ㉠`를 `equation_number`(`(ㄱ)`)로 인식하고도 기본 text에서 **버린다**(프로브 실측) → 라우트가 `include_equation_tags: true`, `lib/ocr.ts` `normalizeMathpixEquationTags`가 `\tag{ㄱ}`→`\tag{1}` · `equation*` 벗기기 · `align*`→`aligned`. ⚠ 태그 라벨 정규화는 proofread `normalizeTagLabels` 하나가 소유한다 — Mathpix는 가끔 리더 잔재를 태그 **안**에 넣는다(`\tag{$\cdots … \cdot($ ㄱ}`, 실측). 내용에서 `$`·리더·괄호·공백을 걷어 자모·원문자 하나면 `\tag{n}`, 판단 불가면 무접촉. 결정적 세트 맨 앞(Step 000)에도 걸려 정돈이 기존 문항의 깨진 태그를 고친다(이 옵션 때문에 OCR 다행 수식은 `array`가 아니라 `aligned`로 온다). **검증**: 태그 `인용번호미해결`(풀이 논리·계산 패스·판정자 기준) · 일괄 검증(`batch` — `batchVerify.ts` 한 곳이 설정, 단건 바디 바이트 불변)의 **풀이** 검증에서만 `dropCitationFindings`(parse.ts 순수 함수 — 태그 일치 + 인용 토큰 24자 안 "미해결" 서술; `정답불일치`·격상분 제외)가 거른다. ⚠ **단건 정밀 검증에서는 절대 거르지 않는다**(스케치). OCR이 좋아지면 route의 그 호출 한 줄을 지운다
- **사이드바 hover peek는 `collapsed`와 별개 층이다 (Phase 67)**: 접힘 레일의 My·공유·최근 아이콘에 마우스를 올리면 **패널만** 본문 위로 펼친다(자리 56 고정 · 본문 리플로 0). `collapsed`를 **읽기만** 하므로 M2 F(자동접힘·강제 펼침 금지) 위반이 아니다. 판단은 `lib/sidebarPeek.ts`(import 0 · `test:peek` — 리듀서 + 렌더 파생값 `peekView`), 배선은 `hooks/useSidebarPeek.ts`. ⚠ **aside는 세 겹**(자리 `<aside>` · 패널 absolute · 콘텐츠 래퍼)이고 **래퍼가 옛 aside의 flex column을 이어받는다** — 빠지면 푸터 바닥 고정·최근 `flex:1`·My 섹션 스크롤이 함께 깨지는데 **폴더가 적은 계정에서는 우연히 맞아 보인다**. ⚠ 패널 애니메이션은 **width** — transform·clip-path 금지(폴더 ⋯ 메뉴·아이콘 피커·이동 픽커·최근 ⋮ 메뉴가 비포털 fixed라 좌표 기준이 바뀌고 잘린다). ⚠ aside z 80은 **idle 외에만** — 상시면 고정 모드에서 ShareButton 바깥클릭 배경(40) 위로 올라간다. pin 직후 자리가 자라는 200ms는 `pinning` 단계가 80을 지킨다. ⚠ 타이머 effect deps는 **phase가 아니라 seq**(`pendingSeq`·`leaveSeq`) — 레일 빠른 이동의 leave·enter가 React 18 배칭으로 한 렌더에 합쳐지면 phase가 pending→pending이라 phase deps로는 재시작되지 않는다(변이로 67ms 조기 열림 실증; 손으로 천천히 옮기면 안 보인다). ⚠ document 리스너는 **capture**(onPointerDown stopPropagation 30곳) · 핸들러에서 preventDefault·stopPropagation 금지. ⚠ `transitionend`는 `target===currentTarget` · `width`, 패널과 aside가 **다른 이벤트**(합치면 pinning이 조기 종료). transitioncancel은 듣지 않는다(폴백 타이머). ⚠ 리듀서 ctx `dragKind`는 **DnD**(useDragKind)이지 Sidebar `dragging`(리사이즈 — 접힘에선 늘 false)이 아니다. ctx는 이벤트 페이로드로. ⚠ **no-op 이벤트는 같은 state 객체**, `hold`는 안정 식별자, 소비처는 `usePeekHold(열림 불리언)` — 어기면 hold effect ↔ 재판정 리렌더 루프. ⚠ **메뉴 hold의 필요는 "메뉴를 연 채 패널·메뉴 둘 다의 바깥으로"에서만 드러난다** — 메뉴는 패널 DOM 자손이라 메뉴 위에서는 pointerleave가 안 난다(검사를 메뉴 위만 재면 hold를 빼도 통과한다). ⚠ **드롭 뒤 inside 재판정은 Chromium에선 없어도 통과한다**(오버레이 제거 뒤 pointerover 재발송) — Safari·Firefox 대비 안전망이니 Chrome 테스트 통과를 근거로 지우지 말 것. 터치는 `railClick`이 inside를 세우고 **항목 선택 9종·바깥 탭**으로만 닫힌다 · 키보드 click(`detail 0`)은 무시 · reduced-motion 규칙 없음(@media 규약) · 모양(D17)은 드로어 카드 규격(`DRAWER_RADIUS`·`--drawer-shadow`)을 open·closing에만, transition 없이 · 공유 열림·하위 펼침은 **Sidebar가 소유**(ShareTree 제어형 — 접힘으로 언마운트돼도 산다, 영속 없음) · 공유 레일 래퍼는 My와 같은 두 겹(한 겹이면 슬롯 54 → 최근 레일 y 329가 325로) · **67b 후속**: 트리거는 **레일 전체**(패널 진입 = `railEnter(null)` · 아이콘 이탈도 `railEnter(null)` · 패널 이탈만 `railLeave`) — 기준은 "레일에 80ms 머물렀는가"라 **레일 안 이동은 타이머를 다시 시작하지 않고** 열릴 때 포인터 밑 섹션(없으면 세 헤더 모두 접힘)을 연다; 고정 펼침의 패널 진입은 리듀서가 `collapsed` 페이로드로 거른다 · peek 헤더의 사이드바 버튼은 **레일과 같은 왼쪽 자리**(아니면 "열기" 위 클릭이 워드마크에 떨어진다) · ⚠ 헤더 hover 배경은 **절대배치 조각** — margin/padding으로 넓히면 다음 형제 marginTop과 겹쳐 **폴더 없는 계정에서만** 밀린다 · ⚠ 섹션 슬라이드는 `SectionSlide`(grid 0fr↔1fr, peek 전용, 펼침·접힘 동시 200ms): 접힌 내용은 슬라이드 끝에 뺀다(남기면 숨은 드롭 타깃) · 넘침 자르기는 슬라이드 중·닫힘에만(손잡이 left -12) · **grid 칸 첫 자식 marginTop은 바깥과 안 겹친다**("미지정" marginTop은 폴더 있을 때만) · **열은 `minmax(0, 1fr)`**(기본이면 긴 폴더 이름이 칸을 넓힌다) — 앞 둘은 폴더 없는 홈, 뒤는 긴 이름 하니스에서만 보였다
...
- **Row 1은 저장까지, Row 2는 탭까지 민다 (M7 D7~D10)** — 클러스터(버전·댓글·Agent·스테퍼 2)는 절대배치 `right 16 · zIndex 10 · height 56`이고 드로어 밑으로 들어간다(paddingRight는 ResizeObserver 실측 `clusterW + 16 + 3` — ⚠ **콜백 ref**로 잰다: 마운트 effect는 문항 로딩 중이라 Row 1이 없어 폭 0이 남고 저장 버튼이 클러스터 밑에 깔린다, 2026-09-12 실증). 2026-08-28의 "Row 1·2는 밀지 않는다"는 부분 철회. ⚠ FolderPathBar 래퍼는 `overflow:hidden`이 아니라 **`clip-path: inset(0 0 -320px 0)` + `zIndex 20`** — 칩 드롭다운(top:100%)이 잘리지 않으면서 가로만 자른다(clip-path는 스태킹 컨텍스트를 만든다)
- **저장은 편집기를 리마운트하지 않는다 (M7 D11)**: 로컬 블록 id는 세션 내내 유지되고 `origBlockIds`만 서버 id다. 저장 뒤 `setAllBlocks(서버본)`을 되살리지 말 것. dirty effect는 **`problem`을 보지 않는다**(저장·검증 완료의 `setProblem`은 편집이 아니다 — deps에 넣던 때의 잠복 버그 2건: 저장 직후 첫 편집의 dirty 삼킴 · 검증 완료 뒤 헛 dirty). dirty 해제는 `editSeqRef`가 저장 시작값과 같을 때만(저장 중 친 글자는 남는다). ⚠ undo·redo·복원의 세대 id 재생성·리마운트는 의도라 손대지 말 것
- **30분 자동 저장은 silent다 (M7 D12·D13)** — `AUTOSAVE_MS`, 기준점은 마지막 저장(`lastSavedAt`), 스냅샷 없음, `restoringRef` 가드(`handleRestore`는 async). Phase 55 D2 개정. 드래프트는 그대로 크래시 안전망
- **블록 클립보드는 메모리 싱글턴 (M7 D15~D18)** — 시스템 클립보드 미사용. ⌘C는 선택 텍스트가 있으면 네이티브에 양보(미리보기 드래그 복사), ⌘V는 접힘 모드에서만·빈 탭은 맨 앞. 미디어 블록은 URL 공유(D17) → **장래 Storage 정리 기능을 넣으면 참조 수 문제가 생긴다**
- **교정 = API 내용 검토, 정돈 = 분할 + 결정적 정형화 (M7 D19)** — `autoFixDeterministicIssues`의 소비처는 **둘**이다: `lib/blockTidy.ts`(편집창 broom 버튼 · 시트 가져오기 '블록 정돈' 토글)와 `lib/ocr.ts`(편집창·댓글 OCR 후처리) — M7 때 "blockTidy 하나"라 적었던 것은 틀렸다(M9 §1-H7). 교정 버튼에 결정적 규칙을 되돌리지 말 것. R2 머리 정리(`15.`·`정답 ③`)는 question이 아닌 탭 전부(M9 D26 — 문제 탭은 첫 행 문제번호만). 화살표 규칙(Step 3)은 인라인 **앞머리**만 — `\Rightarrow`·`\implies`·`\Longrightarrow` → `⇒`, `\rightarrow` → `→`, `\Leftrightarrow` → `⇔`(2026-09-12 확장); 중간·display·`\iff` 무접촉. 검증 공통 규약(`COMMON_RULES`)이 텍스트 `⇒`·`⇔`·`→`·`↔`·`∴`·`∵`를 수식 논리기호와 같은 뜻으로 읽으라고 일러 둔다(정돈이 꺼낸 화살표를 표기 오류로 지적하지 않게)
- **경우 라벨은 AI 경계에서만 붙인다 (M7 D20)** — raw_text·검증 `text`에 넣지 말 것(인용 앵커). 검증은 `[블록 n] (case C1)` 헤더(`caseLabel` 필드 — `normalizeBlocks`가 재구성하므로 거기서도 통과), 토론·⌘J는 `C1. ` 접두. 원천은 `buildCaseLabels` 하나. 토론 경로는 `d.data()`에 doc id가 없어 `id: d.id`를 실어야 옛 블록의 키가 안 겹친다
- **CM 툴팁의 부모는 body가 아니라 0×0 fixed 호스트다 (M7 D25′, 2026-09-12 사고)**: CM은 `tooltips({parent})` 컨테이너에 `view.themeClasses`를 통째로 붙이므로 `EditorView.theme({'&': …})`의 **모든 `&` 규칙이 그 컨테이너에도 적용**된다. body 직속이던 때 `'&': { height: '100%' }`가 뷰포트 높이 div를 블록 수만큼 만들어 문서가 넘쳤다(실측 3블록 → 713×3 = 2139px, 켬·끔 동일 — Phase 65 S11 이후 편집창 휠 끝에서 화면 전체가 밀리던 버그). 호스트에 `overflow`·`transform`을 주지 말 것. 편집창 스크롤러 2개는 `overscroll-behavior-y: contain`(D22) + `window` 스크롤 트립와이어(D23)로 이중 방어. `html, body { overflow: hidden }`은 넣지 않는다(공개 라우트·폰 셸)
- **ProblemView 제목행 우단 클러스터 = 댓글·agent·스테퍼 (M7 D26)** — 절대배치 래퍼 하나, `rightReserve` 미가산이라 드로어가 열리면 셋이 함께 덮인다(의도). h1엔 제목·원본인증 배지만
- **사이드바 헤더에 bold 아이콘·700 글자는 없다 (M7 D28~D30)** — M6 검수 6차 판정 철회(덕수 2026-09-12). 위계는 아이콘 톤(`--text-primary` vs 행 `--text-secondary`) 하나
...
- **색 팔레트는 아이보리·클레이·빨강 셋이다 (M6 색 정리, 덕수 2026-09-09)**: 상태·강조·선택·하이라이트에 파랑·초록·노랑·구글 레드를
  ...
  경고(lint warning·시트 warn 배지) = `--mathory-red` · 강조 텍스트·칩 = `--mathory-red-dark`/`--accent-primary` · 선택·hover 틴트 =
...
- **코칭 블록 (Phase 59a)**: `coach_important` · `coach_caution`. GitHub alert의 시각 문법(왼쪽 색 바 + 아이콘 + 라벨)을 **블록 타입**으로 가져왔다 — `> [!IMPORTANT]` 인라인 문법은 지원하지 않는다(블록이 편집 단위이므로 **타입이 의미를 나른다**). 라벨은 raw_text가 아니라 렌더가 붙인다(경우 번호와 같은 방침) → `lib/coachBlock.ts` + `components/ui/CoachLabel.tsx`를 5사이트가 공유한다. 색은 **M6 후속(덕수 2026-09-09)부터 `--coach-important: var(--mathory-red-dark)`** — 로고·Tip 라벨·왼쪽 바가 전부 `--coach-accent` 하나를 읽으므로 토큰 하나로 셋이 함께 빨강이다(경우 dot과 같은 레드 — 구조 신호 색 통일). 옛 보라 `#6639ba`(GitHub light 700 계열)는 폐기. 명암비 카드면 3.80 · hover면 3.28로 **텍스트 4.5:1 미달을 알고 채택**했다 — 라벨 식별은 색이 아니라 **아이콘 + 600 굵기**가 담당한다(코칭 색과 dim 본문 `#675F52`의 대비가 1.2:1뿐이던 때부터의 규약). ⚠ 상자 안쪽 패딩(1em)은 **들여쓰기가 아니라 상자의 내부 여백**이다(글상자 3종의 전례). 바깥 좌단은 본문 0이라 경우 사이에 끼어도 rail(-1.8em)과 겹치지 않는다
...
## 현재 Phase: **개선묶음 M9 — 각종 기능 개선·버그 수정** — 구현 완료(2026-09-19) · 9커밋(S0~S8) · push 대기 · **덕수 실물 검수 대기**
...
폴더뷰 클릭(A) · 우측 패널 지속(B) · AI 비용(C) · 폰 가로 크롬(D) · ⇒ 행 위치(E) · 빈 탭(F) · ㄱ. 내어쓰기(G) · 인용 인덱스(H) · 블록 정돈 조정(I).
**Firestore 규칙 0 · Storage 규칙 0 · 스키마 0 · 렌더 5사이트 0 · 폰 세로 0.** 신설 3(`lib/invisibles.ts` · `lib/aiPricing.ts` · `lib/panelStore.ts`) + 프로브 1 ·
로직 테스트 21종 **539건** · `test:rules` 69. **규약은 「핵심 패턴」 맨 앞의 M9 절 7개가 소유한다.**

- **스케치를 뒤집은 원인 셋**: E는 분리 결함이 아니라 렌더 정규식(`⇒ $…$` 행을 수식 전용 행으로 못 봐 문단 합침) · F는 삭제 가드가 아니라 저장 중간 실패·로드 무가드·`+` 부재 · G는 폭 0 문자가 아니라 **초성 자모 U+1100**
...
- ⚠ 남은 일: 덕수 실물 검수(실행판 §9-4) · push 뒤 Vercel 빌드 로그 확인 · `claude-reporter` ai_models 단가 15/75 → 5/25(꺼져 있음) · Vercel 미사용 env `ANTHROPIC_MODEL` 정리(선택)
...
- **67b 후속(2026-09-17, 확정본 §14)**: 덕수 실사용 요청 3건 — 레일 전체가 peek 트리거(열기 버튼·새 문제·검색·시트·빈 곳·푸터, 섹션 아이콘이 아니면 세 헤더 접힌 채로) · 섹션 헤더 hover 배경(peek·고정 펼침) · peek 중 섹션 상하 슬라이드(펼침·접힘 동시 200ms). 결정 11건 전항 권장안. 커밋 P1~P5 · 신규 `SectionSlide.tsx` · `test:peek` 47 · 로직 검증 **495건**. 가장 값비싼 발견: margin으로 넓힌 hover 배경과 grid 슬라이드 칸이 **폴더 없는 계정에서만** 행을 밀었고, grid 기본 열이 **긴 폴더 이름에서만** 칸을 넓혔다 — 폴더 없는 홈과 긴 이름 하니스를 둘 다 기준으로 둔 것이 살렸다. **덕수 실물 검수 종결(2026-09-17, "대만족 · 잘 작동")** — 트리거 체감·열기 버튼 자리 클릭·hover 배경·슬라이드 체감. 열림 지연 80ms 유지 확정(결정 4)
...
ICONS 56 → **61종**(+9/−4, toggle 2종은 후속 철회 · user-circle 5차 · 헤더 bold 3종 6차) · 자산 3,024 → **1,512개**(regular 단일) · 커밋 S1~S12 + 검수 후속 6(1차 6건 · 2차 카드 그림자 · 3차 색 팔레트 전수 정리 · 4차 체크박스·파비콘 · 5차 My user-circle · 6차 헤더 bold·13.5/700) · 로직 검증 371 → **373건**.
...
- **카드(D9~D11 → 검수 2차 확정)**: 문제 카드 radius **12** + 은은한 그림자 `--card-shadow-problem`(2겹, 드로어의 절반 강도) · 풀이 **0(직각)** · 그림자 없음.
  **테두리는 둘 다 0.5px `--border-content`** — "읽는 대상 / 쓰는 대상"을 모서리와 그림자로 가른다. ⚠ D10의 hairline 테두리와 검수 1차의
  1px은 검수 2차에서 "부담스럽다"로 **철회**됐고 `--border-card-problem` 토큰은 삭제했다(소비처 0) — 테두리로 차별화하지 말 것.
  판별 `!isToneScoped(tab.id)` · peek 카드도 12(그림자는 `--drawer-shadow`)
...
- **잔여 도안**: 파비콘 Pretendard SemiBold 'M' path(D15, 실측 0.999 일치) · ~~토글 Phosphor 24(D17~D19)~~ **철회 → 옛 트랙 도안 18×11** · IconSave cloud(D20) ·
  IconComment `chat-dots`(D21′) · 받은 문항 `rotate(180deg)`(D24) · BlockchainBadge `--text-muted`(D25)
- **사이드바(D22·D23′)**: `SidebarSectionHeader` 공용(My **IconUserCircleBold**(검수 5차 — 폴더 행과 겹치던 folder 대체) · 공유 IconShareBold · 최근 IconRecentBold — 검수 6차부터 **bold 16 · 글자 13.5/700**, 래퍼 `'8px 12px'`) · ⚠ **M7 K(2026-09-12)에서 bold·700 철회** — regular 16 · 아이콘 `--text-primary` · 글자 13.5/500 `--text-secondary`
...
- **덕수 검수 1차(2026-09-09, "매우 만족") 반영 6건**: ① 코칭 블록 로고·Tip·왼쪽 바 → `--mathory-red-dark`(토큰 1개) ② 리스트 칼럼 정렬 —
  제목 왼쪽 · 나머지 가운데, 헤더 동일 ③ ~~문제 카드 테두리 1px(2배)~~(검수 2차에서 원복) · 풀이 카드 radius 0 ④ 제목행 구분선이 스테퍼 아래서 끊김 —
  불투명 절대배치 래퍼(57)가 border-box 헤더의 아래 1px 선을 덮고 있었다 → 래퍼 `HEADER_H − 1` ⑤ ToggleSwitch를 옛 트랙+손잡이로
  **복귀**(D17~D19 철회), 22×13 → 18×11 — `toggleOff/On` ICONS 삭제(59 → **57종**) ⑥ Row 2 줄바꿈 아이콘 20 → 16(80%)
- **덕수 검수 3차(2026-09-09) — 색 정리**: ① 사이드바 '시트 가져오기' 아이콘 14 → 18(이웃과 동일) ② 체크박스 전역 accent-color(팝업 2곳의
...
문서: `docs/phasedocs/Phase61h 정밀 검증 군더더기 검출 v4 실행판.md`
(계보: v1 web → v2 web 덕수 확정·착수판 → **v3 CLI 실측 교차검토**(정정 E1~E9·보완 G1~G10·결정 N1~N5 전부 권장안) → **v4 = 실행판**. §11이 구현·프로브 기록)

덕수 정의 — **결론에 영향을 주지 않아도** ① 무관한 서술 ② 중언부언 ③ ⟺로 이을 수 있는데 한쪽 조건만으로 간 느슨한 서술 — 을
결함과 **별도 축**으로 검출한다. 같은 뼈대(1차 후보 → 2차 판정 → 코드 합성)를 쓰되 1차는 별도 패스, 2차는 별도 판정자, 리포트는
별도 절(`report.garbage`)이고 **종합 판정에 들어가지 않는다**. **원본이 이 파일(`prompts.ts`)이고 시트 STEP4가 후속 이식**한다(61g와 반대).
**Firestore 규칙 0 · 스키마 0(`VerifyReport.garbage` additive) · `VerificationState` 0 · 목록 배지 0 · 일괄 검증(61d) 0 · 그림 경로(61f) 0 ·
클라 흐름(`verifyFlow`) 0 · 결함 프롬프트 2개(`_LOGIC`·`PROMPT_JUDGE`) 무변경 · 문제 검증 바이트 무변경.**
수정 7파일 · 신규 0 · 커밋 S1~S5 · 로직 검증 80 → **87건**(`test:verify` 46 → 53).

- **⚠ 군더더기는 종합 판정에 안 들어간다** — `synthesizeVerdict`에 `garbage`를 넘기지 말 것. `✓ 이상 없음 · 군더더기 2건`이 정상 상태다.
  **예외는 escalate뿐**: 군더더기 판정자가 "이건 결함"이라 넘긴 후보는 `findings`에 **`check`**로 들어가 verdict를 `check`까지 올린다
  (`fail`은 절대 아니다 — D5). reason 접두 `[군더더기 검토에서 격상]`이 출처다(N4)
- **⚠ 판정자는 둘이고 성향이 반대다** — 결함 `PROMPT_JUDGE`는 "남기는 쪽"(`CONSERVATIVE_RULES`), 군더더기 `PROMPT_GARBAGE_JUDGE`는
  "확신할 때만 valid". **합치지 말 것**(D4). 1차도 마찬가지 — `garbageRecallRules`에 `recallRules`의 "놓치는 쪽이 훨씬 나쁩니다"가
  **일부러 없다**(T7이 부재를 고정). 군더더기의 비용 계산은 결함과 반대다(놓쳐도 틀리지 않는다)
- **⚠ 1차 게이트(전부 실패 → 502 · 전부 skip → skip 리포트)는 결함 패스 기준이다** — `FirstPass.severity`가 없는 패스만 본다(E4·N1).
  군더더기 패스까지 세면 결함 패스가 다 죽거나 다 skip이어도 "ok + 군더더기"가 나와 61b D13′·61f B-8이 조용히 깨진다.
  군더더기 패스의 실패·skip은 "군더더기 없음"으로 흘린다. **프로브 `runOne`도 같은 두 줄**
- **⚠ 군더더기 판정자(2차) 실패는 결함 리포트를 죽이지 않는다(N2)** — 절만 빼고 `note`에 "군더더기 판정 실패". 별도 축이라 D13′의
  보호 대상(결함 판정)이 훼손되지 않고 1차 후보도 노출되지 않는다. 결함 판정 실패는 종전대로 502
- **구조 블록(`(heading)`·`### 제목`·coach·callout)은 대상이 아니다** — 프롬프트 [4] 문장으로. 오탐이 실측되면 텍스트 치환(블록 번호 보존)으로
- **태그 정규화는 별도 함수 `normalizeGarbageTag`** — `normalizeTag` 폴백이 `논리오류`라 섞으면 군더더기가 결함으로 샌다. ⚠ 힌트 순서
  **무관 → 중복 → 느슨**이고 `'필요'`·`'충분'` 단독 힌트 금지 — `'불필요'`가 `'필요'`를 품어 v2 초안이 자기 T1을 통과 못 했다(E3)
- **병합 키에 severity가 앞선다** — 같은 인용의 결함·군더더기 후보는 둘 다 남는다(축이 독립). 상한 계산 8 · 논리 12 · **군더더기 6** ·
  병합 **26**(합). `SOLUTION_FIRST_PASSES`는 `FirstPass[]` — 원소 하나에만 `severity`를 두면 배열 타입이 합집합이 되어 `p.pass.severity`가
  TS2339로 막힌다(v3 §9-① 실증) → 문제 패스까지 넷 다 `FirstPass`
- **카드**: 군더더기 절은 결함 아래 · 확정 칩은 `--bg-warn` 틴트(결함색 `--accent-danger` 금지) · 좌측 띠·머리 아이콘은 결함 verdict만.
  폰(`PhoneApp`)은 같은 `CommentPanel`이라 코드 0
- **프로브 2차 `maxTokens 16000`은 라우트 32k와 갈린 옛 값이다(E9·N3)** — 측정 연속성 때문에 두었다. `truncated`도 프로브엔 없다
- 스모크(2026-09-10, 자작 문항): 군더더기 후보 4 → 확정 2(중복·무관) · 기각 2 · 격상 0 · verdict `ok` 유지 · **군더더기 판정 184초**(후보 4건).
  ⚠ 200초 문턱에 가깝다 — Stage 2·3에서는 군더더기 판정 최장 52.5초라 이상치였다(실행판 §11-3)
- **프로브 Stage 2·3(2026-09-10, n=10×2)**: 결함 기각률 57%(= 61g 베이스라인 56%, 61h가 결함 축을 안 흔들었다) · 군더더기 1차 후보 평균 1.14 ·
  태그 무관 4/중복 2/느슨 2 · 확정 2·확인 2·기각 4 · escalate 실물 0 · 1차 실패 0. ⚠ **`suggestion`은 코드가 valid일 때만 싣는다** —
  판정자가 uncertain에 "삭제"를 채웠다(프롬프트 [3] 불이행 실측). ⚠ D7 구조 블록 오탐은 프로브로 못 잰다(시트 말뭉치는 전부 `text`) — UI 검수 몫
...
(계보: v1 web → v4 CLI 실측 교차검토 → **v5 = 실행판**. v2·v3는 만들지 않았다.
 §3이 v1 정정 4건·보완 9건, §3-1이 v4 → v5 착수 점검 보강 4건)

시트 STEP3가 결함 유형을 2 → **6종**으로 넓힌 것(`prompts/pmt.csv` `dda7bba` ·
`QualityVerification.gs` `95556fc`)을 풀이 **논리 패스**에 이식했다.
**서버 로직 0(상한 배선만) · Firestore 규칙 0 · 스키마 0 · 마이그레이션 0 · 리포트 카드 0 ·
클라이언트 0 · 렌더 5사이트 0 · 전처리 0 · 그림 경로(61f) 0 · 일괄 검증(61d) 코드 0.**
수정 5파일 · 신규 0 · 커밋 S1~S5 · 로직 검증 365 → **371건**. 프로덕션 빌드 통과.
**규약은 아래 Phase 61b 절이 소유한다**(태그 체계 · 상한 단일 출처 · D5 비이식).

- **논리 태그 4 → 7종**: `근거없는가정`·`충분성미확인` 신규 + `수식비일관`을 논리 패스에서도 발급.
  1차에 "답이 맞더라도 논증이 정당화 못하면 결함" 전제와 **전역 검토** 관점(조건이 다 쓰였나 ·
  경우가 전체를 덮나 · 확정 전 충분성을 봤나)이 들어갔다
- **2차에 유형별 valid 기준**(`[2']`)이 생겼다. 판정 근거를 실제 수학적 검증으로 요구한다
- **상한**: 논리 패스 8 → **12**, 병합 12 → **20**(= 패스 상한의 합, 병합은 더 이상 자르지 않는다)
- ⚠ **문제 검증 1차는 무변경**이다(패스가 하나·cap 8이라 병합 상한이 예나 지금이나 안 걸린다).
  2차는 `PROMPT_JUDGE` 공용이라 `[2']` 절이 함께 붙는다
- ⚠ **착수 후 프로브에서 반드시 볼 것**: ① 후보 0건 비율 ② 2차 기각률(61b 베이스라인 **56%**)
  ③ 2차 소요 시간(한 문항이라도 **200초 초과**면 `MERGE_CANDIDATE_CAP`을 16·14로 내린다)
...
수정 6파일 · 신규 0 · 아이콘 55 → **56종**. 로직 검증 **365건 무회귀**.
...
- **덕수 검수 보완 — 거터(2회 왕복, 1차는 전량 원복)**: 폭 2/3·구분선 강화를 넣었다가 되돌렸다.
  ⚠ **`minWidth 1.8em`은 여유가 아니라 2자리 번호가 꽉 채우는 값**이라, 낮추면 1자리 블록과 2자리 블록의
...
- **덕수 검수 종결(2026-09-08, "모두 정상")** — 반영 1건: **아이콘 ↵ 단일 + 켬일 때 박스**(D9′).
  ...
  보정이 필요 없었다)
- ⌥Z는 window 리스너라 **항상 동작**한다. ~~Row 2 버튼은 `showToolbar` 게이트 아래라 그림 블록이 활성이면 눌리지 않는다~~ ⚠ **M7 후속(2026-09-12)에서 게이트를 항목별로 갈랐다** — 블록에 넣는 것(강조·상용구·특수문자·표·AI 완성)과 좌측 `$`·`$$`·팔레트만 활성 텍스트 블록이 필요하고, 탭 단위 동작(정돈·교정·찾기·전체 접기·블록 복사/붙여넣기·줄바꿈)은 그림 블록이 활성이어도 눌린다(시트로 가져온 문항은 첫 블록이 그림이라 정돈 버튼이 흐렸다)
...
- 덕수 검수 **종결**(2026-09-06, "다 만족"): Q1~Q7 전항 정상. 반영 3건 — ① agent 드로어
  ...
  **짝 간격 축소**(S10 — 글리프 간격 ~28→~9px, 레버는 UnifiedToolbar의 marginLeft·
  `legacyMathSvgProps(shift)`)
- **배포 완료(2026-09-06)**: Vercel 빌드 로그에 `[icons:check] OK — 55종` ·
  **`[icons:assets] OK — 3024개`** 실측 확인(덕수) — 자산 생성 배선이 Vercel에서
  실제로 돈다. 유일한 배포 함정(Build Command 직접 지정 → 아이콘 전부 빈칸)은 해소
...
- **⚠ 두 형식이 한 셀에 섞일 수 있다 — 표본은 아직 못 봤다**: 패치 11은 M/N열에서 링크를 못 찾은
  ...
  지키고 있다
- **⚠ 세 번째 형식 — 좌우 배치 그림은 Mathpix figure 환경이다 (2026-09-15 후속, 실행판 §11)**: 나란한 그림은
  `\begin{figure}\includegraphics{https://cdn.mathpix.com/cropped/…}\caption{…}\end{figure}`로 오고 **GAS가 Drive로 옮기지 않는다**
  → CDN 주소가 키, 프록시 `?url=` 갈래, 블록은 [그림·캡션]으로 위아래. ⚠ 스캔에서 figure 환경 갈래가 구형 태그 갈래보다 **앞**이어야
  한다(뒤면 잔재가 남는다) · ⚠ `MATHPIX_FIG_URL_RE`는 프록시 SSRF 게이트라 넓히지 말 것 · ⚠ 파일명에 `[`·`]`가 온다(`[2027]…`) —
  alt는 균형 대괄호로 받는다. Mathpix 주소는 만료될 수 있다(그 그림만 리터럴로 남는다)
...
- **⚠ EditorView Row1·Row2는 '덮는다' (개선묶음 M2 R6) — ⚠ M7 D7~D9가 부분 철회: Row 1은 저장 버튼까지, Row 2는 탭까지 **민다**. 클러스터(버전·댓글·Agent·스테퍼 2)만 덮인다(아래 M7 절)**: 아래 "덮지 않고 밀어낸다" 규약을 그 두 행에 한해 의도적으로 깼다 — 패널을 여닫을 때마다 제목·탭이 줄었다 늘었다 하며 "편집 대상이 바뀐 것처럼" 보였다. **Row3(content-frame)의 밀어내기는 그대로**다(본문까지 덮으면 편집이 막힌다). 대가로 Row1 우측 끝의 버전 기록·글꼴 조절이 패널 뒤로 숨는다
...
- **2차 판정 모델은 Opus 5다 (2026-09-10, 4.8에서 전환 — 같은 단가 $5/$25)**: 같은 행 10개를 고정한 A/B에서 4.8은
  실재하는 표기 결함 4건(행 1543 — 기호가 깨져 흰 공·검은 공이 둘 다 ◯)을 **전부 기각**(기각률 12/13 = 92%)했고 Opus 5는
  확정했다(기각률 6/12 = 50%, 베이스라인 56%와 같다). 판정 최장 52.6s → 38.1s. ⚠ 1차(Gemini)가 비결정적이라 두 판정자가 받은
  후보 집합이 같지 않다 — 판정자만 가르려면 저장한 1차 후보를 두 판정자에 재생하는 `--judge-only`가 필요하다(후속). 기본값은
  `route.ts`·`ai-provider.ts`·프로브 셋이 함께 움직인다(env `VERIFY_CLAUDE_MODEL`로 덮어쓸 수 있다). agent 대화의 Opus 5는
  Firestore `ai_models` 문서(콘솔 전용 쓰기)가 따로 결정한다
- **Opus 4.8은 `thinking`을 생략하면 사고가 꺼진 채 돈다** — 오류·경고 없이 품질만 조용히
  떨어진다(Opus 5는 기본 켜짐이지만 `thinking:'adaptive'` 명시를 유지한다). `budget_tokens`(400) · assistant prefill(400) ·
  `tool_choice:{type:'any'}`(최종 JSON 턴 불가)는 전부 금지
...
- **풀이 1차는 계산·표기 / 논리 / 군더더기(61h) 세 패스다 (병렬 필수)** — 셋째는 별도 축이라 1차 게이트에 참여하지 않는다(위 61h 절): 한 프롬프트에 태그 7개를 넣으면 눈에
  띄는 표기·계산이 먼저 소모되고 논리가 묻힌다(실측: 논리 6건 중 1건만 검출). 시트도 STEP2·STEP3로
  나눠 돈다. ⚠ **직렬로 보내면 300초 예산이 무너진다** — `Promise.all`
  ⚠ **상한은 패스마다 다르다 (Phase 61g)**: 계산 8 · **논리 12** · 병합 **20**(= 합이라 병합
  단계는 자르지 않는다). 61g 이전에는 병합이 12여서 이어 붙이기 구조상 계산 8건이 앞자리를 채우고
  **잘리는 쪽이 언제나 논리**였다. 교차 병합은 답이 아니다 — 계산 cap이 8인 한 결과 집합이 같고
  순서만 바뀐다(61g E1). ⚠ **논리 패스는 이제 태그가 7종**이라 "눈에 띄는 것이 먼저 소모된다"는
  이 절의 실패 양상이 **한 패스 안에서** 재현될 수 있다. 시트가 6종 단일 패스로 운용 중이라 전례는
  있으나, 프로브에서 **태그 분포를 셀 것** — 논리비약이 사라지고 신규 2종만 올라오면 후퇴 신호다
...
- **후보 상한의 단일 출처는 `lib/verify/prompts.ts`다 (Phase 61g D9)**: `CAP_PROBLEM`·`CAP_CALC`·
  `CAP_LOGIC`이 각 프롬프트 객체의 `cap`으로 나가고 `MERGE_CANDIDATE_CAP`이 병합 상한을 준다.
  라우트·프로브가 그것을 읽는다. ⚠ **모델에게 알리는 수(`recallRules(cap)` 문자열)와 코드가
  강제하는 수(`sanitizeFindings`)가 갈리면 안 된다** — 프로브가 8·12를 리터럴로 따로 박아 두어
  실제로 갈렸던 적이 있다(측정이 프로덕션과 다른 상한에서 돌았다). 테스트 T6가 둘의 일치를 고정한다.
  ⚠ **상수 선언은 프롬프트 객체보다 앞줄에** — 객체의 `system`이 모듈 초기화 시점에 평가된다.
  ⚠ 파싱 결과는 **그 패스와 짝지어** 들 것(`{pass, json}`) — `alive`는 실패를 거른 배열이라
  인덱스로 대응시키면 한 패스가 죽었을 때 상한이 어긋난다
- **풀이 논리 태그는 7종이고 원본은 시트다 (Phase 61g)**: `논리비약`·`논리오류`·**`근거없는가정`**·
  `경우누락`·**`충분성미확인`**·`수식비일관`·`문제풀이불일치`. 앞의 여섯은 시트 V2 6종의 이식이고
  `문제풀이불일치`가 Mathory 고유분이다. ⚠ **`수식비일관`은 이름과 달리 서술 불일치까지 받는다**
  (시트 (F)를 태그를 늘리지 않고 흡수했다 — 계산 패스의 좁은 정의는 그대로 두었고, 좁은 것이 넓은
  것의 부분집합이라 2차 기준이 갈리지 않는다). ⚠ **태그 추가는 개명이 아니다** — E-4 체계는 그대로고
  저장된 옛 리포트의 태그는 문자열이라 그대로 남으며 카드는 매핑 표 없이 찍는다(Firestore 0).
  ⚠ `normalizeTag`의 신규 힌트는 **problem 분기 아래**에 둘 것(위로 가면 문제 검증으로 샌다).
  `추론/infer/invalid` 힌트는 **넣지 않았다** — 폴백이 이미 `논리오류`라 무익하고 뒤 힌트를 가로챈다
- **⚠ 시트 V2의 uncertain 조임 문구는 이식하지 않았다 (Phase 61g D5 — 유일한 비이식)**:
  시트 2차 [3]의 "'애매해 보인다', '검증이 복잡하다'는 uncertain의 근거가 아니다"는 위
  `CONSERVATIVE_RULES`("걸러 낼 확신이 없으면 invalid가 아니라 uncertain")와 **정면 충돌**한다.
  다음에 시트를 옮겨올 때도 빼둘 것. ⚠ 문장을 빼는 것만으로는 부족하다 — 유형별 기준 절(`[2']`)이
  항목마다 valid/invalid로 끝나는 이분법이라 **구조가 같은 압력**을 만든다. 그래서 그 절의
  **머리에** "가리지 못하면 uncertain, [2]가 우선"을 명시했다. 이 한 줄을 지우지 말 것
- **⚠ Anthropic SDK는 비스트리밍 요청에 `max_tokens > 21,333`이면 요청을 보내기도 전에 던진다 (2026-09-11 사고)**:
  0.124.0의 `messages.create`는 **스트리밍이 아니고 `timeout`도 지정되지 않은** 요청에서
  `(60분 × max_tokens)/128k > 10분`이면 `AnthropicError('Streaming is required …')`를 낸다
  (`client.js` `calculateNonstreamingTimeout`). 검증 2차 `JUDGE_MAX_TOKENS = 32,000`이 정확히 여기 걸려
  **판정이 전부 500**으로 죽었다(경계 실측: 21,333 통과 / 21,334 차단). 처방은 `ClaudeProvider` 생성자의
  `timeout` 명시 하나다(`CLAUDE_REQUEST_TIMEOUT_MS` = 10분 = 업그레이드 이전과 같은 실효 동작).
  ⚠ **값을 300초 밑으로 내리지 말 것** — SDK가 타임아웃을 재시도 대상으로 보므로(maxRetries 2) 오히려
  총 시간이 늘고 같은 자리에서 죽는다. ⚠ **호출부가 아니라 클라이언트에 건다** — pause_turn 재요청 루프와
  앞으로 생길 호출까지 한 줄로 덮는다. ⚠ SDK엔 조건이 하나 더 있다(`MODEL_NONSTREAMING_TOKENS`, 현재
  Opus 4/4.1 식별자 3개만 8,192) — 그 계열로 모델을 바꾸면 timeout을 명시해도 거절될 수 있다.
  ⚠ **가장 값비싼 함정: 프로브는 raw fetch라 이 가드를 안 지난다** — `verifyProbe`는 멀쩡한데 앱만 죽는다.
  "프로브가 통과했으니 괜찮다"가 성립하지 않는 자리다(SDK 경유 여부가 갈린다)
- **⚠ 500 응답에 사유를 함께 싣는다 (같은 사고)**: 위 사고 때 UI에도 서버 로그에도 "검증 중 오류가
  발생했습니다" 한 줄뿐이라 원인을 짚는 데 하루가 걸렸다. 이제 `redactSecrets(e.message)` 160자를 덧붙인다.
  ⚠ 오늘의 세 SDK는 키를 **헤더**로 보내고 메시지에 싣지 않는 것을 확인했지만(Gemini `RequestUrl.toString()`에도
  key 없음) 그대로 흘리지는 않는다 — 이 사고 자체가 SDK 업그레이드로 동작이 바뀐 사례다
- 로직 검증: `npm run test:verify` (53개 · 하니스 전체 87개 — 61h에서 46/80 → 53/87)
...
- **401/403/429는 즉시 전체 중단**(허용목록 미등록·토큰 만료·**AI 할당량 초과** → 이후 전건 실패), **연속 3건 실패도 중단**.
  ⚠ 429는 2026-09-10에 추가 — Gemini **일일 250회**(`GenerateRequestsPerDayPerProjectPerModel`, gemini-3.1-pro)가 프로브·배치로
  소진되자 라우트가 500 "예상치 못한 오류"를 내고 배치는 "연속 실패가 이어져 중단"만 보여 원인이 어디에도 없었다. 이제 라우트의
  `quotaExhaustedMessage`가 제공자 429를 **429 + 회복 시각**("약 20시간 뒤(9/11 09:01 이후)")으로 옮기고 배치가 그 문구로 멈춘다.
  ⚠ 61h로 **풀이 검증 1회 = Gemini 3호출**(문제 1 + 풀이 3 = 문항당 4) — 38문항 배치 ≈ 152회라 250회/일의 6할. 배치 전날 프로브를
  돌리면 다음 날 배치가 막힌다. 모델별 버킷이라 급하면 `VERIFY_GEMINI_MODEL`로 다른 모델을 임시 지정할 수 있다.
  ⚠ **2026-09-10 조치: `GEMINI_API_KEY`를 GAS와 같은 PBMAI 프로젝트(ggulsuhak, Tier 2) 키로 교체**(`.env.local`·Vercel Production·Preview).
  Gemini 키를 읽는 곳은 `lib/ai-provider.ts` 한 파일(verify·discuss·ai-complete)과 프로브뿐이라 이 한 변수로 전부 옮겨진다. 옛 Mathory
  프로젝트(gen-lang-client-0382525972, Tier 1 · 250회/일) 키는 폐기. 이제 시트와 Mathory가 한 프로젝트 버킷을 나눠 쓴다
