...
- **⚠️ Gemini의 thinking 토큰은 `maxOutputTokens`를 같이 먹는다**: thinkingLevel HIGH로 예산을
  소진하면 남은 자리에 JSON을 억지로 채우느라 `reason`이 **같은 문장의 반복·영문 진행 메모로 퇴화**
  한다(실측: 1차 출력 10,713토큰을 쓴 행에서 후보 6건 reason이 전부 쓰레기). **그런데 `quote`는
  정확했다** — 지점은 맞히고 설명만 무너지므로, 겉보기엔 "1차가 헛것을 본다"로 오진하기 쉽다.
  시트의 STEP3 1차는 `maxOutputTokens`를 아예 걸지 않는다. 넉넉히 줄 것(현재 32k).
  ⚠ **예산을 올리면 비용이 아니라 시간이 는다** — 상한은 목표가 아니라서 실제 출력 토큰은
  오히려 줄었고(35k→26k), 대신 최장 소요가 118초→228초가 됐다. 낮은 예산이 빨랐던 이유는
  thinking을 중간에 잘라먹었기 때문이고 그게 바로 필러 퇴화의 원인이었다
