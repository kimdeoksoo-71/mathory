...
- **덕수 검수 보완 3건(2026-09-08) — 거터**: ① 폭 **49 → 33.4px(68%)** ② 좌우 스크롤 시 고정은
  **이미 만족해 조치 0** ③ 구분선 `--border-subtle` → **`--block-hairline`**(대비 1.06 → 1.66:1).
  ⚠ 폭 레버는 `minWidth`가 **아니다**(2자리 번호가 이미 꽉 채우고 있었다) — `fontSize 0.82em` ·
  번호 padding · **lint 열 폭**(점 하나에 21px을 쓰고 있었다) 셋이다. 위 규약 절 참조
