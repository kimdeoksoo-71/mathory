 ...
 * ⚠️ **반각 공백(U+0020)도 허용**한다 — `[강대모의고사X] 시즌4_18회 (260914)_해설_…`처럼 stem에 공백이 온다
 *    (2026-09-16 실측: 게이트 탈락 → 텍스트 잔류 → 자동 수정이 alt를 `$4$`·`$\mathrm{X}$`로 오염).
 *    탭·개행은 여전히 배제한다(alt 스캔이 행 단위). 앞뒤 공백은 호출부가 trim한다.
