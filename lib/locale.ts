 ...
 */
// M9 D21 — 행머리 텍스트 화살표 접두(`⇒ $…$`)도 수식 전용 행으로 본다. 정돈(proofread Step 3)이
//   `$\Rightarrow x$`를 `⇒ $x$`로 꺼낸 행이 여기에 안 걸리면 세 행이 한 문단으로 합쳐지고 줄바꿈이
//   화살표 **뒤**에서 일어나 "기호가 윗행 끝"으로 보인다. ⚠ 집합은 닫혀 있다(`⇒ ⇔ → ↔ ∴ ∵`) —
//   아무 접두나 허용하면 "산문 뒤 수식 행은 이어 붙는다" 계약이 깨진다. 구분 공백 필수.
